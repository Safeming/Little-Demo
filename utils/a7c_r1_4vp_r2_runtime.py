...
def axis_angle_pose_to_rotation_6d(pose) -> np.ndarray:
    ...
    return np.swapaxes(matrices[..., :, :2], -1, -2).reshape(values.shape[0], -1)


def load_pose_rotation_6d(model_dir, frame_ids, joint_indices) -> np.ndarray:
    root = Path(model_dir)
...
def pose_manifest_sha256(model_dir, frame_ids, repo_root) -> str:
    ...
    for frame in frame_ids:
        ...
        digest.update(f"{_file_sha256(path)}  {relative}\n".encode("utf-8"))
    return digest.hexdigest()
...
def build_runtime_inputs(
    *, probe, feature_names, feature_group, pose_by_frame, camera_index,
    frame_index, carrier_ids, a5_weight, spatial_scale, depth_scale,
    ...
    forbidden = {
        ...
        "gaussian_id", "image_name", "held_block_identity",
    }
    ...
    for key, expected in (("camera_index", cameras), ("frame_index", frames),
                          ("carrier_ids", carriers)):
        if not np.array_equal(np.asarray(probe[key]).reshape(-1), expected):
            raise ValueError(f"probe {key} manifest differs")
    ...
    required = (
        "visibility", "camera_x_over_z", "camera_y_over_z", "log_depth",
        "alpha_transmittance_mass", "semantic_support_mean", "alpha_mean",
    )
    ...
    adjacency = dense_overlap_adjacency(
        ...
        visibility=torch.from_numpy(visibility), spatial_scale=float(spatial_scale),
        depth_scale=float(depth_scale),
        edge_log_weight_minimum=float(edge_log_weight_minimum),
    ).numpy()
    mass = runtime_target_mass(
        alpha_transmittance_mass=torch.from_numpy(field["alpha_transmittance_mass"]),
    ...
    pose = np.asarray(
        [_finite_array("pose row", pose_by_frame[int(frame)]) for frame in frames],
        dtype=np.float32,
    )
    if pose.shape != (features.shape[0], 36):
        raise ValueError("runtime pose must have shape [samples, 36]")
    return {
        "features": selected_features, "pose": pose,
        "projected_xy": projected_xy.astype(np.float32, copy=False),
        "log_depth": field["log_depth"], "visibility": visibility,
...
class ViewPoseResidualCompositor(nn.Module):
    def __init__(
        self, view_dimension: int, view_embedding_dimension: int,
        pose_dimension: int, pose_embedding_dimension: int,
        gru_hidden_dimension: int, residual_gate_scale: float,
        minimum_gate: float, maximum_gate: float,
    ) -> None:
        super().__init__()
        dimensions = (
            view_dimension, view_embedding_dimension, pose_dimension,
            pose_embedding_dimension, gru_hidden_dimension,
        )
        ...
        self.view_encoder = nn.Sequential(nn.Linear(int(view_dimension), embedding), nn.SiLU())
        self.graph_encoder = nn.Sequential(nn.Linear(4 * embedding, embedding), nn.SiLU())
        self.pose_encoder = nn.Sequential(
            nn.Linear(int(pose_dimension), int(pose_embedding_dimension)), nn.SiLU(),
            nn.Linear(int(pose_embedding_dimension), embedding), nn.SiLU(),
        )
        self.temporal = nn.GRU(
            input_size=3 * embedding, hidden_size=int(gru_hidden_dimension),
            num_layers=1, batch_first=True, dropout=0.0, bidirectional=True,
        )
    ...
    def forward(self, view, pose, adjacency, visibility, base_gates):
    ...
    def predict_with_residual(
        self, view, pose, adjacency, visibility, base_gates
        ...
        for name, value in (("view", view), ("pose", pose),
                            ("adjacency", adjacency), ("visibility", visibility),
        ...
        graph = self.graph_encoder(torch.cat(
            (node, message, node - message, global_context.expand(-1, carriers, -1)),
            dim=-1,
        ))
        pose_embedding = self.pose_encoder(pose).unsqueeze(1).expand(-1, carriers, -1)
        sequence_input = torch.cat(
            (graph, pose_embedding, graph * pose_embedding), dim=-1
        ).transpose(0, 1)
        ...
        gates = torch.clamp(
            ...
            min=self.minimum_gate, max=self.maximum_gate,
        )
