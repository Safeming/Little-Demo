...
class LPIPS(nn.Module):
    def __init__(self):
        ...
        self.loss_fn_vgg_cpu = None
        self.min_size = 32

    def _ensure_min_size(self, img_pred, img_gt):
        if img_pred.shape[-2] <= 0 or img_pred.shape[-1] <= 0:
            return None, None
        target_h = max(int(img_pred.shape[-2]), self.min_size)
        target_w = max(int(img_pred.shape[-1]), self.min_size)
        if target_h == int(img_pred.shape[-2]) and target_w == int(img_pred.shape[-1]):
        ...
        return _resize(img_pred), _resize(img_gt)

    def _cpu_lpips(self, img_pred, img_gt):
        if self.loss_fn_vgg_cpu is None:
            self.loss_fn_vgg_cpu = lpips.LPIPS(net='vgg').cpu()
            self.loss_fn_vgg_cpu.eval()
        with torch.no_grad():
            score = self.loss_fn_vgg_cpu(img_pred.detach().cpu(), img_gt.detach().cpu(), normalize=True)
        return score.to(device=img_pred.device, dtype=img_pred.dtype)

    def forward(self, inputs, targets, valid_mask=None, reduction='mean'):
        if valid_mask is not None:
            x, y, w, h = cv2.boundingRect(valid_mask.cpu().numpy().astype(np.uint8))
            if w <= 0 or h <= 0:
                return torch.zeros(1, device=inputs.device, dtype=inputs.dtype)
            img_pred = inputs[:, y:y + h, x:x + w]
            img_gt = targets[:, y:y + h, x:x + w]
        else:
            img_pred = inputs
        ...
        img_pred, img_gt = self._ensure_min_size(img_pred, img_gt)
        if img_pred is None:
            return torch.zeros(1, device=inputs.device, dtype=inputs.dtype)
        ...
        try:
            ...
            score = self._cpu_lpips(img_pred, img_gt)
