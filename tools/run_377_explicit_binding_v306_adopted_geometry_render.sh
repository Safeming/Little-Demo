...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-v306_adopted_geometry_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
...
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v306_adopted_geometry_render_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v306_adopted_geometry_render_${RUN_ID}}"
...
Goal:
  v306 adopts the v304/v305 strict no-train explicit-binding geometry path:
  component-level center fidelity in the rigid deformer plus signed dynamic
  screen-space covariance/center actuator in the renderer. It does not train
  or update color/SH/texture. The output is a reusable render/export package
  for the current explicit-binding model.
EOF
...
render_variant() {
  ...
  local component_enable="$5"
  local component_required="$6"
  local center_offset_enable="$7"
  local hydra_dir="$8"
  ...
  env "${COMMON_ENV[@]}" "$PYTHON_BIN" render.py \
    ...
    "++pipeline.covariance_signed_dynamic_enable=$center_offset_enable" \
    ...
    "++pipeline.covariance_signed_screen_actuator_enable=$center_offset_enable" \
    ...
    "++model.deformer.rigid.geometry_fidelity_non_rigid_min=0.000" \
...
ADOPTED_RENDER_EXP="$EXP_ROOT/adopted_guard_045_outer035_m0"
if [ "$RENDER_BASELINE" = "1" ]; then
  BASELINE_RENDER_EXP="$EXP_ROOT/baseline_v281_screen_mid"
  log_event "render_start" "baseline_v281_screen_mid"
  render_variant baseline_v281_screen_mid "$BASELINE_RENDER_EXP" false 0.0 false false false "$HYDRA_RUN_ROOT/baseline_v281_screen_mid"
  analyze_variant baseline_v281_screen_mid "$BASELINE_RENDER_EXP"
  log_event "render_done" "baseline_v281_screen_mid"
fi

log_event "render_start" "adopted_guard_045_outer035_m0"
render_variant adopted_guard_045_outer035_m0 "$ADOPTED_RENDER_EXP" true 0.45 true true true "$HYDRA_RUN_ROOT/adopted_guard_045_outer035_m0"
analyze_variant adopted_guard_045_outer035_m0 "$ADOPTED_RENDER_EXP"
log_event "render_done" "adopted_guard_045_outer035_m0"
