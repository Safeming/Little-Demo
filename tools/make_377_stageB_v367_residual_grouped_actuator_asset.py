...
def _image_size(dataset_root: Path, subject: str, image_name: str, fallback: tuple[int, int] = (512, 512)) -> tuple[int, int]:
...
def _make_outer_action(
    *,
    ...
    action = {
        "component_key": f"v367_outer:{_safe_key(source_key)}:a{action_index}",
        ...
        "reason": "v367_residual_grouped_outer_semantic_local_3d_protect",
        "v367_source_bbox_x": float(outer.get("bbox_x", 0.0) or 0.0),
        "v367_source_bbox_y": float(outer.get("bbox_y", 0.0) or 0.0),
        "v367_source_bbox_w": float(outer.get("bbox_w", 0.0) or 0.0),
        "v367_source_bbox_h": float(outer.get("bbox_h", 0.0) or 0.0),
        "v367_source_centroid_x": float(outer.get("centroid_x", 0.0) or 0.0),
        "v367_source_centroid_y": float(outer.get("centroid_y", 0.0) or 0.0),
        **support,
        **outer_owner,
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build v367 subject-general residual-driven grouped actuator asset."
    )
    ...
    for frame in frames:
        ...
        if cam_name not in cams:
            continue
        width, height = _image_size(args.dataset_root, args.subject, image_name)
                    ...
                    for radius_scale in radius_scales:
                        for minor_scale in minor_scales:
                            for covariance_scale in covariance_scales:
                                ...
                                pair_id = f"v367:{source_key}:{mode}:g{group_index}"
                                group_children = []
                                for spec in specs:
                                    ...
                                    child = {
                                        "component_key": (
                                            f"v367_inner:{_safe_key(source_key)}:{mode}:"
                                        ...
                                        "reason": "v367_residual_driven_grouped_inner_micro_child",
                                        "v367_mode": mode,
                                        "v367_micro_count": int(len(specs)),
                                        "v367_micro_index": int(spec["micro_index"]),
                                        "v367_screen_x": float(target_u),
                                        "v367_screen_y": float(target_v),
                                        "v367_anchor_screen_x": float(anchor_u),
                                        "v367_anchor_screen_y": float(anchor_v),
                                        "v367_anchor_depth": float(anchor_depth),
                                        "v367_depth": float(depth),
                                        "v367_renderer_space_cache": bool(image_name in renderer_space_cache),
                                        "v367_source_bbox_x": float(inner.get("bbox_x", 0.0) or 0.0),
                                        "v367_source_bbox_y": float(inner.get("bbox_y", 0.0) or 0.0),
                                        "v367_source_bbox_w": float(inner.get("bbox_w", 0.0) or 0.0),
                                        "v367_source_bbox_h": float(inner.get("bbox_h", 0.0) or 0.0),
                                        "v367_source_centroid_x": float(inner.get("centroid_x", 0.0) or 0.0),
                                        "v367_source_centroid_y": float(inner.get("centroid_y", 0.0) or 0.0),
                                        "v367_residual_mask_enable": bool(residual_mask is not None),
                                        **{f"v367_{key}": value for key, value in residual_stats.items()},
                                        **inner_owner,
                                    }
    ...
    payload = {
        "version": "v367_residual_driven_grouped_actuator_asset",
        "policy": (
            ...
            "outer components become semantic local-3D protect/shrink actions. All pairs still require "
            "action-level raw contour validation before training."
        ),
        "source": {
