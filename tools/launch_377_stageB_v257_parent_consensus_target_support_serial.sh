...
RUN_ID="${RUN_ID:-stageB_v257_parent_consensus_target_support_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
GPU="${GPU:-0}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_stageB_v257_parent_consensus_target_support_${RUN_ID}}"
mkdir -p "$LOG_DIR"
...
env \
  EXPERIMENT_VERSION="v257" \
  EXPERIMENT_SLUG="parent_consensus_target_support" \
  TRAIN_NAME="v257a_parent_consensus_target_support" \
  LOG_PREFIX="377_stageB_v257_parent_consensus_target_support" \
  RENDER_TAG="v257" \
  PURPOSE="v257 parent-consensus target support: fuse multi-view target-projected component specs per parent before spawning support Gaussians" \
  RUN_ID="$RUN_ID" \
  GPU="$GPU" \
  LOG_DIR="$LOG_DIR" \
  WAIT_FOR_FREE_GPU=1 \
...

{
  ...
  echo "EXPECTED_FINAL_CKPT=$ROOT/exp/stageB/377_hulk_light_v257a_parent_consensus_target_support_${RUN_ID}/ckpt145310.pth"
