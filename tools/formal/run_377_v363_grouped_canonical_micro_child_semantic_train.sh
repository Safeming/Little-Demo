...
V363_ASSET_JSON="${V363_ASSET_JSON:-}"
BASE_CKPT="${BASE_CKPT:-$ROOT/exp/formal/377_v338_semantic_train_formal_377_v338_mainline_20260522_141739_bjt/ckpt138410.pth}"
RUN_ID="${RUN_ID:-formal_377_v363_grouped_canonical_micro_child_semantic_train_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
EXP_DIR="${EXP_DIR:-$ROOT/exp/formal/377_v363_grouped_canonical_micro_child_semantic_train_${RUN_ID}}"
HYDRA_RUN_DIR="${HYDRA_RUN_DIR:-$EXP_DIR/hydra_runtime}"

if [ -z "$V363_ASSET_JSON" ] || [ ! -e "$V363_ASSET_JSON" ]; then
  echo "missing required V363_ASSET_JSON: $V363_ASSET_JSON" >&2
  exit 2
fi
...
exec "$ROOT/tools/formal/run_377_v338_semantic_train.sh" "$@" \
  "++explicit_binding_adopted_signed_dynamic_component_local_asset_json=$V363_ASSET_JSON" \
  "++pipeline.split_child_component_enable=true" \
  "++pipeline.split_child_component_asset_json=$V363_ASSET_JSON" \
