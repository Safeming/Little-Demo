...
SEED="${SEED:--1}"
TIME_BUDGET_SECONDS="${TIME_BUDGET_SECONDS:-14400}"
...
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageA2/logs/v215_boundary_suppress_4gpu_$RUN_ID}"
...
for required in "$BASE_CKPT" "$V210B_CKPT" "$V213A100_CKPT" "$V214J300_CKPT" "$V214V350_CKPT" "$PARSER_ROOT" "$V211_PLAN"; do
  if [ ! -e "$required" ]; then
...
printf 'RUN_ID=%s\nSTART_BJT=%s\nDEADLINE_BJT=%s\nTIME_BUDGET_SECONDS=%s\nMIN_START_SECONDS=%s\nBASE_CKPT=%s\nV210B_CKPT=%s\nV213A100_CKPT=%s\nV214J300_CKPT=%s\nV214V350_CKPT=%s\nV211_ANALYSIS_DIR=%s\nCAM_WEIGHTS=%s\nSMOKE=%s\nDO_RENDER=%s\n' \
  ...
  "$BASE_CKPT" "$V210B_CKPT" "$V213A100_CKPT" "$V214J300_CKPT" "$V214V350_CKPT" \
  "$V211_ANALYSIS_DIR" "$CAM_WEIGHTS" "$SMOKE" "$DO_RENDER" | tee "$LOG_DIR/run_info.txt"

cat >> "$LOG_DIR/run_info.txt" <<'EOF'

v215 purpose:
  Test semantic-boundary-aware residual allocation for the cloth high-frequency
  view-conflict branch. The goal is not lower LPIPS alone; accepted checkpoints
  must keep cloth detail inside the upper/lower cloth regions and avoid boundary
  or skin/face/hair/shoe leakage.

acceptance:
  - v198a remains strict anchor
  - v210b@109150 remains auxiliary edge anchor
  - v214v@109350 is current balanced clarity candidate
  - v214j@109300 is edge-only diagnostic candidate
  - Prefer checkpoints with boundary_l1 around 0.06603-0.06610 and edge_px < 2.735
EOF
...
run_named_job() {
  local gpu="$1"
  local job="$2"
  local short_v214v="60,120,200,300"
  local mid_v214v="80,160,260,420"
  local short_v214j="60,120,220,360"
  local short_v210b="80,160,260,420"

  case "$job" in
    v215a_v214v350_boundary_mild)
      run_one "$job" "$gpu" 300 9.0e-07 "$V214V350_CKPT" 109350 "$short_v214v"
      ;;
    v215b_v214v350_boundary_strong)
      run_one "$job" "$gpu" 300 8.0e-07 "$V214V350_CKPT" 109350 "$short_v214v" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.52" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.strength=[0.0,1,0.50,60,0.72]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.threshold=0.10" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.power=1.60" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.min_scale=0.28" \
        "opt.lambda_l1_boundary=0.210" \
        "++opt.perceptual_adaptive_edge_protect=0.96" \
        "opt.grad_clip=0.0016"
      ;;
    v215c_v214v350_gate_only_boundary)
      run_one "$job" "$gpu" 300 7.0e-07 "$V214V350_CKPT" 109350 "$short_v214v" \
        ...
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.strength=[0.0,1,0.42,60,0.60]" \
        "opt.lambda_l1_boundary=0.206" \
        "opt.grad_clip=0.0014"
      ;;
    v215d_v214v350_mlp_only_lowcap)
      run_one "$job" "$gpu" 300 7.0e-07 "$V214V350_CKPT" 109350 "$short_v214v" \
        "++opt.texture_trainable_name_patterns=[detail_high_freq_view_conflict_mlp.*]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.45" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.014" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.strength=[0.0,1,0.45,60,0.64]" \
        "opt.lambda_l1_boundary=0.208" \
        "opt.grad_clip=0.0014"
      ;;
    v215e_v214j300_boundary_repair)
      run_one "$job" "$gpu" 360 8.0e-07 "$V214J300_CKPT" 109300 "$short_v214j" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.42" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.014" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.strength=[0.0,1,0.55,80,0.78]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.threshold=0.08" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.power=1.80" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.min_scale=0.25" \
        "opt.lambda_l1_boundary=0.214" \
        "opt.lambda_perceptual=0.108" \
        "++opt.perceptual_adaptive_edge_protect=0.97" \
        "opt.grad_clip=0.0014"
      ;;
    v215f_v214j300_gate_repair)
      run_one "$job" "$gpu" 360 6.5e-07 "$V214J300_CKPT" 109300 "$short_v214j" \
        "++opt.texture_trainable_name_patterns=[detail_high_freq_view_conflict_gate_mlp.*]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.50" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.018" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.strength=[0.0,1,0.55,80,0.80]" \
        "opt.lambda_l1_boundary=0.214" \
        "++opt.perceptual_adaptive_edge_protect=0.97" \
        "opt.grad_clip=0.0012"
      ;;
    v215g_v210b150_boundary_short)
      run_one "$job" "$gpu" 420 2.4e-06 "$V210B_CKPT" 109150 "$short_v210b" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.54" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.018" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.init_output_scale=0.26" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.strength=[0.0,1,0.40,80,0.58]" \
        "opt.lambda_l1_boundary=0.204" \
        "opt.grad_clip=0.0020"
      ;;
    v215h_v210b150_strict_skin_boundary)
      run_one "$job" "$gpu" 420 2.2e-06 "$V210B_CKPT" 109150 "$short_v210b" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.50" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.018" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.init_output_scale=0.24" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.point_gate.exclude_semantic_id_weights=[[0,1.00],[1,1.00],[2,0.92],[5,1.00]]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.point_gate.exclude_semantic_name_weights=[[hair,1.00],[face,1.00],[skin,0.92],[shoes,1.00]]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.strength=[0.0,1,0.48,80,0.68]" \
        "opt.lambda_l1_boundary=0.208" \
        "opt.grad_clip=0.0018"
      ;;
    v215i_v214v350_lowpercep_mild)
      run_one "$job" "$gpu" 420 8.0e-07 "$V214V350_CKPT" 109350 "$mid_v214v" \
        ...
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.48" \
        "opt.lambda_l1_boundary=0.210" \
        "opt.grad_clip=0.0014"
      ;;
    v215j_v214v350_skinhard_boundary)
      run_one "$job" "$gpu" 420 8.0e-07 "$V214V350_CKPT" 109350 "$mid_v214v" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.point_gate.exclude_semantic_id_weights=[[0,1.00],[1,1.00],[2,0.94],[5,1.00]]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.point_gate.exclude_semantic_name_weights=[[hair,1.00],[face,1.00],[skin,0.94],[shoes,1.00]]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.50" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.strength=[0.0,1,0.50,80,0.70]" \
        "opt.lambda_l1_boundary=0.210" \
        "opt.grad_clip=0.0014"
      ;;
    v215k_v214j300_lowpercep_edgehold)
      run_one "$job" "$gpu" 420 7.0e-07 "$V214J300_CKPT" 109300 "80,160,260,420" \
        "opt.lambda_perceptual=0.104" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.40" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.012" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.strength=[0.0,1,0.58,80,0.82]" \
        "opt.lambda_l1_boundary=0.216" \
        "++opt.perceptual_adaptive_edge_protect=0.98" \
        "opt.grad_clip=0.0012"
      ;;
    v215l_v213a100_boundary_recheck)
      run_one "$job" "$gpu" 480 7.0e-07 "$V213A100_CKPT" 109100 "80,160,300,480" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.42" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.014" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.strength=[0.0,1,0.50,100,0.72]" \
        "opt.lambda_l1_boundary=0.212" \
        "opt.lambda_perceptual=0.108" \
        "opt.grad_clip=0.0012"
      ;;
    *)
