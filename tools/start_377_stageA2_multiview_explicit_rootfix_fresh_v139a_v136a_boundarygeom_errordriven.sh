...
EXP_DIR="${1:-$ROOT/exp/stageA2/377_multiview_explicit_hq_rootfix_fresh_v139a_v136a_boundarygeom_errordriven_resume}"
START_CKPT="${2:-$ROOT/exp/stageA2/377_multiview_explicit_hq_rootfix_fresh_v136a_v135a_ownerdetailboost_resume/best_ckpt.pth}"
ITERATIONS="${3:-4000}"
if [ "$#" -ge 3 ]; then
  shift 3
else
...
exec env PYTHONUNBUFFERED=1 "$PYTHON_BIN" "$ROOT/tools/run_377_stageA2_multiview_explicit.py" \
  ...
  --option stageA_377_multiview_explicit_hq_v139a_v136a_boundarygeom_errordriven_v1 \
