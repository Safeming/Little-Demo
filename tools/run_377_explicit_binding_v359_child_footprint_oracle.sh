...
RUN_ID="${RUN_ID:-v359_child_footprint_oracle_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v359_child_footprint_oracle_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v359_child_footprint_oracle_${RUN_ID}}"
...
ACTION_KEYS="${ACTION_KEYS:-auto_failed_gain}"
RGB_DELTA_THRESHOLD="${RGB_DELTA_THRESHOLD:-0.0035}"
OPACITY_DELTA_THRESHOLD="${OPACITY_DELTA_THRESHOLD:-0.0035}"
COMPONENT_PAD_PX="${COMPONENT_PAD_PX:-6}"
TARGET_DILATE_PX="${TARGET_DILATE_PX:-0}"
TARGET_MODE="${TARGET_MODE:-component_overlap}"
MIN_COMPONENT_OVERLAP="${MIN_COMPONENT_OVERLAP:-1}"
...
V359_ASSET_JSON="$ASSET_DIR/v359_validated_child_footprint_oracle_asset.json"
...
build_action_keys() {
  if [ "$ACTION_KEYS" != "auto_failed_gain" ]; then
    printf '%s\n' "$ACTION_KEYS" | tr ',' '\n'
    return
  fi
  "$PYTHON_BIN" - "$V356_ACTION_VALIDATION_TSV" <<'PY'
import csv
import sys
...
with Path(sys.argv[1]).open("r", encoding="utf-8", newline="") as handle:
    for row in csv.DictReader(handle, delimiter="\t"):
        if row.get("status") != "drop":
            continue
        target_gain = float(row.get("target_gain", 0.0) or 0.0)
        inner = float(row.get("inner_delta_control", 0.0) or 0.0)
        opacity_inner = float(row.get("opacity_inner_delta_control", 0.0) or 0.0)
        if target_gain > 0.0 and (inner < 0.0 or opacity_inner < 0.0):
            rows.append((target_gain, row["component_key"]))
for _, key in sorted(rows, reverse=True):
    print(key)
PY
}
...
mapfile -t KEYS < <(build_action_keys)
if [ "${#KEYS[@]}" -eq 0 ]; then
  echo "no v356 failed-gain split-child actions selected" >&2
  exit 4
fi
log_event "selected_actions" "$(printf '%s,' "${KEYS[@]}")"
...
for component_key in "${KEYS[@]}"; do
  ...
  candidate_exp="$EXP_ROOT/action_validation/$safe_key/child_footprint_oracle_plus_v345"
  if [ ! -d "$control_exp" ] || [ ! -d "$child_exp" ]; then
    echo "missing v356 action render dirs for $component_key: $source_root" >&2
    exit 3
  fi
  log_event "footprint_oracle_compose_start" "$component_key"
  "$PYTHON_BIN" tools/make_377_stageB_v359_child_footprint_oracle_render.py \
    ...
    --rgb-delta-threshold "$RGB_DELTA_THRESHOLD" \
    --opacity-delta-threshold "$OPACITY_DELTA_THRESHOLD" \
    --component-pad-px "$COMPONENT_PAD_PX" \
    --target-dilate-px "$TARGET_DILATE_PX" \
    --min-component-overlap "$MIN_COMPONENT_OVERLAP" \
    --write-mask \
    > "$LOG_DIR/compose_${safe_key}.log" 2>&1
  analyze_render "action_${safe_key}_child_footprint_oracle_plus_v345" "$candidate_exp"
  append_action_validation_row "$component_key" "$image_name" "$control_exp" "$candidate_exp"
  log_event "footprint_oracle_compose_done" "$component_key"
done

"$PYTHON_BIN" - "$V356_SEED_ASSET_JSON" "$ACTION_VALIDATION_TSV" "$V359_ASSET_JSON" <<'PY'
import csv
import json
import sys
from pathlib import Path
...
for child in data.get("children", data.get("actions", [])):
    if str(child.get("component_key", "")) in kept:
        ...
        item["child_footprint_oracle_enable"] = True
        item["split_child_reason"] = "v359_child_contribution_footprint_oracle_action_validated"
        children.append(item)
payload = {
    "version": "v359_child_contribution_footprint_oracle_asset",
    "policy": "v359 diagnostic asset: child contribution kept only where actual child-control footprint intersects target inner residual, then action-level do-no-harm.",
...
echo "V359_ASSET_JSON=$V359_ASSET_JSON"
