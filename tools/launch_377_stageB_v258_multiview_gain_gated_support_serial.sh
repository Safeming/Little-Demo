...
RUN_ID="${RUN_ID:-stageB_v258_multiview_gain_gated_support_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
GPU="${GPU:-0}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_stageB_v258_multiview_gain_gated_support_${RUN_ID}}"
mkdir -p "$LOG_DIR"
...
env \
  EXPERIMENT_VERSION="v258" \
  EXPERIMENT_SLUG="multiview_gain_gated_support" \
  TRAIN_NAME="v258a_multiview_gain_gated_support" \
  LOG_PREFIX="377_stageB_v258_multiview_gain_gated_support" \
  RENDER_TAG="v258" \
  PURPOSE="v258 multiview target gain-gated support: keep parent-consensus target support only when target under residual exceeds target/anchor over penalties" \
  RUN_ID="$RUN_ID" \
  GPU="$GPU" \
  LOG_DIR="$LOG_DIR" \
  WAIT_FOR_FREE_GPU=1 \
  ...
  BOUNDARY_COMPONENT_SUPPORT_PARENT_CONSENSUS_MAX_OFFSET_STD=0.035 \
...

{
  ...
  echo "EXPECTED_FINAL_CKPT=$ROOT/exp/stageB/377_hulk_light_v258a_multiview_gain_gated_support_${RUN_ID}/ckpt145310.pth"
