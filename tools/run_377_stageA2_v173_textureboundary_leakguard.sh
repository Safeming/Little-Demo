...
BATCH_ID="${V173_RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
SUMMARY="$LOG_DIR/v173_batch_${BATCH_ID}_summary.tsv"
REGISTRY="$LOG_DIR/v173_batch_${BATCH_ID}_runs.txt"
RUNNER_LOG="$LOG_DIR/v173_batch_${BATCH_ID}.log"

printf '%s\n' "$RUNNER_LOG" > "$LOG_DIR/v173_latest_runner_log.txt"
printf '%s\n' "$SUMMARY" > "$LOG_DIR/v173_latest_summary.txt"
...
run_render() {
  ...
  printf '%s\n' "$render_log" > "$LOG_DIR/v173_latest_render_log.txt"
...
run_one() {
  ...
  printf '%s\n' "$exp_dir" > "$LOG_DIR/v173_latest_exp.txt"
  printf '%s\n' "$train_log" > "$LOG_DIR/v173_latest_log.txt"
...
echo "[$(date '+%F %T')] v173 batch started"
echo "summary=$SUMMARY"
echo "registry=$REGISTRY"

run_one "v173a" 2000 "stageA_377_multiview_explicit_hq_v173a_v170e_textureboundary_leakguard_v1" "textureboundary_leakguard"

echo "[$(date '+%F %T')] v173 batch finished"
cat "$SUMMARY"
