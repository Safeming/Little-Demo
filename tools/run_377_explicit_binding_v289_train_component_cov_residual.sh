...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-v289_train_component_cov_residual_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
...
AUDIT_DIR="${AUDIT_DIR:-$ROOT/exp/stageB/logs/377_stageB_v289_train_component_audit_${RUN_ID}}"
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v289_train_component_cov_residual_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v289_train_component_cov_residual_${RUN_ID}}"
...
Goal:
  v289 fixes the v288 train/eval mask mismatch. v288 trained with test-view component CSV,
  so train-view component lookup fell back to broad binding priors. v289 first audits
  train views and uses train-view component CSV during training, while keeping test-view
  component CSV for raw gate renders.
EOF
...
log_event "v289_train_start" "$EXP_ROOT"
RUN_ID="$RUN_ID" \
...
log_event "v289_train_done" "$EXP_ROOT"
