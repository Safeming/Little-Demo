...
RUN_ID="${RUN_ID:-v378_visible_contributor_closure_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v378_visible_contributor_closure_grouped_actuator_${RUN_ID}}"
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v378_visible_contributor_closure_grouped_actuator_${RUN_ID}}"
ASSET_DIR="$LOG_DIR/assets"
EVENTS="$LOG_DIR/events.tsv"
ASSET_JSON="$ASSET_DIR/v378_visible_contributor_closure_asset.json"
ASSET_TSV="$ASSET_DIR/v378_visible_contributor_closure_actions.tsv"
...
MAX_ACTIONS="${MAX_ACTIONS:-96}"
VISIBLE_RADIUS_PX="${VISIBLE_RADIUS_PX:-36}"
ANCHOR_IDS="${ANCHOR_IDS:-8}"
CHILD_OPACITY="${CHILD_OPACITY:-0.04}"
TRAIN_ON_STRICT_PASS="${TRAIN_ON_STRICT_PASS:-true}"
TRAIN_STEPS="${TRAIN_STEPS:-2000}"

RAW_GATE_RUN_ID="formal_377_v378_visible_contributor_closure_raw_gate_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')"
RAW_GATE_LOG="$LOG_DIR/v378_raw_gate.launcher.log"
RAW_GATE_SUMMARY="$ROOT/exp/formal/logs/377_v338_raw_contour_gate_${RAW_GATE_RUN_ID}/summary.tsv"
TRAIN_LAUNCH_LOG="$LOG_DIR/v378_semantic_train.launcher.log"

for required in "$PYTHON_BIN" "$BASE_EXP/.hydra/config.yaml" "$DATA_ROOT" "$CANDIDATE_CKPT" "$SEED_ASSET_JSON" "$VALIDATION_TSV"; do
...
env CUDA_VISIBLE_DEVICES="$GPU" OMP_NUM_THREADS="$CPU_THREADS_PER_JOB" MKL_NUM_THREADS="$CPU_THREADS_PER_JOB" OPENBLAS_NUM_THREADS="$CPU_THREADS_PER_JOB" NUMEXPR_NUM_THREADS="$CPU_THREADS_PER_JOB" PYTHONUNBUFFERED=1 \
  ...
  --seed-json "$SEED_ASSET_JSON" --validation-tsv "$VALIDATION_TSV" --out-json "$ASSET_JSON" --out-tsv "$ASSET_TSV" \
  --max-actions "$MAX_ACTIONS" --visible-radius-px "$VISIBLE_RADIUS_PX" --anchor-ids "$ANCHOR_IDS" \
  > "$LOG_DIR/make_v378_visible_contributor_closure_asset.log" 2>&1
...
env -u LOG_DIR -u EXP_ROOT -u HYDRA_RUN_ROOT GPU="$GPU" PYTHON_BIN="$PYTHON_BIN" CPU_THREADS_PER_JOB="$CPU_THREADS_PER_JOB" \
  RUN_ID="$RAW_GATE_RUN_ID" CANDIDATE_CKPT="$CANDIDATE_CKPT" CANDIDATE_VARIANT_NAME="candidate_v378_visible_contributor_closure" \
...
with open(sys.argv[1],encoding='utf-8',newline='') as h:
    for r in csv.DictReader(h,delimiter='\t'):
        if r.get('variant')=='candidate_v378_visible_contributor_closure': s=r.get('status','')
print(s)
PY
)"
...
if [ "$TRAIN_ON_STRICT_PASS" = "true" ] && [ "$RAW_GATE_STATUS" = "strict_pass" ]; then
  TRAIN_RUN_ID="formal_377_v378_visible_contributor_closure_semantic_train_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')"
  TRAIN_EXP_DIR="$ROOT/exp/formal/377_v378_visible_contributor_closure_semantic_train_${TRAIN_RUN_ID}"
  TRAIN_SCRIPT="$LOG_DIR/v378_semantic_train.launch.sh"
