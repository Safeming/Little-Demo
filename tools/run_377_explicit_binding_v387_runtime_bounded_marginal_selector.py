...
def main() -> int:
    ...
    parser.add_argument("--run-id", default=os.environ.get("RUN_ID", f"v387_runtime_bounded_marginal_selector_{bjt_stamp()}"))
    ...
    parser.add_argument("--include-all-source-images", default=os.environ.get("INCLUDE_ALL_SOURCE_IMAGES", "true"))
    ...
    args.log_dir = args.log_dir or ROOT / f"exp/stageB/logs/377_explicit_binding_v387_runtime_bounded_marginal_selector_{args.run_id}"
    args.exp_root = args.exp_root or ROOT / f"exp/stageB/377_explicit_binding_v387_runtime_bounded_marginal_selector_{args.run_id}"
    ...
    validation_tsv = args.log_dir / "marginal_validation.tsv"
    summary_tsv = args.log_dir / "summary.tsv"
    final_asset = asset_dir / "v387_final_runtime_bounded_marginal_selector_asset.json"
    raw_gate_log = args.log_dir / "v387_raw_gate.launcher.log"
    train_log = args.log_dir / "v387_semantic_train.launcher.log"
    ...
    source_images = ordered_unique(
        [
            str(group_by_id[str(row.get("pair_id", ""))].get("image_name", row.get("image_name", "")) or row.get("image_name", "")).strip()
            for row in prefilter_rows
            if str(row.get("pair_id", "")) in group_by_id
        ]
    )
    ...
    safety_images = ordered_unique(
        (source_images if str(args.include_all_source_images).strip().lower() == "true" else [])
        + canary_images
    )
    (args.log_dir / "canary_images.txt").write_text("\n".join(canary_images) + ("\n" if canary_images else ""), encoding="utf-8")
    (args.log_dir / "source_safety_images.txt").write_text("\n".join(source_images) + ("\n" if source_images else ""), encoding="utf-8")
    (args.log_dir / "safety_images.txt").write_text("\n".join(safety_images) + ("\n" if safety_images else ""), encoding="utf-8")
    append_event(events_path, "canary_images", ",".join(canary_images))
    append_event(events_path, "source_safety_images", ",".join(source_images))
    append_event(events_path, "safety_images", ",".join(safety_images))

    selected_ids: set[str] = set()
    selected_allowlists: dict[str, list[str]] = {}
    control_assets: dict[str, Path] = {}
    control_exps: dict[str, Path] = {}

    def asset_with_allowlists(pair_ids: set[str], allowlists: dict[str, list[str]], version: str, policy: str) -> dict:
        payload = merge_asset(base_asset, closure_asset, pair_ids, version, policy)
        selected_pair_ids = set(pair_ids)
        for key in ("children", "actions"):
            for item in payload.get(key, []) or []:
                ...
                if pair_id not in selected_pair_ids:
                    continue
                allowlist = ordered_unique(allowlists.get(pair_id, safety_images))
                if allowlist:
                    item["activation_image_allowlist"] = allowlist
                    item["v387_runtime_bounded_marginal_selector"] = True
        payload["v387_runtime_contract"] = {
            "activation_image_allowlist": "required for every selected closure pair",
            "selector": "marginal current-vs-trial raw validation over source safety plus canary images",
    ...
    def ensure_control(image_name: str) -> Path:
        ...
        write_json(
            control_asset,
            asset_with_allowlists(set(), {}, "v387_control_v374_asset", "v374 control asset for runtime-bounded marginal validation"),
        )
        control_assets[image_name] = control_asset
        control_exp = args.exp_root / "marginal_validation" / image_name / "control_v374"
        render_and_analyze(
    ...
    def render_current(index: int, image_name: str, current_asset: Path, safe_pair: str) -> Path:
        current_exp = args.exp_root / "marginal_validation" / image_name / safe_pair / f"current_{index:03d}"
        render_and_analyze(
            label=f"current_{index:03d}_{safe_pair}_{image_name}",
            render_exp=current_exp,
            image_name=image_name,
            asset_json=current_asset,
            args=args,
            env=env,
            events_path=events_path,
            log_dir=args.log_dir,
    ...
    fieldnames = [
        ...
        "source_hard_delta_control",
            ...
            "source_inner_delta_current",
            "source_outer_delta_current",
            "source_hard_delta_current",
            "source_opacity_outer_delta_current",
            "worst_outer_delta_control",
            "worst_opacity_outer_delta_control",
            "worst_hard_delta_control",
            "worst_outer_delta_current",
            "worst_opacity_outer_delta_current",
            "worst_hard_delta_current",
    ...
    for index, row in enumerate(prefilter_rows):
        ...
        validation_images = ordered_unique([image_name] + safety_images)
        trial_allowlists = dict(selected_allowlists)
        trial_allowlists[pair_id] = validation_images
        ...
        current_asset = trial_dir / f"current_{index:03d}_{safe_pair}.json"
        trial_asset = trial_dir / f"marginal_{index:03d}_{safe_pair}.json"
        write_json(
            current_asset,
            asset_with_allowlists(
                selected_ids,
                selected_allowlists,
                "v387_current_asset",
                "current selected runtime-bounded closure actions for marginal validation",
            ),
        )
        write_json(
            trial_asset,
            asset_with_allowlists(
                trial_ids,
                trial_allowlists,
                "v387_marginal_trial_asset",
                "v374 plus trial runtime-bounded residual action under marginal validation",
            ),
        )

        control_deltas: dict[str, dict[str, float]] = {}
        current_deltas: dict[str, dict[str, float]] = {}
        bad_reasons: list[str] = []
        for image in validation_images:
            control_exp = ensure_control(image)
            current_exp = render_current(index, image, current_asset, safe_pair)
            candidate_exp = args.exp_root / "marginal_validation" / image / safe_pair / "candidate"
            render_and_analyze(
            ...
            delta_control = metric_delta(candidate_exp, control_exp)
            delta_current = metric_delta(candidate_exp, current_exp)
            control_deltas[image] = delta_control
            current_deltas[image] = delta_current
            ok, reason = no_harm(delta_control, args)
            if not ok:
                bad_reasons.append(f"{image}:control_{reason}")
            ok, reason = no_harm(delta_current, args)
            if not ok:
                bad_reasons.append(f"{image}:marginal_{reason}")

        source_delta = control_deltas[image_name]
        source_delta_current = current_deltas[image_name]
        source_has_gain = source_delta_current["inner"] <= -float(args.min_inner_gain)
        if source_has_gain and not bad_reasons:
            status = "keep"
            reason = "marginal_source_gain_safety_no_harm"
            selected_ids.add(pair_id)
            selected_allowlists[pair_id] = validation_images
        elif not source_has_gain:
            status = "drop"
            reason = "no_marginal_source_inner_gain"
        else:
            status = "drop"
            reason = ";".join(bad_reasons[:6])

        worst_outer = max(delta["outer"] for delta in control_deltas.values())
        worst_opacity_outer = max(delta["opacity_outer"] for delta in control_deltas.values())
        worst_hard = max(delta["hard"] for delta in control_deltas.values())
        worst_outer_current = max(delta["outer"] for delta in current_deltas.values())
        worst_opacity_outer_current = max(delta["opacity_outer"] for delta in current_deltas.values())
        worst_hard_current = max(delta["hard"] for delta in current_deltas.values())
        out_row = {
            "pair_id": pair_id,
            "image_name": image_name,
            "status": status,
            ...
            "source_inner_delta_current": f"{source_delta_current['inner']:.8f}",
            "source_outer_delta_current": f"{source_delta_current['outer']:.8f}",
            "source_hard_delta_current": f"{source_delta_current['hard']:.8f}",
            "source_opacity_outer_delta_current": f"{source_delta_current['opacity_outer']:.8f}",
            ...
            "worst_outer_delta_current": f"{worst_outer_current:.8f}",
            "worst_opacity_outer_delta_current": f"{worst_opacity_outer_current:.8f}",
            "worst_hard_delta_current": f"{worst_hard_current:.8f}",
            "selected_count_after": len(selected_ids),
            "current_asset": str(current_asset),
    ...
    selected_payload = asset_with_allowlists(
        selected_ids,
        selected_allowlists,
        "v387_runtime_bounded_marginal_selector_asset",
        "v374 plus residual bundle actions selected by marginal safety validation with runtime image allowlists.",
    )
    selected_payload["runtime_bounded_marginal_validation"] = {
        "prefilter_tsv": str(args.prefilter_tsv),
        "canary_worst_tsv": str(args.canary_worst_tsv),
        "canary_images": canary_images,
        "source_safety_images": source_images,
        "safety_images": safety_images,
    ...
    raw_gate_summary = ""
    raw_gate_run_id = f"formal_377_v387_runtime_bounded_marginal_selector_raw_gate_{bjt_stamp()}"
    variant_name = "candidate_v387_runtime_bounded_marginal_selector"
    ...
    if (
        ...
        train_run_id = f"formal_377_v387_runtime_bounded_marginal_selector_semantic_train_{bjt_stamp()}"
        train_exp_dir = str(ROOT / f"exp/formal/377_v387_runtime_bounded_marginal_selector_semantic_train_{train_run_id}")
        train_script = args.log_dir / "v387_semantic_train.launch.sh"
    ...
    print(f"MARGINAL_VALIDATION_TSV={validation_tsv}")
