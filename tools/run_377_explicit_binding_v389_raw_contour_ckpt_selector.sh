...
RUN_ID="${RUN_ID:-v389_raw_contour_ckpt_selector_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
EXP_DIR="${EXP_DIR:-$ROOT/exp/formal/377_v389_raw_contour_ckpt_selector_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/formal/logs/377_v389_raw_contour_ckpt_selector_${RUN_ID}}"
...
SELECTOR_SUMMARY="$LOG_DIR/v389_raw_contour_checkpoint_summary.tsv"
SELECTED_JSON="$LOG_DIR/v389_selected_checkpoint.json"
...
CONFIG_CONTRACT=raw_gate_base_config_consistent_with_post_train_contour_selector
INFO
...
for ckpt in "${checkpoints[@]}"; do
  ...
  iter="${ckpt_name#ckpt}"
  gate_run_id="formal_377_v389_${ckpt_name}_raw_gate_${RUN_ID}"
  ...
  variant="candidate_v389_${ckpt_name}_raw_contour_ckpt_selector"
