...
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageA2/logs/v214_9h_clarity_queue_$RUN_ID}"
...
for required in "$BASE_CKPT" "$V210B_CKPT" "$V213A100_CKPT" "$PARSER_ROOT" "$V211_PLAN"; do
  if [ ! -e "$required" ]; then
...
printf 'RUN_ID=%s\nSTART_BJT=%s\nDEADLINE_BJT=%s\nTIME_BUDGET_SECONDS=%s\nMIN_START_SECONDS=%s\nBASE_CKPT=%s\nV210B_CKPT=%s\nV213A100_CKPT=%s\nV211_ANALYSIS_DIR=%s\nCAM_WEIGHTS=%s\nSMOKE=%s\nDO_RENDER=%s\n' \
  ...
  "$BASE_CKPT" "$V210B_CKPT" "$V213A100_CKPT" "$V211_ANALYSIS_DIR" "$CAM_WEIGHTS" "$SMOKE" "$DO_RENDER" \
  | tee "$LOG_DIR/run_info.txt"

cat >> "$LOG_DIR/run_info.txt" <<'EOF'

v214 purpose:
  Use the nine-hour window on the highest-value branch found by v213.
  v213a@109100 improved edge distance, while later v213a checkpoints lowered
  LPIPS but softened the boundary. v214 therefore tests scale caps, slower
  schedules, stricter cloth gates, boundary protection, and low-LR continuation
  from v213a@109100.

queue policy:
  The queue uses all four GPUs. It stops launching new jobs after the deadline
  window is nearly exhausted, but never kills an already-running training job.
EOF
...
run_one() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 "$PYTHON_BIN" train.py \
    ...
    "++opt.perceptual_adaptive_edge_protect=0.86" \
    ...
    "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=[0.0,1,0.30,80,0.55,160,0.65]" \
...
run_named_job() {
  local gpu="$1"
  local job="$2"
  local common_long="100,180,300,600,1200,1800,2400,3000"
  local common_mid="80,100,120,180,300,600,1200,1800"
  local cont_mid="100,200,400,800,1200,1800"

  case "$job" in
    v214a_v198a_cap055_anchor)
      run_one "$job" "$gpu" 1800 5.8e-06 "$BASE_CKPT" 109000 "$common_mid" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=[0.0,1,0.30,80,0.48,160,0.55]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.028" \
        "opt.lambda_l1_boundary=0.180"
      ;;
    v214b_v198a_cap065_slow)
      run_one "$job" "$gpu" 3000 5.6e-06 "$BASE_CKPT" 109000 "$common_long" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=[0.0,1,0.22,120,0.42,300,0.58,900,0.65]" \
        "opt.lambda_l1_boundary=0.182"
      ;;
    v214c_v198a_cap075_boundary)
      run_one "$job" "$gpu" 3000 4.8e-06 "$BASE_CKPT" 109000 "$common_long" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=[0.0,1,0.30,80,0.55,180,0.75]" \
        "opt.lambda_l1_boundary=0.190" \
        "++opt.perceptual_adaptive_edge_protect=0.90" \
        "opt.grad_clip=0.0034"
      ;;
    v214d_v198a_skinhard_cap065)
      run_one "$job" "$gpu" 2400 5.8e-06 "$BASE_CKPT" 109000 "80,100,120,180,300,600,1200,2400" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.point_gate.exclude_semantic_id_weights=[[0,1.00],[1,1.00],[2,0.90],[5,1.00]]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.point_gate.exclude_semantic_name_weights=[[hair,1.00],[face,1.00],[skin,0.90],[shoes,1.00]]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.point_gate.min_gate=0.08" \
        "opt.lambda_l1_boundary=0.184"
      ;;
    v214e_v198a_amp022_cap065)
      run_one "$job" "$gpu" 3000 5.0e-06 "$BASE_CKPT" 109000 "$common_long" \
        ...
        "opt.lambda_l1_boundary=0.184"
      ;;
    v214f_v198a_luma_high_cap065)
      run_one "$job" "$gpu" 2400 4.6e-06 "$BASE_CKPT" 109000 "100,180,300,600,1200,2400" \
        "++opt.lambda_detail_waist_luma_dog=0.009" \
        "++opt.lambda_detail_upper_torso_luma_dog=0.012" \
        "++opt.lambda_detail_upper_torso_core_luma_dog=0.009" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.024" \
        "opt.lambda_l1_boundary=0.182"
      ;;
    v214g_v198a_edge_on_cap065)
      run_one "$job" "$gpu" 2400 4.8e-06 "$BASE_CKPT" 109000 "100,180,300,600,1200,2400" \
        "++opt.reliable_view_apply_edge=true" \
        "opt.lambda_edge_waist=0.010" \
        "opt.lambda_l1_boundary=0.186" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.024"
      ;;
    v214h_v198a_lowpercep_cap065)
      run_one "$job" "$gpu" 3000 5.0e-06 "$BASE_CKPT" 109000 "$common_long" \
        ...
        "opt.lambda_l1_boundary=0.188"
      ;;
    v214i_v198a_scale045_strict)
      run_one "$job" "$gpu" 1800 5.6e-06 "$BASE_CKPT" 109000 "$common_mid" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=[0.0,1,0.24,80,0.38,160,0.45]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.032" \
        "opt.lambda_l1_boundary=0.188"
      ;;
    v214j_v198a_cap085_probe)
      run_one "$job" "$gpu" 2400 3.8e-06 "$BASE_CKPT" 109000 "100,180,300,600,1200,2400" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=[0.0,1,0.24,120,0.48,400,0.70,1200,0.85]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.020" \
        "opt.lambda_l1_boundary=0.190" \
        "opt.grad_clip=0.0030"
      ;;
    v214k_v198a_mlp_only_cap065)
      run_one "$job" "$gpu" 2400 5.0e-06 "$BASE_CKPT" 109000 "100,180,300,600,1200,2400" \
        "++opt.texture_trainable_name_patterns=[detail_high_freq_view_conflict_mlp.*]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.gate_bias=-0.48" \
        "opt.lambda_l1_boundary=0.184"
      ;;
    v214l_v198a_gate_bias_lower)
      run_one "$job" "$gpu" 2400 5.0e-06 "$BASE_CKPT" 109000 "100,180,300,600,1200,2400" \
        ...
        "opt.lambda_l1_boundary=0.184"
      ;;
    v214m_v213a100_cont_lowlr)
      run_one "$job" "$gpu" 1800 1.6e-06 "$V213A100_CKPT" 109100 "$cont_mid" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.65" \
        "opt.lambda_l1_boundary=0.188" \
        "opt.grad_clip=0.0026"
      ;;
    v214n_v213a100_gate_only)
      run_one "$job" "$gpu" 1800 1.2e-06 "$V213A100_CKPT" 109100 "$cont_mid" \
        "++opt.texture_trainable_name_patterns=[detail_high_freq_view_conflict_gate_mlp.*]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.65" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.gate_bias=-0.58" \
        "opt.lambda_l1_boundary=0.188" \
        "opt.grad_clip=0.0022"
      ;;
    v214o_v213a100_mlp_only)
      run_one "$job" "$gpu" 1800 1.4e-06 "$V213A100_CKPT" 109100 "$cont_mid" \
        "++opt.texture_trainable_name_patterns=[detail_high_freq_view_conflict_mlp.*]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.60" \
        "opt.lambda_l1_boundary=0.188" \
        "opt.grad_clip=0.0024"
      ;;
    v214p_v213a100_cap055)
      run_one "$job" "$gpu" 2400 1.8e-06 "$V213A100_CKPT" 109100 "100,200,400,800,1200,1800,2400" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.55" \
        "opt.lambda_l1_boundary=0.190" \
        "opt.lambda_perceptual=0.112" \
        "opt.grad_clip=0.0024"
      ;;
    v214q_v213a100_boundaryhold)
      run_one "$job" "$gpu" 3000 1.5e-06 "$V213A100_CKPT" 109100 "100,200,400,800,1200,1800,2400,3000" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.65" \
        "opt.lambda_l1_boundary=0.198" \
        "++opt.perceptual_adaptive_edge_protect=0.94" \
        "opt.grad_clip=0.0020"
      ;;
    v214r_v213a100_lpips_polish)
      run_one "$job" "$gpu" 3000 1.2e-06 "$V213A100_CKPT" 109100 "100,200,400,800,1200,1800,2400,3000" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.62" \
        "opt.lambda_l1_boundary=0.184" \
        ...
        "opt.grad_clip=0.0020"
      ;;
    v214s_v213a100_ultralow_long)
      run_one "$job" "$gpu" 4200 8.0e-07 "$V213A100_CKPT" 109100 "200,400,800,1200,1800,2400,3000,4200" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.60" \
        "opt.lambda_l1_boundary=0.192" \
        "opt.grad_clip=0.0018"
      ;;
    v214t_v210b150_cap065)
      run_one "$job" "$gpu" 2400 3.8e-06 "$V210B_CKPT" 109150 "100,200,400,800,1200,1800,2400" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=[0.0,1,0.24,120,0.46,400,0.65]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.022" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.init_output_scale=0.30" \
        "opt.lambda_l1_boundary=0.186" \
        "opt.grad_clip=0.0028"
      ;;
    v214u_v210b150_amp018)
      run_one "$job" "$gpu" 3000 3.2e-06 "$V210B_CKPT" 109150 "100,200,400,800,1200,1800,2400,3000" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=[0.0,1,0.22,120,0.42,500,0.62]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.018" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.init_output_scale=0.26" \
        "opt.lambda_l1_boundary=0.188" \
        "opt.grad_clip=0.0026"
      ;;
    v214v_v210b150_boundary)
      run_one "$job" "$gpu" 3000 3.0e-06 "$V210B_CKPT" 109150 "100,200,400,800,1200,1800,2400,3000" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.58" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.020" \
        ...
        "opt.grad_clip=0.0024"
      ;;
    v214w_v210b150_lowpercep)
      run_one "$job" "$gpu" 2400 3.4e-06 "$V210B_CKPT" 109150 "100,200,400,800,1200,1800,2400" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.62" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.020" \
        "opt.lambda_perceptual=0.108" \
        "opt.lambda_l1_boundary=0.190" \
        "opt.grad_clip=0.0026"
      ;;
    v214x_v210b150_cloth_strict_skin)
      run_one "$job" "$gpu" 2400 3.4e-06 "$V210B_CKPT" 109150 "100,200,400,800,1200,1800,2400" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.62" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.022" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.point_gate.exclude_semantic_id_weights=[[0,1.00],[1,1.00],[2,0.90],[5,1.00]]" \
        ...
        "opt.grad_clip=0.0026"
      ;;
    v214y_v210b150_gate_only)
      run_one "$job" "$gpu" 1800 1.8e-06 "$V210B_CKPT" 109150 "100,200,400,800,1200,1800" \
        "++opt.texture_trainable_name_patterns=[detail_high_freq_view_conflict_gate_mlp.*]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.62" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.020" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.init_output_scale=0.28" \
