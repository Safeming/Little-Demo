...
START_CKPT="$ROOT/exp/stageA2/377_multiview_explicit_hq_rootfix_resume_v175a_v170e_hfsource_edgesupervise_screen2500/best_ckpt.pth"
START_SCRIPT="$ROOT/tools/start_377_stageA2_multiview_explicit_rootfix_resume_v169a_v168a_hardcontour_boundaryactuator.sh"

mkdir -p "$LOG_DIR"

BATCH_ID="${V176_RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
SUMMARY="$LOG_DIR/v176_batch_${BATCH_ID}_summary.tsv"
REGISTRY="$LOG_DIR/v176_batch_${BATCH_ID}_runs.txt"
RUNNER_LOG="$LOG_DIR/v176_batch_${BATCH_ID}.log"

printf '%s\n' "$RUNNER_LOG" > "$LOG_DIR/v176_latest_runner_log.txt"
printf '%s\n' "$SUMMARY" > "$LOG_DIR/v176_latest_summary.txt"
printf '%s\n' "$BATCH_ID" > "$LOG_DIR/v176_latest_run_id.txt"
...
run_render() {
  ...
  printf '%s\n' "$render_log" > "$LOG_DIR/v176_latest_render_log.txt"
...
run_one() {
  ...
  local exp_dir="$ROOT/exp/stageA2/377_multiview_explicit_hq_rootfix_resume_${tag}_v175a_${slug}_screen${iterations}"
  local train_log="$LOG_DIR/${tag}_${BATCH_ID}_${slug}.log"

  printf '%s\t%s\t%s\t%s\n' "$tag" "$exp_dir" "$train_log" "$option_name" >> "$REGISTRY"
  printf '%s\n' "$exp_dir" > "$LOG_DIR/v176_latest_exp.txt"
  printf '%s\n' "$train_log" > "$LOG_DIR/v176_latest_log.txt"
...
echo "[$(date '+%F %T')] v176 batch started"
echo "summary=$SUMMARY"
echo "registry=$REGISTRY"

run_one "v176a" 3000 "stageA_377_multiview_explicit_hq_v176a_v175a_sameframe_k2_v1" "sameframe_k2_hfsource"

echo "[$(date '+%F %T')] v176 batch finished"
cat "$SUMMARY"
