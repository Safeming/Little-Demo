...
RUN_ID="${RUN_ID:-v367_residual_grouped_actuator_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v367_residual_grouped_actuator_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v367_residual_grouped_actuator_${RUN_ID}}"
...
V367_BOOTSTRAP_ASSET_JSON="$ASSET_DIR/v367_bootstrap_residual_grouped_actuator_asset.json"
V367_BOOTSTRAP_CANDIDATES_TSV="$ASSET_DIR/v367_bootstrap_residual_grouped_actuator_groups.tsv"
V367_SEED_ASSET_JSON="$ASSET_DIR/v367_seed_residual_grouped_actuator_asset.json"
V367_SEED_CANDIDATES_TSV="$ASSET_DIR/v367_seed_residual_grouped_actuator_groups.tsv"
V367_ASSET_JSON="$ASSET_DIR/v367_validated_residual_grouped_actuator_asset.json"
V367_ACTION_LIST_TSV="$ASSET_DIR/v367_seed_action_groups.tsv"
V367_RENDERER_SPACE_NPZ="$ASSET_DIR/v367_renderer_space_gaussians.npz"
V367_RENDERER_SPACE_TSV="$ASSET_DIR/v367_renderer_space_gaussians.tsv"
RAW_GATE_LOG="$LOG_DIR/v367_raw_gate.launcher.log"
TRAIN_LAUNCH_LOG="$LOG_DIR/v367_semantic_train.launcher.log"
...
make_asset() {
  ...
  "$PYTHON_BIN" tools/make_377_stageB_v367_residual_grouped_actuator_asset.py "${args[@]}"
}
...
log_event "make_bootstrap_asset_start" "$V367_BOOTSTRAP_ASSET_JSON"
make_asset "$V367_BOOTSTRAP_ASSET_JSON" "$V367_BOOTSTRAP_CANDIDATES_TSV" \
  > "$LOG_DIR/make_v367_bootstrap_asset.log" 2>&1
write_action_list "$V367_BOOTSTRAP_ASSET_JSON" "$V367_ACTION_LIST_TSV" -1
log_event "make_bootstrap_asset_done" "$V367_BOOTSTRAP_ASSET_JSON"

log_event "export_renderer_space_start" "$V367_RENDERER_SPACE_NPZ"
env "${COMMON_ENV[@]}" "$PYTHON_BIN" tools/export_renderer_space_gaussians.py \
  --config "$BASE_EXP/.hydra/config.yaml" \
  --checkpoint "$CANDIDATE_CKPT" \
  --action-list-tsv "$V367_ACTION_LIST_TSV" \
  --out-npz "$V367_RENDERER_SPACE_NPZ" \
  --out-tsv "$V367_RENDERER_SPACE_TSV" \
...
log_event "export_renderer_space_done" "$V367_RENDERER_SPACE_NPZ"

log_event "make_seed_asset_start" "$V367_SEED_ASSET_JSON"
make_asset "$V367_SEED_ASSET_JSON" "$V367_SEED_CANDIDATES_TSV" \
  --renderer-space-cache "$V367_RENDERER_SPACE_NPZ" \
  > "$LOG_DIR/make_v367_seed_asset.log" 2>&1
write_action_list "$V367_SEED_ASSET_JSON" "$V367_ACTION_LIST_TSV" "$GROUP_VALIDATE_MAX"
log_event "make_seed_asset_done" "$V367_SEED_ASSET_JSON"
...
while IFS=$'\t' read -r action_index pair_id source_component_key image_name view frame micro_count outer_action_count score; do
  ...
  candidate_exp="$group_root/v367_grouped_actuator"
  ...
  render_and_analyze "group_${safe_pair}" "$views_spec" "$frames_spec" v367_grouped_actuator "$candidate_exp" \
    "pipeline.compute_cov3D_python=true" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_signed_dynamic_component_local_asset_json=$V367_SEED_ASSET_JSON" \
    "++pipeline.covariance_signed_dynamic_component_action_filter='pair_id=$pair_id'" \
    "++pipeline.covariance_signed_dynamic_component_action_required=false" \
    "++pipeline.split_child_component_enable=true" \
    "++pipeline.split_child_component_asset_json=$V367_SEED_ASSET_JSON" \
...
done < "$V367_ACTION_LIST_TSV"

"$PYTHON_BIN" - "$V367_SEED_ASSET_JSON" "$GROUP_VALIDATION_TSV" "$V367_ASSET_JSON" <<'PY'
import csv
import json
import sys
from pathlib import Path
...
payload = {
    **{key: value for key, value in data.items() if key not in ("action_groups", "children", "actions")},
    "version": "v367_validated_residual_driven_grouped_actuator_asset",
    "policy": (
        "Validated v367 asset: one best action-level raw-contour-safe grouped actuator per source residual component."
    ),
...
"$PYTHON_BIN" - "$SUMMARY" "$GROUP_VALIDATION_TSV" "$V367_ASSET_JSON" <<'PY'
import csv
import json
import sys
from pathlib import Path
...
if [ "$RAW_GATE_ENABLE" = "true" ]; then
  selected_groups="$("$PYTHON_BIN" - "$V367_ASSET_JSON" <<'PY'
import json, sys
...
PY
  ...
  if [ "$selected_groups" -gt 0 ]; then
    RAW_GATE_RUN_ID="formal_377_v367_residual_grouped_actuator_raw_gate_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')"
    ...
    env GPU="$GPU" PYTHON_BIN="$PYTHON_BIN" CPU_THREADS_PER_JOB="$CPU_THREADS_PER_JOB" \
      ...
      CANDIDATE_VARIANT_NAME="candidate_v367_residual_grouped_actuator" \
      CANDIDATE_SIGNED_DYNAMIC_COMPONENT_LOCAL_ASSET_JSON="$V367_ASSET_JSON" \
      CANDIDATE_SPLIT_CHILD_COMPONENT_ENABLE=true \
      CANDIDATE_SPLIT_CHILD_COMPONENT_ASSET_JSON="$V367_ASSET_JSON" \
...
with open(path, encoding="utf-8", newline="") as handle:
    for row in csv.DictReader(handle, delimiter="\t"):
        if row.get("variant") == "candidate_v367_residual_grouped_actuator":
...
if [ "$TRAIN_ON_STRICT_PASS" = "true" ] && [ "$RAW_GATE_STATUS" = "strict_pass" ]; then
  TRAIN_RUN_ID="formal_377_v367_residual_grouped_actuator_semantic_train_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')"
  TRAIN_EXP_DIR="$ROOT/exp/formal/377_v367_residual_grouped_actuator_semantic_train_${TRAIN_RUN_ID}"
  ...
  (
    env GPU="$GPU" PYTHON_BIN="$PYTHON_BIN" CPU_THREADS_PER_JOB="$CPU_THREADS_PER_JOB" \
      ...
      "++explicit_binding_adopted_signed_dynamic_component_local_asset_json=$V367_ASSET_JSON" \
      "++pipeline.split_child_component_enable=true" \
      "++pipeline.split_child_component_asset_json=$V367_ASSET_JSON" \
      "++pipeline.split_child_component_action_required=false" \
      "++pipeline.split_child_component_opacity=$CHILD_OPACITY" \
      "++pipeline.split_child_component_radius_scale=1.0" \
      "++pipeline.split_child_component_max_children=-1"
  ) > "$TRAIN_LAUNCH_LOG" 2>&1 &
  TRAIN_PID="$!"
...
echo "V367_ASSET_JSON=$V367_ASSET_JSON"
