...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-v317_pose_render_factor_ab_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
...
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v317_pose_render_factor_ab_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v317_pose_render_factor_ab_${RUN_ID}}"
HYDRA_RUN_ROOT="$LOG_DIR/hydra_runtime"
EVENTS="$LOG_DIR/events.tsv"
VARIANTS_TSV="$LOG_DIR/variants.tsv"
...
Goal:
  v317 is a root-cause no-train A/B over the explicit-binding pose render path.
  It keeps explicit binding active for pose deformation, but switches the rendered
  center/rotation among x_bar, x_rigid, x_soft, x_free, hard layer assignment,
  non-rigid geometry gating, v41 priors, and non-rigid carry. Short color/texture
  training runs only if a no-train candidate passes the raw contour gate.
EOF

printf 'variant\tpose_mode\trotation_mode\tcenter_blend\trotation_blend\tgeom_blend\trigid_carry\tsoft_carry\tv41_priors\tsoft_rigid_blend\tsoft_normal_blend\torthogonalize\n' > "$VARIANTS_TSV"
printf '%s\n' \
  'baseline_current	layer_blend	matching	1.0	1.0	0.35	0.4	0.7	true	0.6	0.8	false' \
  'geom0_layer_blend	layer_blend	matching	1.0	1.0	0.0	0.4	0.7	true	0.6	0.8	false' \
  'v41_off_layer_blend	layer_blend	matching	1.0	1.0	0.35	0.4	0.7	false	0.6	0.8	false' \
  'carry0_layer_blend	layer_blend	matching	1.0	1.0	0.35	0.0	0.0	true	0.6	0.8	false' \
  'hard_layer_matching	hard_layer	matching	1.0	1.0	0.35	0.4	0.7	true	0.6	0.8	false' \
  'rigid_matching	rigid	matching	1.0	1.0	0.35	0.4	0.7	true	0.6	0.8	false' \
  'soft_matching	soft	matching	1.0	1.0	0.35	0.4	0.7	true	0.6	0.8	false' \
  'free_matching	free	matching	1.0	1.0	0.35	0.4	0.7	true	0.6	0.8	false' \
  'soft_center_barrot	soft	bar	1.0	1.0	0.35	0.4	0.7	true	0.6	0.8	false' \
  'free_center_barrot	free	bar	1.0	1.0	0.35	0.4	0.7	true	0.6	0.8	false' \
  'soft_free_blend	soft_free_blend	matching	1.0	1.0	0.35	0.4	0.7	true	0.6	0.8	false' \
  'rigid_soft_blend	rigid_soft_blend	matching	1.0	1.0	0.35	0.4	0.7	true	0.6	0.8	false' \
  'layer_blend_rot_free	layer_blend	free	1.0	1.0	0.35	0.4	0.7	true	0.6	0.8	false' \
  'layer_blend_orth	layer_blend	matching	1.0	1.0	0.35	0.4	0.7	true	0.6	0.8	true' \
  >> "$VARIANTS_TSV"
...
COMMON_BINDING_ARGS=(
  ...
  "++model.deformer.rigid.identity_render_enable=false"
)
...
render_variant() {
  ...
  local center_blend="$6"
  local rotation_blend="$7"
  local geom_blend="$8"
  local rigid_carry="$9"
  local soft_carry="${10}"
  local v41_priors="${11}"
  local soft_rigid_blend="${12}"
  local soft_normal_blend="${13}"
  local orthogonalize="${14}"
  local config_dir="${15}"
  local hydra_dir="${16}"
  ...
  env "${COMMON_ENV[@]}" "$PYTHON_BIN" render.py \
    ...
    "${COMMON_RENDER_ARGS[@]}" \
    "${COMMON_BINDING_ARGS[@]}" \
    "++model.deformer.rigid.pose_render_mode=$pose_mode" \
    "++model.deformer.rigid.pose_render_rotation_mode=$rotation_mode" \
    "++model.deformer.rigid.pose_render_center_blend=$center_blend" \
    "++model.deformer.rigid.pose_render_rotation_blend=$rotation_blend" \
    ...
    "++model.deformer.rigid.v41_priors_enable=$v41_priors" \
    "++model.deformer.rigid.soft_rigid_blend=$soft_rigid_blend" \
    "++model.deformer.rigid.soft_normal_blend=$soft_normal_blend" \
    "++model.deformer.rigid.rotation_orthogonalize_enable=$orthogonalize" \
...
run_render_eval() {
  ...
  local ckpt="$2"
  local pose_mode="$3"
  local rotation_mode="$4"
  local center_blend="$5"
  local rotation_blend="$6"
  local geom_blend="$7"
  local rigid_carry="$8"
  local soft_carry="$9"
  local v41_priors="${10}"
  local soft_rigid_blend="${11}"
  local soft_normal_blend="${12}"
  local orthogonalize="${13}"
  local config_dir="${14}"
  local render_exp="$EXP_ROOT/$variant"

  log_event "render_start" "$variant pose=$pose_mode rot=$rotation_mode geom=$geom_blend carry=$rigid_carry/$soft_carry v41=$v41_priors"
  render_variant "$variant" "$ckpt" "$render_exp" "$pose_mode" "$rotation_mode" "$center_blend" "$rotation_blend" "$geom_blend" "$rigid_carry" "$soft_carry" "$v41_priors" "$soft_rigid_blend" "$soft_normal_blend" "$orthogonalize" "$config_dir" "$HYDRA_RUN_ROOT/render_${variant}"
  log_event "analyze_start" "$variant"
  analyze_variant "$variant" "$render_exp"
  log_event "render_done" "$variant"
}

while IFS=$'\t' read -r variant pose_mode rotation_mode center_blend rotation_blend geom_blend rigid_carry soft_carry v41_priors soft_rigid_blend soft_normal_blend orthogonalize; do
  if [ "$variant" = "variant" ]; then
    continue
  fi
  run_render_eval "$variant" "$BASE_CKPT" "$pose_mode" "$rotation_mode" "$center_blend" "$rotation_blend" "$geom_blend" "$rigid_carry" "$soft_carry" "$v41_priors" "$soft_rigid_blend" "$soft_normal_blend" "$orthogonalize" "$BASE_EXP/.hydra"
done < "$VARIANTS_TSV"

"$PYTHON_BIN" - "$NO_TRAIN_SUMMARY" "$VARIANTS_TSV" "$EXP_ROOT" "$SELECTED_ENV" <<'PY'
import csv
import json
import shlex
import sys
...
selected_env = Path(sys.argv[4])
variants = list(csv.DictReader(variants_path.open(encoding="utf-8"), delimiter="\t"))
...
metrics = {row["variant"]: load_metrics(row["variant"]) for row in variants}
baseline = metrics.get("baseline_current")
if baseline is None:
    raise RuntimeError("baseline_current metrics missing")

header = [
    "variant", "pose_mode", "rotation_mode", "center_blend", "rotation_blend",
    "geom_blend", "rigid_carry", "soft_carry", "v41_priors",
    "soft_rigid_blend", "soft_normal_blend", "orthogonalize", "render_exp",
...
for spec in variants:
    ...
    if values is None:
        rows.append([spec.get(key, "") for key in header[:12]] + [str(exp_root / name)] + ["nan"] * 12 + ["false", "false", "missing"])
        continue
    ...
    row = [
        ...
        spec["soft_carry"],
        spec["v41_priors"],
        spec["soft_rigid_blend"],
        spec["soft_normal_blend"],
        spec["orthogonalize"],
        values["render_exp"],
        f"{values['fg']:.8f}",
    ...
    if status in ("strict_pass", "probe_pass"):
        rank = (0 if status == "strict_pass" else 1, values["hard"], values["edge"], values["fg"], values["inner"])
        if selected is None or rank < selected[0]:
            selected = (rank, spec, status)
...
if selected is None:
    ...
    _, spec, status = selected
    values = {
        "TRAIN_SELECTED": "1",
        "SELECT_REASON": status,
        "SELECTED_VARIANT": spec["variant"],
        "SELECTED_POSE_MODE": spec["pose_mode"],
        "SELECTED_ROTATION_MODE": spec["rotation_mode"],
        "SELECTED_CENTER_BLEND": spec["center_blend"],
        "SELECTED_ROTATION_BLEND": spec["rotation_blend"],
        "SELECTED_GEOM_BLEND": spec["geom_blend"],
        "SELECTED_RIGID_CARRY": spec["rigid_carry"],
        "SELECTED_SOFT_CARRY": spec["soft_carry"],
        "SELECTED_V41_PRIORS": spec["v41_priors"],
        "SELECTED_SOFT_RIGID_BLEND": spec["soft_rigid_blend"],
        "SELECTED_SOFT_NORMAL_BLEND": spec["soft_normal_blend"],
        "SELECTED_ORTHOGONALIZE": spec["orthogonalize"],
    }
    selected_env.write_text("".join(f"{key}={shlex.quote(str(value))}\n" for key, value in values.items()), encoding="utf-8")
PY

source "$SELECTED_ENV"
log_event "no_train_done" "selected=${SELECTED_VARIANT:-none} reason=${SELECT_REASON:-none}"

if [ "$RUN_TRAIN" = "1" ] && [ "${TRAIN_SELECTED:-0}" = "1" ]; then
  ...
  env "${COMMON_ENV[@]}" "$PYTHON_BIN" train.py \
    ...
    "${COMMON_BINDING_ARGS[@]}" \
    "++model.deformer.rigid.pose_render_mode=$SELECTED_POSE_MODE" \
    "++model.deformer.rigid.pose_render_rotation_mode=$SELECTED_ROTATION_MODE" \
    "++model.deformer.rigid.pose_render_center_blend=$SELECTED_CENTER_BLEND" \
    "++model.deformer.rigid.pose_render_rotation_blend=$SELECTED_ROTATION_BLEND" \
    ...
    "++model.deformer.rigid.v41_priors_enable=$SELECTED_V41_PRIORS" \
    ...
    "++model.deformer.rigid.rotation_orthogonalize_enable=$SELECTED_ORTHOGONALIZE" \
  ...
  for step in "${steps[@]}"; do
    ...
    if [ -f "$ckpt" ]; then
      run_render_eval "$variant" "$ckpt" "$SELECTED_POSE_MODE" "$SELECTED_ROTATION_MODE" "$SELECTED_CENTER_BLEND" "$SELECTED_ROTATION_BLEND" "$SELECTED_GEOM_BLEND" "$SELECTED_RIGID_CARRY" "$SELECTED_SOFT_CARRY" "$SELECTED_V41_PRIORS" "$SELECTED_SOFT_RIGID_BLEND" "$SELECTED_SOFT_NORMAL_BLEND" "$SELECTED_ORTHOGONALIZE" "$TRAIN_EXP/.hydra"
    else
      log_event "train_render_skip" "missing=$ckpt"
    fi
  done
else
  log_event "train_skip" "RUN_TRAIN=$RUN_TRAIN TRAIN_SELECTED=${TRAIN_SELECTED:-0} reason=${SELECT_REASON:-none}"
fi
...
baseline = load_metrics(exp_root / "baseline_current")
header = [
