...
def _project_segments(raw, prediction_mask, runtime_mass, a5_weight, camera_index,
                      frame_index, block_ids, contract) -> tuple[np.ndarray, list[dict]]:
...
def train_fold(*, fold, features, pose, adjacency, visibility, base_gates,
               teacher_gates, teacher_mask, prediction_mask, camera_index,
               frame_index, block_ids, runtime_mass, a5_weight, contract,
               output_dir, device) -> dict:
    ...
    if poses.shape != (samples, 36) or teachers.shape != (samples, carriers):
    ...
    if sum(segment.size for segment in training_segments) != int(fit_mask.sum()):
    ...
    tensors = {
        ...
        "base": torch.as_tensor(np.asarray(base_gates, np.float32), device=device),
    ...
    with torch.no_grad():
        initial = {key: float(value.cpu()) for key, value in aggregate_components().items()}
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) in {1, epochs}:
            print(json.dumps({"fold": int(fold), "epoch": epoch + 1, "epochs": epochs}), flush=True)
    with torch.no_grad():
        final = {key: float(value.cpu()) for key, value in aggregate_components().items()}
    ...
    temporal_errors = []
    for segment in training_segments:
        temporal_errors.append(np.abs(np.diff(raw[segment], axis=0) - np.diff(teachers[segment], axis=0)))
    temporal_mae = float(np.mean(np.concatenate([row.reshape(-1) for row in temporal_errors])))
    checkpoint = {
    ...
    np.savez_compressed(
        ...
        prediction_mask=predict_mask, teacher_mask=fit_mask,
        camera_index=np.asarray(camera_index), frame_index=np.asarray(frame_index),
        block_ids=np.asarray(block_ids), feature_mean=feature_stats["mean"],
        feature_scale=feature_stats["scale"], pose_mean=pose_stats["mean"],
        pose_scale=pose_stats["scale"], deployment_eligible=np.asarray(0, np.uint8),
        teacher_eligible=np.asarray(0, np.uint8), paper_test_eligible=np.asarray(0, np.uint8),
    )
    ...
    summary = {
        ...
        "parameter_count": int(parameter_count), "training_segment_count": len(training_segments),
        "training_sample_count": int(fit_mask.sum()), "prediction_sample_count": int(predict_mask.sum()),
        "initial_components": initial, "final_components": final,
        "maximum_gradient_norm_before_clip": maximum_gradient_norm,
        "raw_minimum_gate": float(np.nanmin(raw)), "raw_maximum_gate": float(np.nanmax(raw)),
        "projected_minimum_gate": float(np.nanmin(projected)),
        "projected_maximum_gate": float(np.nanmax(projected)),
        "fit_teacher_mae": fit_mae, "fit_temporal_difference_mae": temporal_mae,
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Train frozen R1.4-VP view-pose folds.")
...
def main(argv=None) -> int:
    ...
    pose_values = load_pose_rotation_6d(args.pose_model_dir, unique_frames, contract["pose_body_joint_indices"])
    ...
    r1_contract = json.loads((REPO_ROOT / contract["source_r1_1_contract"]).read_text(encoding="utf-8"))
    ...
    runtime = build_runtime_inputs(
        ...
        spatial_scale=float(contract["spatial_scale"]), depth_scale=float(contract["depth_scale"]),
    ...
    learned, baselines, freeze_paths = [], [], []
    full_features = np.asarray(probe["features"], np.float32)
    feature_names = list(map(str, probe["feature_names"]))
    nn_keys = build_nearest_neighbor_keys(full_features, feature_names, runtime["pose"])
    for fold, held in enumerate(split["held_block_masks"]):
        ...
        with np.load(args.r1_2b_training_dir / f"fold_{fold}/predictions.npz", allow_pickle=False) as source:
        ...
        learned.append(train_fold(fold=fold, features=runtime["features"], pose=runtime["pose"],
            adjacency=runtime["adjacency"], visibility=runtime["visibility"], base_gates=base,
            teacher_gates=teacher_gates, teacher_mask=teacher_mask, prediction_mask=prediction_mask,
            camera_index=cameras, frame_index=frames, block_ids=blocks, runtime_mass=runtime["runtime_mass"],
            a5_weight=a5_weight, contract=contract, output_dir=fold_root, device=args.device))
        nn_raw = np.full(base.shape, np.nan, np.float64)
        nn_raw[prediction_mask] = nearest_neighbor_predict(nn_keys[teacher_mask], teacher_gates[teacher_mask], nn_keys[prediction_mask], k=int(contract["nearest_neighbor_k"]))
        nn_projected, nn_certs = _project_segments(nn_raw, prediction_mask, runtime["runtime_mass"], a5_weight, cameras, frames, blocks, contract)
        nn_root = args.output_dir / "nearest_neighbor" / f"fold_{fold}"
        nn_root.mkdir(parents=True, exist_ok=True)
        np.savez_compressed(nn_root / "predictions.npz", raw_gates=nn_raw, projected_gates=nn_projected,
            prediction_mask=prediction_mask, teacher_mask=teacher_mask,
            camera_index=cameras, frame_index=frames, block_ids=blocks,
            deployment_eligible=np.asarray(0, np.uint8), teacher_eligible=np.asarray(0, np.uint8), paper_test_eligible=np.asarray(0, np.uint8))
        _write_json(nn_root / "projection_certificates.json", _jsonable(nn_certs))
        nn_summary = {"fold": fold, "k": int(contract["nearest_neighbor_k"]),
            "fit_teacher_mae": float(np.mean(np.abs(nn_raw[teacher_mask] - teacher_gates[teacher_mask]))),
            "deployment_eligible": False, "teacher_eligible": False, "paper_test_eligible": False}
        _write_json(nn_root / "summary.json", nn_summary)
        baselines.append(nn_summary)
        freeze_paths.extend([fold_root / "model.pt", fold_root / "predictions.npz", fold_root / "projection_certificates.json", fold_root / "summary.json",
                             nn_root / "predictions.npz", nn_root / "projection_certificates.json", nn_root / "summary.json"])
    _write_json(args.output_dir / "training/summary.json", {"folds": learned, "paper_test_eligible": False})
    _write_json(args.output_dir / "nearest_neighbor/summary.json", {"folds": baselines, "paper_test_eligible": False})
    frozen = {str(path.relative_to(args.output_dir)): _sha256(path) for path in freeze_paths}
    _write_json(args.output_dir / "models_frozen.json", {"artifacts": frozen, "paper_test_eligible": False})
    return 0
