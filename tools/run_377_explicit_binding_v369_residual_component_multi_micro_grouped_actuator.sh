...
RUN_ID="${RUN_ID:-v369_residual_component_multi_micro_grouped_actuator_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v369_residual_component_multi_micro_grouped_actuator_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v369_residual_component_multi_micro_grouped_actuator_${RUN_ID}}"
...
V369_BOOTSTRAP_ASSET_JSON="$ASSET_DIR/v369_bootstrap_residual_component_multi_micro_grouped_actuator_asset.json"
V369_BOOTSTRAP_CANDIDATES_TSV="$ASSET_DIR/v369_bootstrap_residual_component_multi_micro_grouped_actuator_groups.tsv"
V369_SEED_ASSET_JSON="$ASSET_DIR/v369_seed_residual_component_multi_micro_grouped_actuator_asset.json"
V369_SEED_CANDIDATES_TSV="$ASSET_DIR/v369_seed_residual_component_multi_micro_grouped_actuator_groups.tsv"
V369_ASSET_JSON="$ASSET_DIR/v369_validated_residual_component_multi_micro_grouped_actuator_asset.json"
V369_ACTION_LIST_TSV="$ASSET_DIR/v369_seed_action_groups.tsv"
V369_RENDERER_SPACE_NPZ="$ASSET_DIR/v369_renderer_space_gaussians.npz"
V369_RENDERER_SPACE_TSV="$ASSET_DIR/v369_renderer_space_gaussians.tsv"
RAW_GATE_LOG="$LOG_DIR/v369_raw_gate.launcher.log"
TRAIN_LAUNCH_LOG="$LOG_DIR/v369_semantic_train.launcher.log"
...
for row in groups:
    by_target[(row["image_name"], row["source_component_key"], row["target_rank"])].append(row)
ordered = []
while by_target:
    keys = sorted(
        list(by_target),
        key=lambda key: (
            -by_target[key][0]["score"],
            -by_target[key][0]["target_score"],
            key[0],
            key[1],
            key[2],
        ),
...
with out_path.open("w", encoding="utf-8", newline="") as handle:
    fieldnames = [
        ...
        "micro_count", "outer_action_count", "score", "target_rank", "target_score",
    ]
...
log_event "make_bootstrap_asset_start" "$V369_BOOTSTRAP_ASSET_JSON"
make_asset "$V369_BOOTSTRAP_ASSET_JSON" "$V369_BOOTSTRAP_CANDIDATES_TSV" \
  > "$LOG_DIR/make_v369_bootstrap_asset.log" 2>&1
write_action_list "$V369_BOOTSTRAP_ASSET_JSON" "$V369_ACTION_LIST_TSV" -1
log_event "make_bootstrap_asset_done" "$V369_BOOTSTRAP_ASSET_JSON"

log_event "export_renderer_space_start" "$V369_RENDERER_SPACE_NPZ"
env "${COMMON_ENV[@]}" "$PYTHON_BIN" tools/export_renderer_space_gaussians.py \
  --config "$BASE_EXP/.hydra/config.yaml" \
  --checkpoint "$CANDIDATE_CKPT" \
  --action-list-tsv "$V369_ACTION_LIST_TSV" \
  --out-npz "$V369_RENDERER_SPACE_NPZ" \
  --out-tsv "$V369_RENDERER_SPACE_TSV" \
...
log_event "export_renderer_space_done" "$V369_RENDERER_SPACE_NPZ"

log_event "make_seed_asset_start" "$V369_SEED_ASSET_JSON"
make_asset "$V369_SEED_ASSET_JSON" "$V369_SEED_CANDIDATES_TSV" \
  --renderer-space-cache "$V369_RENDERER_SPACE_NPZ" \
  > "$LOG_DIR/make_v369_seed_asset.log" 2>&1
write_action_list "$V369_SEED_ASSET_JSON" "$V369_ACTION_LIST_TSV" "$GROUP_VALIDATE_MAX"
log_event "make_seed_asset_done" "$V369_SEED_ASSET_JSON"
...
while IFS=$'\t' read -r action_index pair_id source_component_key image_name view frame micro_count outer_action_count score; do
  ...
  candidate_exp="$group_root/v369_grouped_actuator"
  ...
  render_and_analyze "group_${safe_pair}" "$views_spec" "$frames_spec" v369_grouped_actuator "$candidate_exp" \
    "pipeline.compute_cov3D_python=true" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++pipeline.split_child_component_enable=true" \
    "++pipeline.split_child_component_asset_json=$V369_SEED_ASSET_JSON" \
...
done < "$V369_ACTION_LIST_TSV"

"$PYTHON_BIN" - "$V369_SEED_ASSET_JSON" "$GROUP_VALIDATION_TSV" "$V369_ASSET_JSON" <<'PY'
import csv
import json
import sys
from pathlib import Path
...
payload = {
    **{key: value for key, value in data.items() if key not in ("action_groups", "children", "actions")},
    "version": "v369_validated_residual_driven_grouped_actuator_asset",
    "policy": (
        "Validated v369 asset: one best action-level raw-contour-safe grouped actuator per source residual target."
    ),
...
"$PYTHON_BIN" - "$SUMMARY" "$GROUP_VALIDATION_TSV" "$V369_ASSET_JSON" <<'PY'
import csv
import json
import sys
from pathlib import Path
...
if [ "$RAW_GATE_ENABLE" = "true" ]; then
  selected_groups="$("$PYTHON_BIN" - "$V369_ASSET_JSON" <<'PY'
import json, sys
...
PY
  ...
  if [ "$selected_groups" -gt 0 ]; then
    RAW_GATE_RUN_ID="formal_377_v369_residual_component_multi_micro_grouped_actuator_raw_gate_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')"
    ...
    env GPU="$GPU" PYTHON_BIN="$PYTHON_BIN" CPU_THREADS_PER_JOB="$CPU_THREADS_PER_JOB" \
      ...
      CANDIDATE_VARIANT_NAME="candidate_v369_residual_component_multi_micro_grouped_actuator" \
      CANDIDATE_SPLIT_CHILD_COMPONENT_ENABLE=true \
      CANDIDATE_SPLIT_CHILD_COMPONENT_ASSET_JSON="$V369_ASSET_JSON" \
...
with open(path, encoding="utf-8", newline="") as handle:
    for row in csv.DictReader(handle, delimiter="\t"):
        if row.get("variant") == "candidate_v369_residual_component_multi_micro_grouped_actuator":
...
if [ "$TRAIN_ON_STRICT_PASS" = "true" ] && [ "$RAW_GATE_STATUS" = "strict_pass" ]; then
  TRAIN_RUN_ID="formal_377_v369_residual_component_multi_micro_grouped_actuator_semantic_train_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')"
  TRAIN_EXP_DIR="$ROOT/exp/formal/377_v369_residual_component_multi_micro_grouped_actuator_semantic_train_${TRAIN_RUN_ID}"
  TRAIN_SCRIPT="$LOG_DIR/v369_semantic_train.launch.sh"
...
env GPU="$GPU" PYTHON_BIN="$PYTHON_BIN" CPU_THREADS_PER_JOB="$CPU_THREADS_PER_JOB" \\
  ...
  "++pipeline.split_child_component_enable=true" \\
  "++pipeline.split_child_component_asset_json=$V369_ASSET_JSON" \\
...
echo "V369_ASSET_JSON=$V369_ASSET_JSON"
