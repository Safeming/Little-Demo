...
RUN_ID="${RUN_ID:-stageB_v239_v240_boundary_repair_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
GPUS="${GPUS:-1,2,3}"
BRANCHES_CSV="${BRANCHES_CSV:-v239a_v237a1000_micro_finish,v239b_v233d_ultra_mild_geom_short,v240a_boundary_clone_tiny_once}"
SEED="${SEED:--1}"
...
BASE_V233D_EXP="${BASE_V233D_EXP:-$ROOT/exp/stageB/377_hulk_light_v233d_shoes_preserve_control_stageB_compact_v233_skincloth_20260512_161652_bjt}"
BASE_V233D_CKPT="${BASE_V233D_CKPT:-$BASE_V233D_EXP/ckpt135710.pth}"
BASE_V233D_ITER="${BASE_V233D_ITER:-135710}"

BASE_V237A_EXP="${BASE_V237A_EXP:-$ROOT/exp/stageB/377_hulk_light_v237a_boundary_geom_mild_stageB_v237_v238_boundary_support_20260513_153533_bjt}"
BASE_V237A_CKPT="${BASE_V237A_CKPT:-$BASE_V237A_EXP/ckpt136710.pth}"
BASE_V237A_ITER="${BASE_V237A_ITER:-136710}"

LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_stageB_v239_v240_boundary_repair_${RUN_ID}}"
...
for required in \
  "$PYTHON_BIN" \
  "$BASE_V233D_EXP/.hydra/config.yaml" \
  "$BASE_V233D_CKPT" \
  "$BASE_V237A_EXP/.hydra/config.yaml" \
  "$BASE_V237A_CKPT" \
  "$DATA_ROOT" \
  "$PARSER_ROOT" \
  "$COMPACT_MAPPING"; do
...

{
  ...
  echo "BASE_V233D_EXP=$BASE_V233D_EXP"
  echo "BASE_V233D_CKPT=$BASE_V233D_CKPT"
  echo "BASE_V233D_ITER=$BASE_V233D_ITER"
  echo "BASE_V237A_EXP=$BASE_V237A_EXP"
  echo "BASE_V237A_CKPT=$BASE_V237A_CKPT"
  echo "BASE_V237A_ITER=$BASE_V237A_ITER"
  echo "PURPOSE=v239 no-densify micro geometry probes plus v240 strict tiny boundary densify"
...
render_checkpoint() {
  ...
  local render_exp="${exp_dir}_render_boundary_repair_ckpt${global_iter}"
  if [ ! -f "$ckpt" ]; then
...
train_branch() {
  ...
  local base_iter="$5"
  local iterations="$6"
  local checkpoint_steps="$7"
  local disable_densify="$8"
  local require_no_densify="$9"
  shift 9
  local overrides=("$@")
  ...
  log_event "$gpu" "$name" "train_start" "base_iter=$base_iter iterations=$iterations disable_densify_on_resume=$disable_densify"
  ...
  if [ "$DO_RENDER" = "1" ]; then
    ...
    for step in "${STEPS[@]}"; do
      render_checkpoint "$name" "$gpu" "$exp_dir" "$base_iter" "$step" || true
    done
  fi
}
...
run_branch() {
  ...
  case "$name" in
    v239a_v237a1000_micro_finish)
      train_branch "$name" "$gpu" "$BASE_V237A_EXP" "$BASE_V237A_CKPT" "$BASE_V237A_ITER" 800 "400,800" true true \
        "opt.position_lr_init=0.0000007" \
        "opt.position_lr_final=0.0000002" \
        "opt.scaling_lr=0.0000020" \
        "++opt.boundary_aware_xyz_scale=0.012" \
        "++opt.boundary_aware_scaling_scale=0.050" \
        "++opt.boundary_aware_opacity_scale=0.16" \
        "++opt.boundary_aware_boundary_scaling_residual_scale=0.22" \
        "++opt.boundary_signed_grow_loss_scale=0.24" \
        "++opt.boundary_signed_shrink_loss_scale=1.08" \
        "++opt.lambda_boundary_scaling_residual_reg=0.0024" \
        "++opt.lambda_boundary_scaling_residual_smooth=0.0024" \
    ...
    v239b_v233d_ultra_mild_geom_short)
      train_branch "$name" "$gpu" "$BASE_V233D_EXP" "$BASE_V233D_CKPT" "$BASE_V233D_ITER" 1000 "500,1000" true true \
        "opt.position_lr_init=0.0000012" \
        "opt.position_lr_final=0.00000025" \
        "opt.scaling_lr=0.0000030" \
        "++opt.boundary_aware_xyz_scale=0.018" \
        "++opt.boundary_aware_scaling_scale=0.070" \
        "++opt.boundary_aware_opacity_scale=0.18" \
        "++opt.boundary_aware_boundary_scaling_residual_scale=0.28" \
        "++opt.boundary_signed_grow_loss_scale=0.30" \
        "++opt.boundary_signed_shrink_loss_scale=1.04" \
        "++opt.lambda_boundary_scaling_residual_reg=0.0021" \
        "++opt.lambda_boundary_scaling_residual_smooth=0.0021" \
        "opt.percent_dense=0.0" \
        "opt.densify_until_iter=0" \
        "opt.densify_from_iter=1000000" \
        "opt.opacity_reset_interval=1000000" \
    ...
    v240a_boundary_clone_tiny_once)
      local branch_iterations="${V240A_ITERATIONS:-1000}"
      local branch_checkpoint_steps="${V240A_CHECKPOINT_STEPS:-500,1000}"
      local densify_from=$((BASE_V233D_ITER + ${V240A_DENSIFY_FROM_OFFSET:-340}))
      local densify_until=$((BASE_V233D_ITER + ${V240A_DENSIFY_UNTIL_OFFSET:-540}))
      train_branch "$name" "$gpu" "$BASE_V233D_EXP" "$BASE_V233D_CKPT" "$BASE_V233D_ITER" "$branch_iterations" "$branch_checkpoint_steps" false false \
        "opt.position_lr_init=0.0000010" \
        "opt.position_lr_final=0.00000025" \
        "opt.scaling_lr=0.0000030" \
        "++opt.boundary_aware_xyz_scale=0.016" \
        "++opt.boundary_aware_scaling_scale=0.060" \
        "++opt.boundary_aware_opacity_scale=0.16" \
        "++opt.boundary_aware_boundary_scaling_residual_scale=0.24" \
        "++opt.boundary_signed_grow_loss_scale=0.32" \
        "++opt.boundary_signed_shrink_loss_scale=1.08" \
        "++opt.lambda_boundary_scaling_residual_reg=0.0023" \
        "++opt.lambda_boundary_scaling_residual_smooth=0.0023" \
        "opt.percent_dense=0.050" \
        "opt.densification_interval=200" \
        "opt.densify_from_iter=$densify_from" \
        "opt.densify_until_iter=$densify_until" \
        "opt.densify_grad_threshold=0.00090" \
        ...
        "++model.gaussian.binding_densify_strict_candidate_max_points_per_lineage=6" \
        "++model.gaussian.binding_densify_strict_candidate_max_points=72" \
        "++model.gaussian.binding_densify_strict_candidate_max_ratio=0.0016" \
        ...
        "++model.gaussian.binding_densify_arm_only=false" \
        ...
        "++model.gaussian.binding_densify_clone_candidate_seed_boundary_threshold=0.10" \
        "++model.gaussian.binding_densify_clone_candidate_seed_max_points_per_lineage=3" \
        "++model.gaussian.binding_densify_clone_candidate_seed_max_points=24" \
        "++model.gaussian.binding_densify_clone_candidate_seed_max_ratio=0.0006" \
        "++model.gaussian.binding_densify_clone_candidate_seed_min_risk_score=0.12" \
        "++model.gaussian.binding_densify_child_attenuate_enable=true" \
        "++model.gaussian.binding_densify_child_opacity_factor=0.40" \
        "++model.gaussian.binding_densify_child_scale_factor=0.72" \
        "++model.gaussian.binding_densify_child_arm_only=false" \
        "++model.gaussian.binding_densify_child_boundary_threshold=0.10" \
        ...
        "++model.gaussian.binding_densify_child_immediate_refresh_boundary_threshold=0.10" \
        "++model.gaussian.binding_densify_child_immediate_refresh_max_points_per_lineage=4" \
        "++model.gaussian.binding_densify_child_immediate_refresh_max_points=32" \
        "++model.gaussian.binding_densify_child_immediate_refresh_max_ratio=0.0010" \
        "++model.gaussian.binding_densify_child_immediate_refresh_min_score=0.12" \
        "++model.gaussian.binding_densify_debug_verbose=true" \
        "opt.grad_clip=0.0027"
      ;;
    *)
