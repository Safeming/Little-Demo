...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-v300_polar_covariance_stabilizer_ab_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
...
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v300_polar_covariance_stabilizer_ab_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v300_polar_covariance_stabilizer_ab_${RUN_ID}}"
...
printf 'variant\tcov_mode\tpolar_aniso\tscreen_enable\tscreen_shrink\tscreen_grow\tguard_enable\tguard_mode\tguard_strength\tboundary_min\tlayer_ids\tregion_ids\tjoint_ids\tthin_min\tsurface_min\tsurface_max\tmax_points\tguard_aniso\trender_exp\n' > "$PARAMS"
...
Goal:
  v300 targets the explicit-binding root cause instead of point/support loss
  tuning. It keeps the explicit-bound centers, but stabilizes Gaussian
  covariance footprint by removing shear/anomalous scale from the non-orthogonal
  binding rotation_precomp. Two families are tested:
    1. residual-free polar covariance stabilization from binding state only;
    2. the same stabilizer under the current v281 screen actuator baseline.

Gate:
  Raw render contour/residual gate only. This script intentionally does not
  start short training; if the no-train geometry gate fails, training would only
  repeat the v296-v299 failure mode.
EOF
...
render_raw() {
  ...
  local cov_mode="$2"
  local polar_aniso="$3"
  local screen_enable="$4"
  local screen_shrink="$5"
  local screen_grow="$6"
  local guard_enable="$7"
  local guard_mode="$8"
  local guard_strength="$9"
  local boundary_min="${10}"
  local layer_ids="${11}"
  local region_ids="${12}"
  local joint_ids="${13}"
  local thin_min="${14}"
  local surface_min="${15}"
  local surface_max="${16}"
  local max_points="${17}"
  local guard_aniso="${18}"
  local render_exp="${19}"
  local hydra_dir="${20}"
  ...
  env "${COMMON_ENV[@]}" "$PYTHON_BIN" render.py \
    ...
    "++pipeline.covariance_polar_anisotropy_clamp=$polar_aniso" \
    "++pipeline.covariance_polar_det_min=0.0" \
    "++pipeline.covariance_polar_det_max=0.0" \
    "++pipeline.covariance_polar_det_power=1.0" \
    ...
    "++pipeline.binding_covariance_guard_enable=$guard_enable" \
    "++pipeline.binding_covariance_guard_mode=$guard_mode" \
    "++pipeline.binding_covariance_guard_strength=$guard_strength" \
    ...
    "++pipeline.binding_covariance_guard_power=1.0" \
    "++pipeline.binding_covariance_guard_max_points=$max_points" \
    "++pipeline.binding_covariance_guard_anisotropy_clamp=$guard_aniso" \
...
run_variant() {
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$variant" "$cov_mode" "$polar_aniso" "$screen_enable" "$screen_shrink" "$screen_grow" \
    "$guard_enable" "$guard_mode" "$guard_strength" "$boundary_min" "$layer_ids" "$region_ids" \
    "$joint_ids" "$thin_min" "$surface_min" "$surface_max" "$max_points" "$guard_aniso" "$render_exp" \
    >> "$PARAMS"

  log_event "render_start" "$variant"
  render_raw "$variant" "$cov_mode" "$polar_aniso" "$screen_enable" "$screen_shrink" "$screen_grow" \
    "$guard_enable" "$guard_mode" "$guard_strength" "$boundary_min" "$layer_ids" "$region_ids" \
    "$joint_ids" "$thin_min" "$surface_min" "$surface_max" "$max_points" "$guard_aniso" \
...
run_variant baseline_raw default 1.25 false 1.000 1.000 false none 0.00 0.08 soft,free cloth,soft "" "" "" "" 0 1.25
run_variant baseline_v281_screen_mid default 1.25 true 0.940 1.025 false none 0.00 0.08 soft,free cloth,soft "" "" "" "" 0 1.25
run_variant global_polar_det polar_det 1.25 false 1.000 1.000 false none 0.00 0.08 soft,free cloth,soft "" "" "" "" 0 1.25
run_variant global_polar_svd_110 polar_svd_clamp 1.10 false 1.000 1.000 false none 0.00 0.08 soft,free cloth,soft "" "" "" "" 0 1.25
run_variant global_polar_svd_125 polar_svd_clamp 1.25 false 1.000 1.000 false none 0.00 0.08 soft,free cloth,soft "" "" "" "" 0 1.25
run_variant local_polar_det_20 default 1.25 false 1.000 1.000 true polar_det 0.20 0.08 soft,free cloth,soft "" "" "" "" 512 1.25
run_variant local_polar_det_35 default 1.25 false 1.000 1.000 true polar_det 0.35 0.08 soft,free cloth,soft "" "" "" "" 512 1.25
run_variant local_polar_svd_35 default 1.25 false 1.000 1.000 true polar_svd_clamp 0.35 0.08 soft,free cloth,soft "" "" "" "" 512 1.12
run_variant screen_global_polar_det polar_det 1.25 true 0.940 1.025 false none 0.00 0.08 soft,free cloth,soft "" "" "" "" 0 1.25
run_variant screen_local_polar_det_20 default 1.25 true 0.940 1.025 true polar_det 0.20 0.08 soft,free cloth,soft "" "" "" "" 512 1.25
run_variant screen_local_polar_svd_25 default 1.25 true 0.940 1.025 true polar_svd_clamp 0.25 0.08 soft,free cloth,soft "" "" "" "" 512 1.12
...
header = [
    ...
    "guard_enable", "guard_mode", "guard_strength", "boundary_min", "layer_ids", "region_ids",
    "joint_ids", "thin_min", "surface_min", "surface_max", "max_points", "guard_aniso",
    "render_exp", "fg", "boundary", "edge", "inner", "outer", "hard",
...
with summary_path.open("w", encoding="utf-8", newline="") as handle:
    ...
    for row in rows:
        ...
        writer.writerow([
            row["variant"], row["cov_mode"], row["polar_aniso"], row["screen_enable"],
            row["screen_shrink"], row["screen_grow"], row["guard_enable"], row["guard_mode"],
            row["guard_strength"], row["boundary_min"], row["layer_ids"], row["region_ids"],
            row["joint_ids"], row["thin_min"], row["surface_min"], row["surface_max"],
            row["max_points"], row["guard_aniso"], row["render_exp"],
