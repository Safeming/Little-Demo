...
RUN_ID="${RUN_ID:-v350_paired_rowlocal_component_3d_asset_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v350_paired_rowlocal_component_3d_asset_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v350_paired_rowlocal_component_3d_asset_${RUN_ID}}"
...
V350_ASSET_JSON="$ASSET_DIR/v350_paired_rowlocal_component_3d_asset.json"
V350_ROW_GUARD_JSON="$ASSET_DIR/v350_paired_rowlocal_component_3d_row_guard_upper_bound.json"
V350_CANDIDATES_TSV="$ASSET_DIR/v350_paired_rowlocal_component_3d_candidates.tsv"
...
log_event "make_asset_start" "$V350_ASSET_JSON"
"$PYTHON_BIN" tools/make_377_stageB_v349_paired_component_3d_asset.py \
  ...
  --out-json "$V350_ASSET_JSON" \
  --out-row-guard-json "$V350_ROW_GUARD_JSON" \
  --out-candidates-tsv "$V350_CANDIDATES_TSV" \
  "${MAKE_ARGS[@]}" \
  > "$LOG_DIR/make_v350_asset.log" 2>&1
log_event "make_asset_done" "$V350_ASSET_JSON"
...
run_window() {
  ...
  render_and_analyze "$window" "$views_spec" "$frames_spec" v350_paired_rowlocal_component_3d "$CANDIDATE_CKPT" "$window_root/v350_paired_rowlocal_component_3d" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_signed_dynamic_component_local_asset_json=$V350_ASSET_JSON"

  render_and_analyze "$window" "$views_spec" "$frames_spec" v350_paired_rowlocal_component_3d_plus_v345 "$CANDIDATE_CKPT" "$window_root/v350_paired_rowlocal_component_3d_plus_v345" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_signed_point_screen_actuator_drop_images='$POINT_SCREEN_DROP_IMAGES'" \
    "++explicit_binding_adopted_signed_dynamic_component_local_asset_json=$V350_ASSET_JSON"

  render_and_analyze "$window" "$views_spec" "$frames_spec" v350_row_guard_upper_bound_plus_v345 "$CANDIDATE_CKPT" "$window_root/v350_row_guard_upper_bound_plus_v345" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_signed_point_screen_actuator_drop_images='$POINT_SCREEN_DROP_IMAGES'" \
    "++explicit_binding_adopted_signed_dynamic_component_row_guard_json=$V350_ROW_GUARD_JSON"
}
...
variants = [
    ...
    "formal_v338_current",
    "v345_screen_guard",
    "v350_paired_rowlocal_component_3d",
    "v350_paired_rowlocal_component_3d_plus_v345",
    "v350_row_guard_upper_bound_plus_v345",
]
...
echo "V350_ASSET_JSON=$V350_ASSET_JSON"
echo "V350_ROW_GUARD_JSON=$V350_ROW_GUARD_JSON"
echo "V350_CANDIDATES_TSV=$V350_CANDIDATES_TSV"
echo
echo "Dense gate command only if sentinel probe passes:"
echo "CANDIDATE_CKPT=$CANDIDATE_CKPT \\"
echo "CANDIDATE_VARIANT_NAME=candidate_v350_paired_rowlocal_component_3d_plus_v345 \\"
echo "CANDIDATE_SIGNED_POINT_SCREEN_ACTUATOR_DROP_IMAGES='$POINT_SCREEN_DROP_IMAGES' \\"
echo "CANDIDATE_SIGNED_DYNAMIC_COMPONENT_LOCAL_ASSET_JSON=$V350_ASSET_JSON \\"
echo "TEST_VIEWS_SPEC='[21,22,23]' TEST_FRAMES_SPEC='[0,570,1]' \\"
echo "RUN_ID='formal_v350_paired_rowlocal_component_3d_dense_gate_$(TZ=Asia/Shanghai date '+%Y%m%d')' \\"
