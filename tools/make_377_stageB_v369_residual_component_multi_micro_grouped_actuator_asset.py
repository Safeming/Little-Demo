...
def _inner_residual_targets_for_component(
    *,
    render_exp: Path,
    dataset_root: Path,
    subject: str,
    ...
    gate = _component_gate_mask(rec, inner.shape, pad_px=float(gate_pad_px))
    n, labels, stats, centroids = cv2.connectedComponentsWithStats(inner.astype(np.uint8), 8)
    targets = []
    for idx in range(1, n):
        area = int(stats[idx, cv2.CC_STAT_AREA])
        ...
        comp = labels == idx
        overlap = int((comp & gate).sum())
        if overlap < int(min_gate_overlap):
            continue
        cx, cy = centroids[idx]
        dist = math.sqrt(
            (float(cx) - float(rec.get("centroid_x", 0.0) or 0.0)) ** 2
            + (float(cy) - float(rec.get("centroid_y", 0.0) or 0.0)) ** 2
        )
        ...
        score = overlap * 16.0 + min(area, 600) - 1.5 * dist
        targets.append((
            float(score),
            comp,
            {
                "residual_target_index": int(idx),
                "residual_target_score": float(score),
                "residual_mask_pixels": int(area),
                "residual_mask_overlap_pixels": int(overlap),
                "residual_mask_cx": float(cx),
                "residual_mask_cy": float(cy),
                "residual_mask_bbox_x": float(x),
                "residual_mask_bbox_y": float(y),
                "residual_mask_bbox_w": float(w),
                "residual_mask_bbox_h": float(h),
                "residual_mask_component_dist_px": float(dist),
            },
        ))
    ...
    out = []
    for out_index, (_score, mask, stats_row) in enumerate(targets):
        stats_row = dict(stats_row)
        stats_row["residual_target_rank"] = int(out_index)
        out.append((mask, stats_row))
    return out
...
def _make_outer_action(
    *,
    ...
    action = {
        "component_key": f"v369_outer:{_safe_key(source_key)}:a{action_index}",
        ...
        "reason": "v369_residual_grouped_outer_semantic_local_3d_protect",
        "v369_source_bbox_x": float(outer.get("bbox_x", 0.0) or 0.0),
        "v369_source_bbox_y": float(outer.get("bbox_y", 0.0) or 0.0),
        "v369_source_bbox_w": float(outer.get("bbox_w", 0.0) or 0.0),
        "v369_source_bbox_h": float(outer.get("bbox_h", 0.0) or 0.0),
        "v369_source_centroid_x": float(outer.get("centroid_x", 0.0) or 0.0),
        "v369_source_centroid_y": float(outer.get("centroid_y", 0.0) or 0.0),
        **support,
        **outer_owner,
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build v369 residual-component multi-target multi-micro grouped actuator asset."
    )
    ...
    parser.add_argument("--max-groups-per-image", default=0, type=int)
    parser.add_argument("--max-groups-per-source", default=0, type=int)
    parser.add_argument("--max-groups-per-target", default=0, type=int)
    ...
    parser.add_argument("--residual-targets-per-inner", default=3, type=int)
    parser.add_argument("--residual-min-gate-overlap", default=1, type=int)
    parser.add_argument("--residual-gate-pad-px", default=18.0, type=float)
    ...
    image_group_counts: Counter[str] = Counter()
    ...
    for frame in frames:
        ...
        for inner in inner_rows:
            ...
            if not outer_selected:
                continue
            residual_targets: list[tuple[np.ndarray | None, dict[str, float]]] = []
            if bool(args.residual_mask_enable):
                residual_targets = _inner_residual_targets_for_component(
                    render_exp=args.current_render_exp,
                    ...
                    max_targets=int(args.residual_targets_per_inner),
                    min_gate_overlap=int(args.residual_min_gate_overlap),
                    gate_pad_px=float(args.residual_gate_pad_px),
                )
            if not residual_targets:
                residual_targets = [(None, {"residual_target_rank": 0, "residual_target_index": -1})]

            source_key = _csv_source_key(inner)
            for residual_mask, residual_stats in residual_targets:
                target_rank = int(residual_stats.get("residual_target_rank", 0) or 0)
                target_suffix = f"t{target_rank}"
                target_key = f"{source_key}:target{target_rank}"
                            ...
                            for minor_scale in minor_scales:
                                ...
                                for covariance_scale in covariance_scales:
                                    if int(args.max_groups) > 0 and len(groups) >= int(args.max_groups):
                                        break
                                    if (
                                        int(args.max_groups_per_image) > 0
                                        and image_group_counts[image_name] >= int(args.max_groups_per_image)
                                    ):
                                        continue
                                    if (
                                        int(args.max_groups_per_source) > 0
                                        and source_group_counts[source_key] >= int(args.max_groups_per_source)
                                    ):
                                        continue
                                    if (
                                        int(args.max_groups_per_target) > 0
                                        and target_group_counts[target_key] >= int(args.max_groups_per_target)
                                    ):
                                        continue
                                    group_index = len(groups)
                                    mode = (
                                        f"{target_suffix}_n{count}_r{radius_scale:g}_m{minor_scale:g}_"
                                        f"d{depth_scale:g}_cg{covariance_scale:g}"
                                    ).replace(".", "p")
                                    pair_id = f"v369:{source_key}:{mode}:g{group_index}"
                                    group_children = []
                                    for spec in specs:
                                        ...
                                        child = {
                                        "component_key": (
                                            f"v369_inner:{_safe_key(source_key)}:{mode}:"
                                        ...
                                        "reason": "v369_self_protected_residual_grouped_inner_micro_child",
                                        "v369_mode": mode,
                                        "v369_micro_count": int(len(specs)),
                                        "v369_micro_index": int(spec["micro_index"]),
                                        "v369_screen_x": float(target_u),
                                        "v369_screen_y": float(target_v),
                                        "v369_anchor_screen_x": float(anchor_u),
                                        "v369_anchor_screen_y": float(anchor_v),
                                        "v369_anchor_depth": float(anchor_depth),
                                        "v369_depth": float(depth),
                                        "v369_renderer_space_cache": bool(image_name in renderer_space_cache),
                                        "v369_source_bbox_x": float(inner.get("bbox_x", 0.0) or 0.0),
                                        "v369_source_bbox_y": float(inner.get("bbox_y", 0.0) or 0.0),
                                        "v369_source_bbox_w": float(inner.get("bbox_w", 0.0) or 0.0),
                                        "v369_source_bbox_h": float(inner.get("bbox_h", 0.0) or 0.0),
                                        "v369_source_centroid_x": float(inner.get("centroid_x", 0.0) or 0.0),
                                        "v369_source_centroid_y": float(inner.get("centroid_y", 0.0) or 0.0),
                                        "v369_residual_mask_enable": bool(residual_mask is not None),
                                        **{f"v369_{key}": value for key, value in residual_stats.items()},
                                        **inner_owner,
                                    }
                                ...
                                groups.append(group)
                                image_group_counts[image_name] += 1
                                source_group_counts[source_key] += 1
                                target_group_counts[target_key] += 1
    ...
    payload = {
        "version": "v369_residual_component_multi_micro_grouped_actuator_asset",
        "policy": (
            "Subject-general residual discovery: current raw diagnostics select multiple residual targets "
            "per inner component row; each target becomes an action-level semantic-local-frame multi-micro "
            "child group, and paired outer components become semantic local-3D protect/shrink actions. "
            "All pairs still require action-level raw contour validation before training."
        ),
        "source": {
        ...
        "thresholds": {
            ...
            "max_groups_per_image": int(args.max_groups_per_image),
            "max_groups_per_source": int(args.max_groups_per_source),
            "max_groups_per_target": int(args.max_groups_per_target),
            ...
            "residual_targets_per_inner": int(args.residual_targets_per_inner),
            "residual_min_gate_overlap": int(args.residual_min_gate_overlap),
            "residual_gate_pad_px": float(args.residual_gate_pad_px),
            "child_self_protect_enable": True,
            "child_self_protect_mode": "paired_outer_screen_ellipse",
            "child_self_protect_outer_max_overlap": 0.0,
            "child_self_protect_inner_radius_fraction": float(args.self_protect_inner_radius_fraction),
