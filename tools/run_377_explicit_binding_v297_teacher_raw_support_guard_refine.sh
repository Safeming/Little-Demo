...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-v297_teacher_raw_support_guard_refine_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
...
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v297_teacher_raw_support_guard_refine_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v297_teacher_raw_support_guard_refine_${RUN_ID}}"
...
Goal:
  v297 fixes the actual texture-only failure mode seen in v296. Geometry,
  opacity, scale, rotation, pose and camera remain frozen, but the loss now
  distills the teacher/base render's raw luma/chroma support in the boundary
  band and outer shell. This directly protects the metric-visible silhouette
  support instead of relying on ordinary RGB L1.
