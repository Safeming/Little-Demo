...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-v332_formal_signed_geometry_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
...
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v332_formal_signed_geometry_render_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v332_formal_signed_geometry_render_${RUN_ID}}"
...

{
  ...
  echo
  echo "Goal:"
  echo "  v332 validates the formal no-train signed geometry inference preset:"
  echo "    ++explicit_binding_render_preset=v320_v307_signed_geometry"
  echo "  The preset resolves the v320 selected component CSV and v304 point CSV"
  echo "  inside render.py, so downstream render/export does not depend on the"
  echo "  old experiment wrapper to inject the adopted geometry settings."
...
print(json.dumps({"summary": str(summary_path), "formal_status": list(csv.DictReader(summary_path.open(encoding="utf-8"), delimiter="\t"))[-1]["status"]}, indent=2))
PY
