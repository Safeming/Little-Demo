...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-v285_opacity_gated_rgb_refine_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
CPU_THREADS_PER_JOB="${CPU_THREADS_PER_JOB:-6}"
TRAIN_ITERS="${TRAIN_ITERS:-200}"
TRAIN_CHECKPOINT_STEPS="${TRAIN_CHECKPOINT_STEPS:-80,160,200}"
...
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v285_opacity_gated_rgb_refine_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v285_opacity_gated_rgb_refine_${RUN_ID}}"
...
Goal:
  v285 follows v283 evidence: most RGB inner missing already has opacity coverage.
  Freeze old xyz/base opacity/base scale/rotation/boundary residuals and train only color/SH/texture.
  The new opacity-gated RGB loss repairs pixels where opacity covers GT but RGB support is weak,
  and weakly suppresses RGB outer leaks. Raw contour gate decides whether any checkpoint is usable.
EOF
...
train_v285() {
  local train_exp="$EXP_ROOT/train_opacity_gated_rgb"
  ...
  env "${COMMON_ENV[@]}" "$PYTHON_BIN" train.py \
    ...
    "exp_dir=$train_exp" \
    "hydra.run.dir=$HYDRA_RUN_ROOT/train_opacity_gated_rgb" \
    "seed=-1" \
    "wandb_disable=true" \
    ...
    "opt.lambda_l1=0.004" \
    "opt.lambda_l1_fg=0.018" \
    "opt.lambda_l1_boundary=0.010" \
    "opt.lambda_perceptual=0.0" \
    "++opt.lambda_opacity_gated_rgb_inner=0.240" \
    "++opt.lambda_opacity_gated_rgb_outer=0.060" \
    "++opt.opacity_gated_rgb_support_threshold=0.025" \
    "++opt.opacity_gated_rgb_support_close_kernel=5" \
    "++opt.opacity_gated_rgb_opacity_threshold=0.06" \
    "++opt.opacity_gated_rgb_inner_band_width=9" \
    "++opt.opacity_gated_rgb_outer_start_width=1" \
    "++opt.opacity_gated_rgb_outer_end_width=20" \
    "++opt.opacity_gated_rgb_mask_dilate=0" \
    "++opt.opacity_gated_rgb_min_pixels=8" \
    ...
    > "$LOG_DIR/train_opacity_gated_rgb.log" 2>&1
...
TRAIN_EXP="$EXP_ROOT/train_opacity_gated_rgb"
train_v285
