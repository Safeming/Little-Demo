...
RUN_ID="${RUN_ID:-stageB_v245_boundary_support_projector_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
GPU="${GPU:-0}"
SEED="${SEED:--1}"
...
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_stageB_v245_boundary_support_projector_${RUN_ID}}"
...
run_residual_mining() {
  local out_dir="$LOG_DIR/v245a_residual_mining"
  log_event "$GPU" "v245a_residual_mining" "start" "render_exp=$BASE_RENDER_EXP"
  "$PYTHON_BIN" "$ROOT/tools/analyze_377_boundary_residuals.py" \
    ...
    --panel-width 220 \
    --out-dir "$out_dir" \
    > "$LOG_DIR/v245a_residual_mining.log" 2>&1
  local status=$?
  if [ "$status" -eq 0 ]; then
    log_event "$GPU" "v245a_residual_mining" "done" "$out_dir"
    summary_row "v245a_residual_mining" "diagnostic" "" "$BASE_RENDER_EXP" "" "ok" "$out_dir"
  else
    log_event "$GPU" "v245a_residual_mining" "failed" "status=$status"
    summary_row "v245a_residual_mining" "diagnostic" "" "$BASE_RENDER_EXP" "" "failed" "status=$status"
  fi
  return "$status"
}
...
render_checkpoint() {
  ...
  local render_exp="${exp_dir}_render_v245_ckpt${global_iter}"
  if [ ! -f "$ckpt" ]; then
...
train_v245b() {
  local name="v245b_boundary_support_projector"
  local iterations="${V245_ITERATIONS:-360}"
  local checkpoint_steps="${V245_CHECKPOINT_STEPS:-120,240,360}"
  ...
  common_env "$PYTHON_BIN" "$ROOT/train.py" \
    ...
    "opt.densify_until_iter=$((BASE_ITER + 250))" \
    ...
    "++model.gaussian.boundary_support_projector_inner_max_points=72" \
    "++model.gaussian.boundary_support_projector_inner_max_ratio=0.0016" \
    "++model.gaussian.boundary_support_projector_inner_max_points_per_lineage=2" \
    "++model.gaussian.boundary_support_projector_outer_max_points=72" \
    "++model.gaussian.boundary_support_projector_outer_max_ratio=0.0016" \
    ...
    "++model.gaussian.boundary_support_projector_offset_scale=0.32" \
...

{
  ...
  echo "PURPOSE=v245 boundary support projector: under-score support clones + over-score contributor shrink"
