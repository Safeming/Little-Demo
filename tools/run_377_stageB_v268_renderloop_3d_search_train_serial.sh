...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-stageB_v268_renderloop_3d_search_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_stageB_v268_renderloop_3d_search_${RUN_ID}}"
mkdir -p "$LOG_DIR"
...
TRAIN_NAME="${TRAIN_NAME:-v268a_renderloop_3d_search}"
...
VARIANT_SCORE_JSON="$LOG_DIR/v268_variant_scores.json"
BEST_INFO_JSON="$LOG_DIR/v268_best_variant.json"
FINAL_GATE_JSON="$LOG_DIR/v268_final_strong_gate.json"
SEARCH_CANDIDATE_DIR="$LOG_DIR/v268_shifted_3d_candidates"
SEARCH_MAX_CANDIDATES="${SEARCH_MAX_CANDIDATES:-12}"
...
run_append_variant() {
  ...
  local render_exp="${append_exp}_render_v268_ckpt${APPEND_ITER}"
  local candidate_csv="$SEARCH_CANDIDATE_DIR/${variant}_candidates.csv"
  local candidate_summary="$SEARCH_CANDIDATE_DIR/${variant}_candidates_summary.json"
  wait_for_gpu "$variant"
  mkdir -p "$SEARCH_CANDIDATE_DIR"
  log_event "$variant" "candidate_search_start" "max=${max_candidates} shift_px=${shift_px} shift_py=${shift_py} shift_ray=${shift_ray}"
  common_env "$PYTHON_BIN" "$ROOT/tools/make_377_stageB_v268_shifted_3d_candidates.py" \
    --config-path "$BASE_EXP/.hydra/config.yaml" \
    --load-ckpt "$BASE_CKPT" \
    --source-csv "$SOURCE_CANDIDATES_CSV" \
    --out-csv "$candidate_csv" \
    --out-summary "$candidate_summary" \
    --dataset-root "$DATA_ROOT" \
    --parser-root "$PARSER_ROOT" \
    --compact-mapping "$COMPACT_MAPPING" \
    --max-candidates "$max_candidates" \
    --pixel-x-offset "$shift_px" \
    --pixel-y-offset "$shift_py" \
    --ray-offset-world "$shift_ray" \
    --tag "$variant" \
    > "$LOG_DIR/${variant}_candidate_search.log" 2>&1
  local candidate_status=$?
  if [ "$candidate_status" -ne 0 ]; then
    log_event "$variant" "candidate_search_failed" "status=$candidate_status"
    summary_row "$variant" "candidate_search" "" "" "" "failed" "status=$candidate_status"
...
run_support_only_train() {
  ...
  local final_render_exp="${train_exp}_render_v268_ckpt${FINAL_ITER}"
  mkdir -p "$train_exp"
  ...
  render_ckpt "$train_exp" "$train_exp/ckpt${FINAL_ITER}.pth" "$final_render_exp" "v268_final_render" || true
  "$PYTHON_BIN" "$ROOT/tools/check_377_stageB_v261_do_no_harm.py" \
    ...
    > "$LOG_DIR/v268_final_strong_gate.log" 2>&1 || true
  summary_row "v268_final_strong_gate" "gate" "" "$final_render_exp" "" "$([ -s "$FINAL_GATE_JSON" ] && "$PYTHON_BIN" - "$FINAL_GATE_JSON" <<'PY' || echo unknown
import json, sys
...

{
  ...
  echo "VARIANTS=center,px_left,px_right,px_up,px_down,ray_front,ray_back,diag_lu,diag_rd"
...
run_append_variant "center_top12_cross5" "$SEARCH_MAX_CANDIDATES" cross5 0.0025 0.50 0.016 0.13 0.26 0.0050 0.0 0.0 0.0 || true
run_append_variant "px_left_top12_cross5" "$SEARCH_MAX_CANDIDATES" cross5 0.0025 0.50 0.016 0.13 0.26 0.0050 -1.5 0.0 0.0 || true
run_append_variant "px_right_top12_cross5" "$SEARCH_MAX_CANDIDATES" cross5 0.0025 0.50 0.016 0.13 0.26 0.0050 1.5 0.0 0.0 || true
run_append_variant "px_up_top12_cross5" "$SEARCH_MAX_CANDIDATES" cross5 0.0025 0.50 0.016 0.13 0.26 0.0050 0.0 -1.5 0.0 || true
run_append_variant "px_down_top12_cross5" "$SEARCH_MAX_CANDIDATES" cross5 0.0025 0.50 0.016 0.13 0.26 0.0050 0.0 1.5 0.0 || true
run_append_variant "ray_front_top12_cross5" "$SEARCH_MAX_CANDIDATES" cross5 0.0025 0.50 0.016 0.13 0.26 0.0050 0.0 0.0 -0.0040 || true
run_append_variant "ray_back_top12_cross5" "$SEARCH_MAX_CANDIDATES" cross5 0.0025 0.50 0.016 0.13 0.26 0.0050 0.0 0.0 0.0040 || true
run_append_variant "diag_lu_top12_cross5" "$SEARCH_MAX_CANDIDATES" cross5 0.0025 0.50 0.016 0.13 0.26 0.0050 -1.0 -1.0 0.0 || true
run_append_variant "diag_rd_top12_cross5" "$SEARCH_MAX_CANDIDATES" cross5 0.0025 0.50 0.016 0.13 0.26 0.0050 1.0 1.0 0.0 || true
...
log_event "v268_select" "best" "variant=$BEST_VARIANT append_exp=$BEST_APPEND_EXP"
summary_row "v268_select" "select" "$BEST_APPEND_EXP" "$(json_get "$BEST_INFO_JSON" best.render_exp)" "$(json_get "$BEST_INFO_JSON" best.ckpt)" "ok" "$BEST_INFO_JSON"
