...
START_CKPT="$ROOT/exp/stageA2/377_multiview_explicit_hq_rootfix_resume_v177a_v176a_sameframe_k4_hfsource_screen4000/best_ckpt.pth"
START_SCRIPT="$ROOT/tools/start_377_stageA2_multiview_explicit_rootfix_resume_v169a_v168a_hardcontour_boundaryactuator.sh"

mkdir -p "$LOG_DIR"

BATCH_ID="${V178_RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
SUMMARY="$LOG_DIR/v178_batch_${BATCH_ID}_summary.tsv"
REGISTRY="$LOG_DIR/v178_batch_${BATCH_ID}_runs.txt"
RUNNER_LOG="$LOG_DIR/v178_batch_${BATCH_ID}.log"

printf '%s\n' "$RUNNER_LOG" > "$LOG_DIR/v178_latest_runner_log.txt"
printf '%s\n' "$SUMMARY" > "$LOG_DIR/v178_latest_summary.txt"
printf '%s\n' "$BATCH_ID" > "$LOG_DIR/v178_latest_run_id.txt"
...
run_render() {
  ...
  printf '%s\n' "$render_log" > "$LOG_DIR/v178_latest_render_log.txt"
...
run_one() {
  ...
  local exp_dir="$ROOT/exp/stageA2/377_multiview_explicit_hq_rootfix_resume_${tag}_v177a_${slug}_screen${iterations}"
  local train_log="$LOG_DIR/${tag}_${BATCH_ID}_${slug}.log"

  printf '%s\t%s\t%s\t%s\n' "$tag" "$exp_dir" "$train_log" "$option_name" >> "$REGISTRY"
  printf '%s\n' "$exp_dir" > "$LOG_DIR/v178_latest_exp.txt"
  printf '%s\n' "$train_log" > "$LOG_DIR/v178_latest_log.txt"
...
echo "[$(date '+%F %T')] v178 9h batch started"
echo "summary=$SUMMARY"
echo "registry=$REGISTRY"
echo "start_ckpt=$START_CKPT"

run_one "v178a" 3000 "stageA_377_multiview_explicit_hq_v178a_v177a_k4_lowlr_stabilize_v1" "k4_lowlr_stabilize"
run_one "v178b" 3000 "stageA_377_multiview_explicit_hq_v178b_v177a_k4_camquality_focus_v1" "k4_camquality_focus"
run_one "v178c" 3000 "stageA_377_multiview_explicit_hq_v178c_v177a_k4_photo_consistency_v1" "k4_photo_consistency"
run_one "v178d" 3000 "stageA_377_multiview_explicit_hq_v178d_v177a_k4_contour_uncertainty_v1" "k4_contour_uncertainty"
run_one "v178e" 3000 "stageA_377_multiview_explicit_hq_v178e_v177a_k6_probe_v1" "k6_probe"

echo "[$(date '+%F %T')] v178 9h batch finished"
cat "$SUMMARY"
