...
RUN_ID="${RUN_ID:-v387_runtime_bounded_marginal_selector_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v387_runtime_bounded_marginal_selector_${RUN_ID}}"
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v387_runtime_bounded_marginal_selector_${RUN_ID}}"
...
V387_CLOSURE_ASSET_JSON="$ASSET_DIR/v387_runtime_bounded_candidate_asset.json"
LAUNCH_LOG="$LOG_DIR/v387_launcher.log"

mkdir -p "$LOG_DIR" "$EXP_ROOT" "$ASSET_DIR"
START_BJT="$(TZ=Asia/Shanghai date '+%F %T BJT')"
EST_END_BJT="$(TZ=Asia/Shanghai date -d '+8 hours 30 minutes' '+%F %T BJT')"
...
V387_CLOSURE_ASSET_JSON=$V387_CLOSURE_ASSET_JSON
...
"$PYTHON_BIN" - "$SOURCE_CLOSURE_ASSET_JSON" "$V387_CLOSURE_ASSET_JSON" <<'PY' > "$LOG_DIR/make_v387_asset.log" 2>&1
import json
import sys
...
asset["version"] = "v387_runtime_bounded_candidate_asset"
asset["policy"] = "v382 closure candidates prepared for runtime-bounded marginal selector. Per-action image allowlists are injected by the selector."
asset["v387_runtime_contract"] = {
...
env -u LOG_DIR -u EXP_ROOT -u HYDRA_RUN_ROOT \
  ...
  RUN_ID="v387_selector_${RUN_ID}" \
  BASE_ASSET_JSON="$BASE_ASSET_JSON" \
  CLOSURE_ASSET_JSON="$V387_CLOSURE_ASSET_JSON" \
  PREFILTER_TSV="$PREFILTER_TSV" \
  CANARY_WORST_TSV="$CANARY_WORST_TSV" \
  CANARY_TOP_K="$CANARY_TOP_K" \
  MAX_CANDIDATES="$MAX_CANDIDATES" \
  ...
  "$PYTHON_BIN" tools/run_377_explicit_binding_v387_runtime_bounded_marginal_selector.py \
  --log-dir "$LOG_DIR/v387_selector" \
  --exp-root "$EXP_ROOT/v387_selector" \
  > "$LAUNCH_LOG" 2>&1

END_BJT="$(TZ=Asia/Shanghai date '+%F %T BJT')"
echo "LOG_DIR=$LOG_DIR"
echo "EXP_ROOT=$EXP_ROOT"
echo "V387_CLOSURE_ASSET_JSON=$V387_CLOSURE_ASSET_JSON"
