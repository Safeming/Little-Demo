...
EXP_DIR="${1:-$ROOT/exp/stageA2/377_multiview_explicit_hq_rootfix_resume_v187a_v179c_heldout_camera_geometry_probe_$RUN_ID}"
...
exec env PYTHONUNBUFFERED=1 "$PYTHON_BIN" train.py \
  ...
  "++opt.camera_geometry_strength=[0.20,1,0.55,300,0.85,900,1.0]" \
  "++opt.camera_geometry_rot_max_deg=0.45" \
  "++opt.camera_geometry_trans_max=0.012" \
  "++opt.camera_geometry_rot_reg_weight=1.0" \
  "++opt.camera_geometry_trans_reg_weight=1.0" \
  "++opt.camera_geometry_lr=5.0e-04" \
  "++opt.lambda_camera_geometry_reg=0.006" \
  "++opt.boundary_opacity_residual_lr=0.0" \
  "++opt.boundary_scaling_residual_lr=0.0" \
  "++opt.latent_weight_decay=0.0" \
  "opt.lambda_l1=0.10" \
  "++opt.lambda_l1_fg=0.55" \
  "opt.lambda_perceptual=0.0" \
  "opt.lambda_dssim=0.0" \
  "++opt.lambda_l1_boundary=0.85" \
  "opt.lambda_mask=0.0" \
  "++opt.lambda_mask_boundary=0.045" \
  "++opt.lambda_mask_boundary_hard=0.070" \
  "++opt.lambda_silhouette_outer=0.045" \
  "++opt.lambda_silhouette_outer_shell=0.060" \
  "++opt.lambda_silhouette_inner=0.025" \
  "++opt.silhouette_outer_ring_width=11" \
  "++opt.silhouette_outer_shell_start_width=1" \
  "++opt.silhouette_outer_shell_end_width=23" \
  "++opt.silhouette_inner_ring_width=9" \
  "++opt.boundary_band_width=15" \
