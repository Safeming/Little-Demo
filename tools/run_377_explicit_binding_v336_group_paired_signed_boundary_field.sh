...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-v336_group_paired_signed_boundary_field_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
...
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v336_group_paired_signed_boundary_field_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v336_group_paired_signed_boundary_field_${RUN_ID}}"
...
Goal:
  v336 keeps signed geometry no-train, but moves v335 away from global
  canonical point sets. It builds per-image group-paired shrink/grow fields,
  protects grow/inner points from shrink, and gates fg/boundary/edge/RGB
  residual/opacity residual before adoption.
EOF
...
make_field local_mild \
  --max-shrink-per-image 32 --max-grow-per-image 48 --max-components-per-direction 8 \
  --top-points-per-component 4 --min-component-area 40 --require-paired-image \
  --protect-grow-from-shrink --protect-inner-points 24 --direction-score-margin -1.0 \
...
make_field local_protect \
  --max-shrink-per-image 32 --max-grow-per-image 80 --max-components-per-direction 12 \
  --top-points-per-component 6 --min-component-area 30 --require-paired-image \
  ...
  --shrink-min-boundary 0.12 --shrink-min-visible-hits 30 --shrink-allowed-regions 2

BASELINE_EXP="$EXP_ROOT/baseline_no_preset"
FORMAL_EXP="$EXP_ROOT/formal_v320_v307_signed_geometry"
LOCAL_MILD_EXP="$EXP_ROOT/group_local_mild"
LOCAL_BALANCED_EXP="$EXP_ROOT/group_local_balanced"
LOCAL_PROTECT_EXP="$EXP_ROOT/group_local_protect"
FORMAL_LOCAL_EXP="$EXP_ROOT/formal_plus_group_local_balanced"
...
mapfile -t LOCAL_MILD_ARGS < <(point_args "$FIELD_DIR/local_mild.json" 1.010 0.20)
mapfile -t LOCAL_BALANCED_ARGS < <(point_args "$FIELD_DIR/local_balanced.json" 1.015 0.25)
mapfile -t LOCAL_PROTECT_ARGS < <(point_args "$FIELD_DIR/local_protect.json" 1.020 0.18)
render_variant group_local_mild "$LOCAL_MILD_EXP" "${LOCAL_MILD_ARGS[@]}"
render_variant group_local_balanced "$LOCAL_BALANCED_EXP" "${LOCAL_BALANCED_ARGS[@]}"
render_variant group_local_protect "$LOCAL_PROTECT_EXP" "${LOCAL_PROTECT_ARGS[@]}"
render_variant formal_plus_group_local_balanced "$FORMAL_LOCAL_EXP" \
  "++explicit_binding_adopted_outer_px=0.18" \
  "++explicit_binding_render_preset=v320_v307_signed_geometry" \
  "++pipeline.covariance_signed_point_json=$FIELD_DIR/local_balanced.json" \
  "++pipeline.covariance_signed_point_screen_actuator_enable=true"

"$PYTHON_BIN" - "$SUMMARY" "$WORST_SUMMARY" \
  baseline_no_preset "$BASELINE_EXP" \
  formal_v320_v307_signed_geometry "$FORMAL_EXP" \
  group_local_mild "$LOCAL_MILD_EXP" \
  group_local_balanced "$LOCAL_BALANCED_EXP" \
  group_local_protect "$LOCAL_PROTECT_EXP" \
  formal_plus_group_local_balanced "$FORMAL_LOCAL_EXP" <<'PY'
import csv, json, sys
from pathlib import Path
