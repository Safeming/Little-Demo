...
ROOT="${ROOT:-/remote-home/ming/semantic-editable-avatar-main}"
PYTHON_BIN="${PYTHON_BIN:-/opt/miniconda3/envs/3dgs-avatar/bin/python}"

GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-stageB_v234_opacity_silhouette_refine_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
ITERATIONS="${ITERATIONS:-300}"
DO_RENDER="${DO_RENDER:-1}"
WAIT_FOR_FREE_GPU="${WAIT_FOR_FREE_GPU:-1}"
GPU_MAX_USED_MB_START="${GPU_MAX_USED_MB_START:-4000}"
GPU_MAX_UTIL_START="${GPU_MAX_UTIL_START:-20}"
...
EXP_DIR="${EXP_DIR:-$ROOT/exp/stageB/377_hulk_light_v234a_opacity_silhouette_refine_${RUN_ID}}"
RENDER_EXP="${RENDER_EXP:-${EXP_DIR}_render_opacity_refine_final}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_stageB_v234_opacity_silhouette_refine_${RUN_ID}}"
mkdir -p "$LOG_DIR"
...
if [ "$WAIT_FOR_FREE_GPU" = "1" ]; then
  while true; do
    ...
    if [ "$used" -le "$GPU_MAX_USED_MB_START" ] && [ "$util" -le "$GPU_MAX_UTIL_START" ]; then
      echo "$(TZ=Asia/Shanghai date '+%F %T BJT') gpu$GPU ready used=${used}MiB util=${util}%"
      break
    fi
    echo "$(TZ=Asia/Shanghai date '+%F %T BJT') gpu$GPU wait used=${used}MiB util=${util}% threshold=${GPU_MAX_USED_MB_START}MiB/${GPU_MAX_UTIL_START}%"
...
env "${COMMON_ENV[@]}" "$PYTHON_BIN" "$ROOT/train.py" \
  ...
  "++opt.train_sample_log_interval=50" \
  ...
  "++opt.lambda_silhouette_outer_spike=0.012" \
  ...
  "++opt.boundary_image_error_score_min=0.015" \
  ...
  "++opt.boundary_signed_shrink_loss_scale=1.20" \
  "++opt.boundary_signed_grow_loss_scale=0.22" \
  "++opt.boundary_signed_share_gain=1.00" \
  "++opt.boundary_signed_share_power=1.0" \
  "++opt.boundary_signed_shrink_share_gain=1.15" \
  "++opt.boundary_signed_shrink_share_power=1.20" \
  ...
  "++opt.boundary_aware_boundary_opacity_residual_scale=0.60" \
  "++opt.boundary_aware_boundary_scaling_residual_scale=0.0" \
  "opt.opacity_lr=0.000050" \
  "opt.scaling_lr=0.0" \
...
echo "TRAIN_DONE_BJT=$(TZ=Asia/Shanghai date '+%F %T BJT')"
...
if [ "$DO_RENDER" = "1" ]; then
  ...
  "$PYTHON_BIN" "$ROOT/tools/analyze_377_render_contours.py" \
    ...
    > "$LOG_DIR/contours.log" 2>&1 || true
  echo "RENDER_DONE_BJT=$(TZ=Asia/Shanghai date '+%F %T BJT')"
