...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-v318_component_greedy_selector_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
...
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v318_component_greedy_selector_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v318_component_greedy_selector_${RUN_ID}}"
...
Goal:
  v318 turns the v307 component-local correction into a greedy render-in-loop
  selector. Each component/group is accepted only if the real raw render passes
  the do-no-harm gate against the baseline. No training is started unless the
  selected component set passes the gate.
EOF
...
"$PYTHON_BIN" tools/run_377_explicit_binding_v318_component_greedy_selector.py \
  ...
  --max-candidates "${MAX_CANDIDATES:-12}" \
  --max-accept "${MAX_ACCEPT:-4}" \
  --max-individual-per-direction "${MAX_INDIVIDUAL_PER_DIRECTION:-4}" \
  --max-image-groups "${MAX_IMAGE_GROUPS:-4}" \
