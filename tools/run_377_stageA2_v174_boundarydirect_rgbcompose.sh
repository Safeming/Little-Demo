...
BATCH_ID="${V174_RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
SUMMARY="$LOG_DIR/v174_batch_${BATCH_ID}_summary.tsv"
REGISTRY="$LOG_DIR/v174_batch_${BATCH_ID}_runs.txt"
RUNNER_LOG="$LOG_DIR/v174_batch_${BATCH_ID}.log"

printf '%s\n' "$RUNNER_LOG" > "$LOG_DIR/v174_latest_runner_log.txt"
printf '%s\n' "$SUMMARY" > "$LOG_DIR/v174_latest_summary.txt"
printf '%s\n' "$BATCH_ID" > "$LOG_DIR/v174_latest_run_id.txt"
...
run_render() {
  ...
  printf '%s\n' "$render_log" > "$LOG_DIR/v174_latest_render_log.txt"
...
run_one() {
  ...
  printf '%s\n' "$exp_dir" > "$LOG_DIR/v174_latest_exp.txt"
  printf '%s\n' "$train_log" > "$LOG_DIR/v174_latest_log.txt"
...
echo "[$(date '+%F %T')] v174 batch started"
echo "summary=$SUMMARY"
echo "registry=$REGISTRY"

run_one "v174a" 2000 "stageA_377_multiview_explicit_hq_v174a_v170e_boundarydirect_rgbcompose_v1" "boundarydirect_rgbcompose"

echo "[$(date '+%F %T')] v174 batch finished"
cat "$SUMMARY"
