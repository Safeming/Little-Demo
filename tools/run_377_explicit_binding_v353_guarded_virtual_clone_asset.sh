...
RUN_ID="${RUN_ID:-v353_guarded_virtual_clone_asset_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v353_guarded_virtual_clone_asset_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v353_guarded_virtual_clone_asset_${RUN_ID}}"
...
V353_SOURCE_CURRENT_VARIANT="${V353_SOURCE_CURRENT_VARIANT:-formal_v338_temporal_selector_grow_only_guard}"
...
V353_ASSET_JSON="$ASSET_DIR/v353_guarded_virtual_clone_asset.json"
V353_ROW_GUARD_JSON="$ASSET_DIR/v353_guarded_virtual_clone_row_guard_upper_bound.json"
V353_CANDIDATES_TSV="$ASSET_DIR/v353_guarded_virtual_clone_candidates.tsv"
...
for required in \
  ...
  "$V345_DENSE_EXP/$V353_SOURCE_CURRENT_VARIANT/diagnostics/contours/contour_samples.csv" \
...
"$PYTHON_BIN" tools/make_377_stageB_v347_component_3d_asset.py \
  ...
  --out-json "$V353_ASSET_JSON" \
  --out-row-guard-json "$V353_ROW_GUARD_JSON" \
  --out-candidates-tsv "$V353_CANDIDATES_TSV" \
  "${MAKE_ARGS[@]}" \
  > "$LOG_DIR/make_v353_asset.log" 2>&1
log_event "make_asset_done" "$V353_ASSET_JSON"
...
run_window() {
  ...
  render_and_analyze "$window" "$views_spec" "$frames_spec" v353_component_local_asset "$CANDIDATE_CKPT" "$window_root/v353_component_local_asset" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_signed_dynamic_component_local_asset_json=$V353_ASSET_JSON"

  render_and_analyze "$window" "$views_spec" "$frames_spec" v353_component_local_asset_plus_v345 "$CANDIDATE_CKPT" "$window_root/v353_component_local_asset_plus_v345" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_signed_point_screen_actuator_drop_images='$POINT_SCREEN_DROP_IMAGES'" \
    "++explicit_binding_adopted_signed_dynamic_component_local_asset_json=$V353_ASSET_JSON"

  render_and_analyze "$window" "$views_spec" "$frames_spec" v353_guarded_virtual_grow_clone_plus_v345 "$CANDIDATE_CKPT" "$window_root/v353_guarded_virtual_grow_clone_plus_v345" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_signed_point_screen_actuator_drop_images='$POINT_SCREEN_DROP_IMAGES'" \
    "++explicit_binding_adopted_signed_dynamic_component_local_asset_json=$V353_ASSET_JSON" \
  ...
  render_and_analyze "$window" "$views_spec" "$frames_spec" v353_guarded_virtual_grow_clone_weighted_plus_v345 "$CANDIDATE_CKPT" "$window_root/v353_guarded_virtual_grow_clone_weighted_plus_v345" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_signed_point_screen_actuator_drop_images='$POINT_SCREEN_DROP_IMAGES'" \
    "++explicit_binding_adopted_signed_dynamic_component_local_asset_json=$V353_ASSET_JSON" \
    "++pipeline.covariance_signed_virtual_grow_clone_enable=true" \
  ...
  render_and_analyze "$window" "$views_spec" "$frames_spec" v353_row_guard_upper_bound_plus_v345 "$CANDIDATE_CKPT" "$window_root/v353_row_guard_upper_bound_plus_v345" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_signed_point_screen_actuator_drop_images='$POINT_SCREEN_DROP_IMAGES'" \
    "++explicit_binding_adopted_signed_dynamic_component_row_guard_json=$V353_ROW_GUARD_JSON"
}
...
variants = [
    ...
    "v353_component_local_asset",
    "v353_component_local_asset_plus_v345",
    "v353_guarded_virtual_grow_clone_plus_v345",
    "v353_guarded_virtual_grow_clone_weighted_plus_v345",
    "v353_row_guard_upper_bound_plus_v345",
]
...
echo "V353_ASSET_JSON=$V353_ASSET_JSON"
echo "V353_ROW_GUARD_JSON=$V353_ROW_GUARD_JSON"
echo "V353_CANDIDATES_TSV=$V353_CANDIDATES_TSV"
echo
echo "Dense gate command only if sentinel probe passes:"
echo "CANDIDATE_CKPT=$CANDIDATE_CKPT \\"
echo "CANDIDATE_VARIANT_NAME=candidate_v353_guarded_virtual_grow_clone_weighted_plus_v345 \\"
echo "CANDIDATE_SIGNED_POINT_SCREEN_ACTUATOR_DROP_IMAGES='$POINT_SCREEN_DROP_IMAGES' \\"
echo "CANDIDATE_SIGNED_DYNAMIC_COMPONENT_LOCAL_ASSET_JSON=$V353_ASSET_JSON \\"
...
echo "RUN_ID='formal_v353_guarded_virtual_clone_dense_gate_$(TZ=Asia/Shanghai date '+%Y%m%d')' \\"
