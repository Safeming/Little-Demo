...
def _parse_xyz(value: str) -> list[float]:
    ...
    if not isinstance(parsed, (list, tuple)) or len(parsed) != 3:
        raise ValueError(f"invalid xyz value: {value}")
...
def _read_candidates(path: Path, max_candidates: int) -> list[dict]:
    ...
    with path.open("r", encoding="utf-8", newline="") as handle:
        ...
        for idx, row in enumerate(reader):
            ...
            for key in (
                ...
                "actual_inner_pixels",
                "actual_outer_pixels",
                "actual_outer_inner_ratio",
                "actual_visible_views",
                "actual_mean_radius_px",
            ):
    ...
    rows.sort(
        key=lambda item: (
            float(item.get("footprint_score", 0.0)),
            float(item.get("actual_inner_pixels", 0.0)),
            -float(item.get("actual_outer_pixels", 0.0)),
...
def _assign_parent_and_canonicalize(
    scene: Scene,
    view,
    pkg: dict,
    candidate: dict,
    base_count: int,
    parent_screen_radius: float,
    ...
    target_xy = torch.tensor([candidate["birth_x"], candidate["birth_y"]], dtype=torch.float32, device="cuda")
    candidate_deformed = torch.tensor(candidate["xyz"], dtype=torch.float32, device="cuda")
    if visible_idx.numel() > 0:
        screen_dist = torch.norm(visible_xy - target_xy.reshape(1, 2), dim=-1)
        within = screen_dist <= float(parent_screen_radius)
        if bool(within.any().item()):
            ...
            pick_local = int(torch.argmin(local_screen / max(float(parent_screen_radius), 1.0) + 8.0 * local_3d).item())
    ...
    return parent_idx, new_xyz.detach()


def main() -> int:
    parser = argparse.ArgumentParser(description="Append v277 actual-radii verified support points without patch expansion.")
    ...
    parser.add_argument("--checkpoint-iteration", type=int, default=136411)
    ...
    for idx in range(len(scene.test_dataset)):
        ...
        if cam in views and frame in frames:
    ...
    for key, key_candidates in candidates_by_key.items():
        ...
        for cand_idx, cand in key_candidates:
            parent_idx, new_xyz = _assign_parent_and_canonicalize(
                scene,
                view,
                pkg,
                cand,
                base_count,
                float(args.parent_screen_radius),
            )
            parent_indices.append(parent_idx)
            canonical_xyz.append(new_xyz)
            ...
            support_component.append(int(cand.get("source_candidate_index", cand_idx)))
            parent_pick_rows.append({
                ...
                "actual_inner_pixels": float(cand.get("actual_inner_pixels", 0.0)),
                "actual_outer_pixels": float(cand.get("actual_outer_pixels", 0.0)),
                "actual_outer_inner_ratio": float(cand.get("actual_outer_inner_ratio", 0.0)),
    ...
    if not parent_indices:
        raise RuntimeError("no verified support candidates could be assigned a parent")
    ...
    if float(args.child_scale_max) > 0.0:
    ...
    ckpt_path = scene.save_checkpoint(
        int(args.checkpoint_iteration),
        filename=f"ckpt{int(args.checkpoint_iteration)}.pth",
        verbose=True,
    )
    summary = {
        "status": "ok",
        ...
        "child_opacity_factor": float(args.child_opacity_factor),
        "child_scale_factor": float(args.child_scale_factor),
        "child_scale_max": float(args.child_scale_max),
        "mean_child_opacity": float(child_opacity.mean().item()),
        "mean_child_scale": float(child_scaling.amax(dim=-1).mean().item()),
    }
    (args.out_dir / "v277_verified_support_append_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    with (args.out_dir / "v277_verified_support_parent_assignments.csv").open("w", newline="", encoding="utf-8") as handle:
