...
RUN_ID="${RUN_ID:-v352_virtual_grow_clone_asset_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v352_virtual_grow_clone_asset_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v352_virtual_grow_clone_asset_${RUN_ID}}"
...
WINDOW_SPECS="${WINDOW_SPECS:-c23_f298_301|[23]|[298,301,1];c22_f240_242|[22]|[240,242,1];c21_f480_482|[21]|[480,482,1];c23_f060_062|[23]|[60,62,1]}"
...
V352_SOURCE_CURRENT_VARIANT="${V352_SOURCE_CURRENT_VARIANT:-formal_v338_temporal_selector_grow_only_guard}"
...
V346_ASSET_JSON="${V346_ASSET_JSON:-$ROOT/exp/stageB/logs/377_explicit_binding_v346_component_local_asset_v346c_formal_source_probe_20260523/assets/v346_component_local_asset.json}"
...
VIRTUAL_GROW_CLONE_OPACITY_SCALE="${VIRTUAL_GROW_CLONE_OPACITY_SCALE:-0.35}"
VIRTUAL_GROW_CLONE_MAX_POINTS="${VIRTUAL_GROW_CLONE_MAX_POINTS:--1}"
VIRTUAL_GROW_CLONE_MIN_SCORE="${VIRTUAL_GROW_CLONE_MIN_SCORE:-0.0}"
VIRTUAL_GROW_CLONE_INNER_PX="${VIRTUAL_GROW_CLONE_INNER_PX:-0.0}"
...
V352_ASSET_JSON="$ASSET_DIR/v352_virtual_grow_clone_asset.json"
V352_ROW_GUARD_JSON="$ASSET_DIR/v352_virtual_grow_clone_row_guard_upper_bound.json"
V352_CANDIDATES_TSV="$ASSET_DIR/v352_virtual_grow_clone_candidates.tsv"
...
for required in \
  ...
  "$V345_DENSE_EXP/$V352_SOURCE_CURRENT_VARIANT/diagnostics/contours/contour_samples.csv" \
  "$V345_SCREEN_GUARD_JSON" "$V345_SCREEN_DROP_LIST" "$V344_ROW_GUARD_JSON" "$V346_ASSET_JSON"; do
  if [ ! -e "$required" ]; then
...
"$PYTHON_BIN" tools/make_377_stageB_v347_component_3d_asset.py \
  ...
  --out-json "$V352_ASSET_JSON" \
  --out-row-guard-json "$V352_ROW_GUARD_JSON" \
  --out-candidates-tsv "$V352_CANDIDATES_TSV" \
  "${MAKE_ARGS[@]}" \
  > "$LOG_DIR/make_v352_asset.log" 2>&1
log_event "make_asset_done" "$V352_ASSET_JSON"
...
run_window() {
  ...
  render_and_analyze "$window" "$views_spec" "$frames_spec" v352_component_local_asset "$CANDIDATE_CKPT" "$window_root/v352_component_local_asset" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_signed_dynamic_component_local_asset_json=$V352_ASSET_JSON"

  render_and_analyze "$window" "$views_spec" "$frames_spec" v352_component_local_asset_plus_v345 "$CANDIDATE_CKPT" "$window_root/v352_component_local_asset_plus_v345" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_signed_point_screen_actuator_drop_images='$POINT_SCREEN_DROP_IMAGES'" \
    "++explicit_binding_adopted_signed_dynamic_component_local_asset_json=$V352_ASSET_JSON"

  render_and_analyze "$window" "$views_spec" "$frames_spec" v352_virtual_grow_clone_plus_v345 "$CANDIDATE_CKPT" "$window_root/v352_virtual_grow_clone_plus_v345" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_signed_point_screen_actuator_drop_images='$POINT_SCREEN_DROP_IMAGES'" \
    "++explicit_binding_adopted_signed_dynamic_component_local_asset_json=$V352_ASSET_JSON" \
    "++pipeline.covariance_signed_virtual_grow_clone_enable=true" \
    "++pipeline.covariance_signed_virtual_grow_clone_opacity_scale=$VIRTUAL_GROW_CLONE_OPACITY_SCALE" \
    "++pipeline.covariance_signed_virtual_grow_clone_max_points=$VIRTUAL_GROW_CLONE_MAX_POINTS" \
    "++pipeline.covariance_signed_virtual_grow_clone_min_score=$VIRTUAL_GROW_CLONE_MIN_SCORE" \
    "++pipeline.covariance_signed_virtual_grow_clone_inner_px=$VIRTUAL_GROW_CLONE_INNER_PX"

  render_and_analyze "$window" "$views_spec" "$frames_spec" v352_virtual_grow_clone_weighted_plus_v345 "$CANDIDATE_CKPT" "$window_root/v352_virtual_grow_clone_weighted_plus_v345" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_signed_point_screen_actuator_drop_images='$POINT_SCREEN_DROP_IMAGES'" \
    "++explicit_binding_adopted_signed_dynamic_component_local_asset_json=$V352_ASSET_JSON" \
    "++pipeline.covariance_signed_virtual_grow_clone_enable=true" \
  ...
  render_and_analyze "$window" "$views_spec" "$frames_spec" v352_row_guard_upper_bound_plus_v345 "$CANDIDATE_CKPT" "$window_root/v352_row_guard_upper_bound_plus_v345" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_signed_point_screen_actuator_drop_images='$POINT_SCREEN_DROP_IMAGES'" \
    "++explicit_binding_adopted_signed_dynamic_component_row_guard_json=$V352_ROW_GUARD_JSON"
}
...
variants = [
    ...
    "v352_component_local_asset",
    "v352_component_local_asset_plus_v345",
    "v352_virtual_grow_clone_plus_v345",
    "v352_virtual_grow_clone_weighted_plus_v345",
    "v352_row_guard_upper_bound_plus_v345",
]
...
echo "V352_ASSET_JSON=$V352_ASSET_JSON"
echo "V352_ROW_GUARD_JSON=$V352_ROW_GUARD_JSON"
echo "V352_CANDIDATES_TSV=$V352_CANDIDATES_TSV"
echo
echo "Dense gate command only if sentinel probe passes:"
echo "CANDIDATE_CKPT=$CANDIDATE_CKPT \\"
echo "CANDIDATE_VARIANT_NAME=candidate_v352_virtual_grow_clone_weighted_plus_v345 \\"
echo "CANDIDATE_SIGNED_POINT_SCREEN_ACTUATOR_DROP_IMAGES='$POINT_SCREEN_DROP_IMAGES' \\"
echo "CANDIDATE_SIGNED_DYNAMIC_COMPONENT_LOCAL_ASSET_JSON=$V352_ASSET_JSON \\"
...
echo "CANDIDATE_SIGNED_VIRTUAL_GROW_CLONE_MAX_POINTS=$VIRTUAL_GROW_CLONE_MAX_POINTS \\"
echo "CANDIDATE_SIGNED_VIRTUAL_GROW_CLONE_MIN_SCORE=$VIRTUAL_GROW_CLONE_MIN_SCORE \\"
echo "CANDIDATE_SIGNED_VIRTUAL_GROW_CLONE_INNER_PX=$VIRTUAL_GROW_CLONE_INNER_PX \\"
...
echo "RUN_ID='formal_v352_virtual_grow_clone_dense_gate_$(TZ=Asia/Shanghai date '+%Y%m%d')' \\"
