...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-v299_screen_space_protected_feature_refine_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
...
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v299_screen_space_protected_feature_refine_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v299_screen_space_protected_feature_refine_${RUN_ID}}"
...
Goal:
  v299 tests the structural failure mode exposed by v296-v298. Geometry,
  opacity, scale, rotation, pose and camera remain frozen. Texture MLP is
  frozen too; only Gaussian feature/SH rows may update. Unlike v298, the
  protection mask is built per training view from screen-space footprint risk:
  any Gaussian whose projected center lies on GT boundary or outer shell has
  feature gradients and Adam state cleared before every optimizer step. This
  targets large-footprint non-boundary points that still affect held-out raw
  silhouette.

Gate:
  Run raw no-train A/B first. Only a strict/probe pass gets a short guarded
  texture refine from the same checkpoint. The checkpoint is accepted only by
  raw contour gate; training loss/PSNR is not the deciding signal.
EOF
...
train_selected_variant() {
  ...
  env "${COMMON_ENV[@]}" "$PYTHON_BIN" train.py \
    ...
    "++opt.boundary_color_grad_protect_enable=true" \
    "++opt.boundary_color_grad_protect_verbose=false" \
    "++opt.screen_space_color_grad_protect_enable=true" \
    "++opt.screen_space_color_grad_protect_boundary_width=17" \
    "++opt.screen_space_color_grad_protect_outer_start_width=1" \
    "++opt.screen_space_color_grad_protect_outer_end_width=38" \
    "++opt.screen_space_color_grad_protect_radius_pad_px=5" \
    "++opt.screen_space_color_grad_protect_min_radius_px=0.0" \
    "++opt.screen_space_color_grad_protect_max_points=0" \
    ...
    "++opt.lambda_teacher_render_distill_boundary=0.70" \
    "++opt.lambda_teacher_render_distill_outer=1.20" \
    "++opt.lambda_teacher_render_distill_inner=0.04" \
    ...
    "++opt.lambda_teacher_render_distill_raw_support_boundary=1.20" \
    "++opt.lambda_teacher_render_distill_raw_support_outer=2.40" \
    "++opt.lambda_teacher_render_distill_raw_support_inner=0.15" \
    ...
    "++opt.contour_uncertainty_band_width=15" \
    "++opt.contour_uncertainty_outer_width=30" \
    "++opt.contour_uncertainty_min_weight=0.10" \
    "opt.lambda_l1=0.006" \
    "opt.lambda_l1_fg=0.020" \
    "opt.lambda_l1_boundary=0.0" \
    "opt.lambda_perceptual=0.0" \
    "++opt.lambda_l1_face=0.003" \
    "++opt.lambda_l1_shoulder_arm=0.002" \
    "++opt.lambda_l1_waist=0.002" \
