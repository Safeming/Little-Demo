...
RUN_ID="${RUN_ID:-v390_direction_boundary_residual_selector_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
EXP_DIR="${EXP_DIR:-$ROOT/exp/formal/377_v390_direction_boundary_residual_selector_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/formal/logs/377_v390_direction_boundary_residual_selector_${RUN_ID}}"
...
BOUNDARY_DIRECTION_CONFLICT_MODE="${BOUNDARY_DIRECTION_CONFLICT_MODE:-suppress}"
BOUNDARY_DIRECTION_TAG_TOPK_RATIO="${BOUNDARY_DIRECTION_TAG_TOPK_RATIO:-0.075}"
BOUNDARY_DIRECTION_TAG_MIN_RATIO="${BOUNDARY_DIRECTION_TAG_MIN_RATIO:-0.045}"
...
SELECTOR_SUMMARY="$LOG_DIR/v390_raw_contour_checkpoint_summary.tsv"
SELECTED_JSON="$LOG_DIR/v390_selected_checkpoint.json"
...
CONFIG_CONTRACT=raw_gate_base_config_consistent_with_direction_separated_boundary_support_selector
INFO
...
for ckpt in "${checkpoints[@]}"; do
  ...
  iter="${ckpt_name#ckpt}"
  gate_run_id="formal_377_v390_${ckpt_name}_raw_gate_${RUN_ID}"
  ...
  variant="candidate_v390_${ckpt_name}_direction_boundary_residual_selector"
...
payload = {
    "selector": "strict_raw_contour_then_inner_v387_floor_then_hard_outer",
