...
EXP_DIR="${1:-$ROOT/exp/stageA2/377_multiview_explicit_hq_v7c_fresh_shallow_lownoise_20k_v1}"
if [ "$#" -ge 1 ]; then
  shift 1
else
  shift "$#"
...
"$PYTHON_BIN" "$ROOT/tools/run_377_stageA2_multiview_explicit.py" \
  ...
  --option stageA_377_multiview_explicit_hq_v7c_fresh_shallow_lownoise_v1 \
  "$@"
