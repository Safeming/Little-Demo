...
SEED="${SEED:--1}"
TIME_BUDGET_SECONDS="${TIME_BUDGET_SECONDS:-10800}"
...
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageA2/logs/v216_owner_hf_rootcause_4gpu_$RUN_ID}"
...
for required in "$BASE_CKPT" "$V210B_CKPT" "$V213A100_CKPT" "$V214J300_CKPT" "$V214V350_CKPT" "$V215C410_CKPT" "$PARSER_ROOT" "$V211_PLAN"; do
  if [ ! -e "$required" ]; then
...
printf 'RUN_ID=%s\nSTART_BJT=%s\nDEADLINE_BJT=%s\nTIME_BUDGET_SECONDS=%s\nMIN_START_SECONDS=%s\nBASE_CKPT=%s\nV210B_CKPT=%s\nV213A100_CKPT=%s\nV214J300_CKPT=%s\nV214V350_CKPT=%s\nV215C410_CKPT=%s\nV211_ANALYSIS_DIR=%s\nCAM_WEIGHTS=%s\nSMOKE=%s\nDO_RENDER=%s\n' \
  ...
  "$BASE_CKPT" "$V210B_CKPT" "$V213A100_CKPT" "$V214J300_CKPT" "$V214V350_CKPT" "$V215C410_CKPT" \
  "$V211_ANALYSIS_DIR" "$CAM_WEIGHTS" "$SMOKE" "$DO_RENDER" | tee "$LOG_DIR/run_info.txt"

cat >> "$LOG_DIR/run_info.txt" <<'EOF'

v216 purpose:
  Fast root-cause/fix probe after v215 showed late view-conflict residuals only
  protect boundaries. This queue opens the direct owner/local/HF appearance
  heads and separately tests photometric/camera-affine normalization, while
  geometry, pose, opacity and scaling stay frozen.
...
acceptance:
  ...
  - A useful result must improve visible cloth/torso detail without pushing
    boundary_l1 above the v214/v215 band or edge_px above the v215 stable edge.
EOF
...
run_named_job() {
  local gpu="$1"
  local job="$2"
  local fast360="60,120,240,360"
  local mid480="80,180,320,480"
  local stable420="80,160,280,420"
  ...
  local boundary_owner_patterns="++opt.texture_trainable_name_patterns=[structured_trunk_output_head_local_color_owner_head_mlps.*,structured_trunk_output_head_local_color_owner_head_gate_mlps.*,structured_trunk_output_head_local_color_owner_head_boundary_mlps.*,structured_trunk_output_head_local_color_owner_head_boundary_gate_mlps.*,structured_trunk_output_head_hf_head_mlp.*,structured_trunk_output_head_hf_head_gate_mlp.*]"

  case "$job" in
    v216a_v214v350_owner_hf_direct)
      run_one "$job" "$gpu" 360 1.8e-05 "$V214V350_CKPT" 109350 "$fast360" \
        "$hf_owner_patterns" \
      ...
      ;;
    v216b_v214v350_owner_hf_photo_affine)
      run_one "$job" "$gpu" 420 1.2e-05 "$V214V350_CKPT" 109350 "$stable420" \
        "$owner_only_patterns" \
        "++opt.camera_affine_enable=true" \
        "++opt.camera_affine_lr=6.0e-05" \
        "++opt.camera_affine_strength=[0.18,1,0.42,160,0.62]" \
        "++opt.lambda_camera_affine_reg=0.0040" \
        "++opt.photometric_correction_enable=true" \
        "++opt.photometric_correction_strength=0.26" \
        "opt.lambda_l1_fg=0.118" \
        "opt.lambda_l1_boundary=0.212" \
        "opt.lambda_perceptual=0.104" \
        "opt.grad_clip=0.0050"
      ;;
    v216c_v215c410_owner_hf_stable_edge)
      run_one "$job" "$gpu" 360 1.4e-05 "$V215C410_CKPT" 109410 "$fast360" \
        "$owner_only_patterns" \
        "opt.lambda_l1_fg=0.120" \
        "opt.lambda_l1_boundary=0.214" \
        "opt.lambda_perceptual=0.102" \
        "++opt.perceptual_adaptive_edge_protect=0.98" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.strength=[0.0,1,0.50,100,0.70]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.min_scale=0.30" \
        "opt.grad_clip=0.0048"
      ;;
    v216d_v215c410_owner_hf_no_vconf)
      run_one "$job" "$gpu" 360 1.5e-05 "$V215C410_CKPT" 109410 "$fast360" \
        "$owner_only_patterns" \
        ...
        "opt.grad_clip=0.0050"
      ;;
    v216e_v210b150_owner_hf_rebuild)
      run_one "$job" "$gpu" 480 3.2e-05 "$V210B_CKPT" 109150 "$mid480" \
        "$hf_owner_patterns" \
        "opt.lambda_l1_fg=0.122" \
        "opt.lambda_l1_boundary=0.206" \
        "opt.lambda_perceptual=0.106" \
        "++opt.owner_local_detail_boost_luma_max_extra=[0.62,1,1.15,180,1.52]" \
        "++opt.owner_local_detail_boost_patch_max_extra=[0.32,1,0.64,180,0.88]" \
        "++model.texture.structured_trunk.output_head.local_color.owner.head.takeover.strength=[0.50,1,0.82,180,1.00]" \
        "++model.texture.structured_trunk.output_head.dual_head.hf_head.scale=[0.58,1,0.78,180,0.94]" \
        "opt.grad_clip=0.0090"
      ;;
    v216f_v210b150_photo_affine_rebuild)
      run_one "$job" "$gpu" 480 2.4e-05 "$V210B_CKPT" 109150 "$mid480" \
        "$owner_only_patterns" \
        "++opt.camera_affine_enable=true" \
        "++opt.camera_affine_lr=8.0e-05" \
        "++opt.camera_affine_strength=[0.22,1,0.50,180,0.72]" \
        "++opt.lambda_camera_affine_reg=0.0035" \
        "++opt.photometric_correction_enable=true" \
        "++opt.photometric_correction_strength=0.30" \
        "opt.lambda_l1_fg=0.122" \
      ...
      ;;
    v216g_v214j300_boundary_owner_edgehold)
      run_one "$job" "$gpu" 420 1.2e-05 "$V214J300_CKPT" 109300 "$stable420" \
        "$boundary_owner_patterns" \
        "opt.lambda_l1_fg=0.118" \
        "opt.lambda_l1_boundary=0.220" \
        "opt.lambda_perceptual=0.100" \
        "++opt.perceptual_adaptive_edge_protect=0.99" \
        "++opt.owner_local_detail_boost_boundary_max_extra=[0.12,1,0.24,160,0.34]" \
        "++model.texture.structured_trunk.output_head.local_color.owner.head.boundary.scale=[0.58,1,0.76,160,0.90]" \
        "++model.texture.structured_trunk.output_head.local_color.owner.head.boundary.takeover.scale=[0.38,1,0.56,160,0.66]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.30" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.010" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.boundary_suppress.strength=[0.0,1,0.58,120,0.78]" \
        "opt.grad_clip=0.0045"
      ;;
    v216h_v213a100_cache_owner_restart)
      run_one "$job" "$gpu" 480 2.6e-05 "$V213A100_CKPT" 109100 "$mid480" \
        "$hf_owner_patterns" \
        "opt.lambda_l1_fg=0.120" \
        "opt.lambda_l1_boundary=0.212" \
        "opt.lambda_perceptual=0.104" \
        "++model.texture.structured_trunk.output_head.local_color.owner.head.takeover.strength=[0.46,1,0.76,180,0.96]" \
        "++model.texture.structured_trunk.output_head.dual_head.hf_head.scale=[0.54,1,0.74,180,0.88]" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.scale=0.28" \
        "++model.texture.detail_residual.high_frequency.view_conflict_residual.max_residual=0.010" \
