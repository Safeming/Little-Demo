...
RUN_ID="${RUN_ID:-v355_validated_paired_rowlocal_asset_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v355_validated_paired_rowlocal_asset_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v355_validated_paired_rowlocal_asset_${RUN_ID}}"
...
ROWS_PER_DIRECTION="${ROWS_PER_DIRECTION:-5}"
PAIRS_PER_FRAME="${PAIRS_PER_FRAME:-2}"
MAX_PAIRS="${MAX_PAIRS:-120}"
MAX_TOP_IDS="${MAX_TOP_IDS:-8}"
MIN_OWNER_CONSISTENCY="${MIN_OWNER_CONSISTENCY:-0.50}"
OWNER_GATE="${OWNER_GATE:-true}"
REQUIRE_OWNER_MATCH="${REQUIRE_OWNER_MATCH:-false}"
INNER_MODE="${INNER_MODE:-paired_local_3d_replace}"
OUTER_MODE="${OUTER_MODE:-paired_local_3d_intersect}"
SCORE_SCALE_BOOST="${SCORE_SCALE_BOOST:-0.35}"
RADIUS_FLOOR="${RADIUS_FLOOR:-0.010}"
RADIUS_PAD="${RADIUS_PAD:-0.006}"
ROW_RADIUS_SCALE="${ROW_RADIUS_SCALE:-1.15}"
PAIR_RADIUS_PAD="${PAIR_RADIUS_PAD:-0.004}"
PAIR_RADIUS_SCALE="${PAIR_RADIUS_SCALE:-1.05}"
PAIR_RADIUS_MAX="${PAIR_RADIUS_MAX:-0.38}"
PAIR_CENTER_MAX="${PAIR_CENTER_MAX:-0.32}"
...
PAIR_VALIDATE_MAX="${PAIR_VALIDATE_MAX:-16}"
...
V355_SEED_ASSET_JSON="$ASSET_DIR/v355_seed_paired_rowlocal_asset.json"
V355_ASSET_JSON="$ASSET_DIR/v355_validated_paired_rowlocal_asset.json"
V355_ROW_GUARD_JSON="$ASSET_DIR/v355_validated_paired_rowlocal_row_guard_upper_bound.json"
V355_CANDIDATES_TSV="$ASSET_DIR/v355_validated_paired_rowlocal_candidates.tsv"
V355_PAIR_LIST_TSV="$ASSET_DIR/v355_seed_pairs.tsv"
...
MAKE_ARGS=()
[ "$OWNER_GATE" = "true" ] && MAKE_ARGS+=(--owner-gate)
...
log_event "make_seed_asset_start" "$V355_SEED_ASSET_JSON"
"$PYTHON_BIN" tools/make_377_stageB_v349_paired_component_3d_asset.py \
  ...
  --rows-per-direction "$ROWS_PER_DIRECTION" \
  --pairs-per-frame "$PAIRS_PER_FRAME" \
  --max-pairs "$MAX_PAIRS" \
  --max-top-ids "$MAX_TOP_IDS" \
  --min-owner-consistency "$MIN_OWNER_CONSISTENCY" \
  --inner-mode "$INNER_MODE" \
  --outer-mode "$OUTER_MODE" \
  --score-scale-boost "$SCORE_SCALE_BOOST" \
  --radius-floor "$RADIUS_FLOOR" \
  --radius-pad "$RADIUS_PAD" \
  --row-radius-scale "$ROW_RADIUS_SCALE" \
  --pair-radius-pad "$PAIR_RADIUS_PAD" \
  --pair-radius-scale "$PAIR_RADIUS_SCALE" \
  --pair-radius-max "$PAIR_RADIUS_MAX" \
  --pair-center-max "$PAIR_CENTER_MAX" \
  --row-local-actions \
  --cluster-min-points "$CLUSTER_MIN_POINTS" \
  --cluster-radius-max "$CLUSTER_RADIUS_MAX" \
  --out-json "$V355_SEED_ASSET_JSON" \
  --out-row-guard-json "$V355_ROW_GUARD_JSON" \
  --out-candidates-tsv "$V355_CANDIDATES_TSV" \
  "${MAKE_ARGS[@]}" \
  > "$LOG_DIR/make_v355_seed_asset.log" 2>&1
log_event "make_seed_asset_done" "$V355_SEED_ASSET_JSON"

"$PYTHON_BIN" - "$V355_SEED_ASSET_JSON" "$V355_PAIR_LIST_TSV" "$PAIR_VALIDATE_MAX" <<'PY'
import csv
import json
import re
import sys
...
with out_path.open("w", encoding="utf-8", newline="") as handle:
    fieldnames = ["pair_index", "pair_id", "image_name", "view", "frame"]
...
if [ "$PAIR_VALIDATE_ENABLE" = "true" ]; then
  while IFS=$'\t' read -r pair_index pair_id image_name view frame; do
    ...
    render_and_analyze "pair_${safe_key}" "$views_spec" "$frames_spec" pair_only_plus_v345 "$CANDIDATE_CKPT" "$pair_root/pair_only_plus_v345" \
      "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
      "++explicit_binding_adopted_signed_point_screen_actuator_drop_images='$POINT_SCREEN_DROP_IMAGES'" \
      "++explicit_binding_adopted_signed_dynamic_component_local_asset_json=$V355_SEED_ASSET_JSON" \
    ...
    append_pair_validation_row "$pair_id" "$image_name" "$pair_root/control_v345" "$pair_root/pair_only_plus_v345"
    log_event "pair_validate_done" "$pair_id"
  done < "$V355_PAIR_LIST_TSV"
fi

"$PYTHON_BIN" - "$V355_SEED_ASSET_JSON" "$PAIR_VALIDATION_TSV" "$V355_ASSET_JSON" <<'PY'
import csv
import json
import sys
from pathlib import Path
...
seed_pairs = data.get("pairs", [])
if rows:
    data["pairs"] = [pair for pair in seed_pairs if str(pair.get("pair_id", "")) in kept]
    data["actions"] = [action for action in data.get("actions", []) if str(action.get("pair_id", "")) in kept]
else:
    data["pairs"] = []
    data["actions"] = []
data["version"] = "v355_validated_paired_rowlocal_asset"
data["policy"] = (
    "v355 keeps only paired inner-grow/outer-shrink row-local actions that pass a single-pair "
    "render validation do-no-harm gate against v338+v345."
)
...
print(f"wrote {out_path} seed_pair_count={len(seed_pairs)} kept_pair_count={len(data.get('pairs', []))} actions={len(data.get('actions', []))}")
PY
log_event "validated_asset_done" "$V355_ASSET_JSON"
...
run_window() {
  ...
  render_and_analyze "$window" "$views_spec" "$frames_spec" v355_validated_paired_rowlocal_plus_v345 "$CANDIDATE_CKPT" "$window_root/v355_validated_paired_rowlocal_plus_v345" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_signed_point_screen_actuator_drop_images='$POINT_SCREEN_DROP_IMAGES'" \
    "++explicit_binding_adopted_signed_dynamic_component_local_asset_json=$V355_ASSET_JSON"

  render_and_analyze "$window" "$views_spec" "$frames_spec" v355_row_guard_upper_bound_plus_v345 "$CANDIDATE_CKPT" "$window_root/v355_row_guard_upper_bound_plus_v345" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_signed_point_screen_actuator_drop_images='$POINT_SCREEN_DROP_IMAGES'" \
    "++explicit_binding_adopted_signed_dynamic_component_row_guard_json=$V355_ROW_GUARD_JSON"
}
...
variants = [
    ...
    "formal_v338_current",
    "v345_screen_guard",
    "v355_validated_paired_rowlocal_plus_v345",
    "v355_row_guard_upper_bound_plus_v345",
]
metrics = ("fg", "boundary", "edge", "inner", "outer", "hard", "opacity_inner", "opacity_outer")
...
echo "V355_ASSET_JSON=$V355_ASSET_JSON"
echo "V355_ROW_GUARD_JSON=$V355_ROW_GUARD_JSON"
echo "V355_CANDIDATES_TSV=$V355_CANDIDATES_TSV"
