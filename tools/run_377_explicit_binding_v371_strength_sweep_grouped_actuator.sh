...
RUN_ID="${RUN_ID:-v371_strength_sweep_grouped_actuator_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v371_strength_sweep_grouped_actuator_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v371_strength_sweep_grouped_actuator_${RUN_ID}}"
...
INNER_PER_FRAME="${INNER_PER_FRAME:-4}"
OUTER_PER_INNER="${OUTER_PER_INNER:-1}"
MAX_GROUPS="${MAX_GROUPS:-192}"
MAX_GROUPS_PER_IMAGE="${MAX_GROUPS_PER_IMAGE:-0}"
MAX_GROUPS_PER_SOURCE="${MAX_GROUPS_PER_SOURCE:-0}"
MAX_GROUPS_PER_TARGET="${MAX_GROUPS_PER_TARGET:-0}"
...
RESIDUAL_GATE_PAD_PX="${RESIDUAL_GATE_PAD_PX:-18.0}"
...
STRENGTH_SWEEP_VARIANTS="${STRENGTH_SWEEP_VARIANTS:-base,co4_cr1.5_np,co3_cr1.5_np}"

GROUP_VALIDATE_MAX="${GROUP_VALIDATE_MAX:-48}"
...
RUN_LABEL="${RUN_LABEL:-v371}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-${RUN_LABEL}_strength_sweep_grouped_actuator}"
RAW_GATE_CANDIDATE_VARIANT="${RAW_GATE_CANDIDATE_VARIANT:-candidate_${EXPERIMENT_NAME}}"
TRAIN_RUN_PREFIX="${TRAIN_RUN_PREFIX:-formal_377_${EXPERIMENT_NAME}_semantic_train}"
TRAIN_EXP_PREFIX="${TRAIN_EXP_PREFIX:-377_${EXPERIMENT_NAME}_semantic_train}"

SUMMARY="$LOG_DIR/summary.tsv"
GROUP_VALIDATION_TSV="$LOG_DIR/group_validation.tsv"
EVENTS="$LOG_DIR/events.tsv"
ASSET_DIR="$LOG_DIR/assets"
V371_BOOTSTRAP_ASSET_JSON="$ASSET_DIR/v371_bootstrap_residual_component_multi_micro_grouped_actuator_asset.json"
V371_BOOTSTRAP_CANDIDATES_TSV="$ASSET_DIR/v371_bootstrap_residual_component_multi_micro_grouped_actuator_groups.tsv"
V371_SEED_BASE_ASSET_JSON="$ASSET_DIR/v371_seed_base_residual_component_multi_micro_grouped_actuator_asset.json"
V371_SEED_ASSET_JSON="$ASSET_DIR/v371_seed_residual_component_multi_micro_grouped_actuator_asset.json"
V371_SEED_CANDIDATES_TSV="$ASSET_DIR/v371_seed_residual_component_multi_micro_grouped_actuator_groups.tsv"
V371_ASSET_JSON="$ASSET_DIR/v371_validated_residual_component_multi_micro_grouped_actuator_asset.json"
V371_ACTION_LIST_TSV="$ASSET_DIR/v371_seed_action_groups.tsv"
V371_RENDERER_SPACE_NPZ="$ASSET_DIR/v371_renderer_space_gaussians.npz"
V371_RENDERER_SPACE_TSV="$ASSET_DIR/v371_renderer_space_gaussians.tsv"
RAW_GATE_LOG="$LOG_DIR/v371_raw_gate.launcher.log"
TRAIN_LAUNCH_LOG="$LOG_DIR/v371_semantic_train.launcher.log"
...
make_asset() {
  ...
  "$PYTHON_BIN" tools/make_377_stageB_v369_residual_component_multi_micro_grouped_actuator_asset.py "${args[@]}"
}
...
for index, group in enumerate(data.get("action_groups", [])):
    ...
    if not image or not pair_id or not source_key or not match:
        continue
    selection_key = str(group.get("selection_key", "") or "")
    if not selection_key:
        selection_key = f"{source_key}:target{int(group.get('residual_target_rank', 0) or 0)}"
    groups.append({
        "action_index": index,
        "pair_id": pair_id,
        ...
        "target_rank": int(group.get("residual_target_rank", 0) or 0),
        "target_score": float(group.get("residual_target_score", 0.0) or 0.0),
        "strength_variant": str(group.get("strength_variant", "base") or "base"),
    })
groups.sort(key=lambda row: (row["score"], row["target_score"]), reverse=True)

def variant_priority(name: str) -> tuple[int, float, str]:
    text = str(name or "base")
    if text == "base":
        return (0, 0.0, text)
    match = re.search(r"co([0-9p.]+)_cr([0-9p.]+)", text)
    if match is None:
        return (5, 0.0, text)
    try:
        opacity = float(match.group(1).replace("p", "."))
        radius = float(match.group(2).replace("p", "."))
    except ValueError:
        return (5, 0.0, text)
    # Representative variants first; larger/aggressive shapes are used after
    # coverage has had a chance to sample other source targets.
    distance = abs(opacity - 3.0) + abs(radius - 1.2)
    tier = 1 if radius <= 1.25 else (2 if radius <= 1.45 else 3)
    return (tier, distance, text)

by_target = defaultdict(deque)
target_meta = {}
for row in groups:
    key = (row["image_name"], row["selection_key"], row["strength_variant"])
    by_target[key].append(row)
    target_meta[key] = row
target_best = {}
for row in groups:
    key = (row["image_name"], row["selection_key"])
    current = target_best.get(key)
    score = (row["score"], row["target_score"])
    if current is None or score > current:
        target_best[key] = score
ordered = []
while by_target:
    keys = sorted(
        list(by_target),
        key=lambda key: (
            variant_priority(key[2]),
            target_meta[key]["view"],
            -target_best.get((key[0], key[1]), (0.0, 0.0))[0],
            -target_best.get((key[0], key[1]), (0.0, 0.0))[1],
            key[0],
            key[1],
        ),
    )
    ...
    image_order = sorted(
        by_image,
        key=lambda image: (
            target_meta[by_image[image][0]]["view"],
            -max(target_best.get((image, key[1]), (0.0, 0.0))[0] for key in by_image[image]),
            image,
        ),
    )
    for image in image_order:
        key = by_image[image][0]
        bucket = by_target.get(key)
        if not bucket:
            by_target.pop(key, None)
            continue
...
with out_path.open("w", encoding="utf-8", newline="") as handle:
    fieldnames = [
        ...
        "micro_count", "outer_action_count", "score", "target_rank", "target_score", "strength_variant",
    ]
...
log_event "make_bootstrap_asset_start" "$V371_BOOTSTRAP_ASSET_JSON"
make_asset "$V371_BOOTSTRAP_ASSET_JSON" "$V371_BOOTSTRAP_CANDIDATES_TSV" \
  > "$LOG_DIR/make_v371_bootstrap_asset.log" 2>&1
write_action_list "$V371_BOOTSTRAP_ASSET_JSON" "$V371_ACTION_LIST_TSV" -1
log_event "make_bootstrap_asset_done" "$V371_BOOTSTRAP_ASSET_JSON"

log_event "export_renderer_space_start" "$V371_RENDERER_SPACE_NPZ"
env "${COMMON_ENV[@]}" "$PYTHON_BIN" tools/export_renderer_space_gaussians.py \
  --config "$BASE_EXP/.hydra/config.yaml" \
  --checkpoint "$CANDIDATE_CKPT" \
  --action-list-tsv "$V371_ACTION_LIST_TSV" \
  --out-npz "$V371_RENDERER_SPACE_NPZ" \
  --out-tsv "$V371_RENDERER_SPACE_TSV" \
...
log_event "export_renderer_space_done" "$V371_RENDERER_SPACE_NPZ"

log_event "make_seed_asset_start" "$V371_SEED_BASE_ASSET_JSON"
make_asset "$V371_SEED_BASE_ASSET_JSON" "$V371_SEED_CANDIDATES_TSV" \
  --renderer-space-cache "$V371_RENDERER_SPACE_NPZ" \
  > "$LOG_DIR/make_v371_seed_asset.log" 2>&1
"$PYTHON_BIN" tools/expand_377_stageB_v371_strength_sweep_asset.py \
  --in-json "$V371_SEED_BASE_ASSET_JSON" \
  --out-json "$V371_SEED_ASSET_JSON" \
  --variants "$STRENGTH_SWEEP_VARIANTS" \
  > "$LOG_DIR/expand_v371_strength_sweep_asset.log" 2>&1
write_action_list "$V371_SEED_ASSET_JSON" "$V371_ACTION_LIST_TSV" "$GROUP_VALIDATE_MAX"
log_event "make_seed_asset_done" "$V371_SEED_ASSET_JSON"
...
while IFS=$'\t' read -r action_index pair_id source_component_key image_name view frame micro_count outer_action_count score; do
  ...
  candidate_exp="$group_root/v371_grouped_actuator"
  ...
  render_and_analyze "group_${safe_pair}" "$views_spec" "$frames_spec" v371_grouped_actuator "$candidate_exp" \
    "pipeline.compute_cov3D_python=true" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++pipeline.split_child_component_enable=true" \
    "++pipeline.split_child_component_asset_json=$V371_SEED_ASSET_JSON" \
...
done < "$V371_ACTION_LIST_TSV"

"$PYTHON_BIN" - "$V371_SEED_ASSET_JSON" "$GROUP_VALIDATION_TSV" "$V371_ASSET_JSON" <<'PY'
import csv
import json
import sys
from pathlib import Path
...
rows = []
best_by_source = {}
groups_by_pair = {str(group.get("pair_id", "")): group for group in data.get("action_groups", [])}
with validation_path.open("r", encoding="utf-8", newline="") as handle:
    for row in csv.DictReader(handle, delimiter="\t"):
        ...
        target_rank = str(group.get("residual_target_rank", "0"))
        selection_key = f"{source}:target{target_rank}"
        if not source or not pair_id:
            continue
        try:
            score = (
            ...
            continue
        current = best_by_source.get(selection_key)
        if current is None or score > current["score"]:
            best_by_source[selection_key] = {"score": score, "pair_id": pair_id}
...
payload = {
    **{key: value for key, value in data.items() if key not in ("action_groups", "children", "actions")},
    "version": "v371_validated_residual_driven_grouped_actuator_asset",
    "policy": (
        "Validated v371 asset: one best action-level raw-contour-safe grouped actuator per source residual target."
    ),
...
"$PYTHON_BIN" - "$SUMMARY" "$GROUP_VALIDATION_TSV" "$V371_ASSET_JSON" <<'PY'
import csv
import json
import sys
from pathlib import Path
...
if [ "$RAW_GATE_ENABLE" = "true" ]; then
  selected_groups="$("$PYTHON_BIN" - "$V371_ASSET_JSON" <<'PY'
import json, sys
...
PY
  ...
  if [ "$selected_groups" -gt 0 ]; then
    RAW_GATE_RUN_ID="formal_377_${EXPERIMENT_NAME}_raw_gate_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')"
    log_event "raw_gate_start" "$RAW_GATE_RUN_ID"
    env -u EXP_ROOT -u LOG_DIR -u HYDRA_RUN_ROOT \
      GPU="$GPU" PYTHON_BIN="$PYTHON_BIN" CPU_THREADS_PER_JOB="$CPU_THREADS_PER_JOB" \
      BASE_CKPT="$BASE_CKPT" \
      CANDIDATE_CKPT="$CANDIDATE_CKPT" \
      CANDIDATE_VARIANT_NAME="$RAW_GATE_CANDIDATE_VARIANT" \
      CANDIDATE_SPLIT_CHILD_COMPONENT_ENABLE=true \
      CANDIDATE_SPLIT_CHILD_COMPONENT_ASSET_JSON="$V371_ASSET_JSON" \
    ...
    RAW_GATE_STATUS="$("$PYTHON_BIN" - "$RAW_GATE_SUMMARY" "$RAW_GATE_CANDIDATE_VARIANT" <<'PY'
import csv, sys
path=sys.argv[1]
...
if [ "$TRAIN_ON_STRICT_PASS" = "true" ] && [ "$RAW_GATE_STATUS" = "strict_pass" ]; then
  TRAIN_RUN_ID="${TRAIN_RUN_PREFIX}_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')"
  TRAIN_EXP_DIR="$ROOT/exp/formal/${TRAIN_EXP_PREFIX}_${TRAIN_RUN_ID}"
  TRAIN_SCRIPT="$LOG_DIR/${RUN_LABEL}_semantic_train.launch.sh"
...
echo "V371_ASSET_JSON=$V371_ASSET_JSON"
