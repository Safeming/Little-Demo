...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-v290_component_topid_cov_residual_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
...
AUDIT_DIR="${AUDIT_DIR:-$ROOT/exp/stageB/logs/377_stageB_v290_train_component_audit_${RUN_ID}}"
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v290_component_topid_cov_residual_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v290_component_topid_cov_residual_${RUN_ID}}"
...
Goal:
  v290 makes the residual actuator component-conditioned. The dynamic mask requires
  a residual component record and activates only the component top contributor ids,
  so a per-point residual no longer applies broadly across all views/components.
EOF
...
log_event "v290_train_start" "$EXP_ROOT"
RUN_ID="$RUN_ID" \
...
COMPONENT_TOP_IDS_ENABLE=true \
COMPONENT_TOP_IDS_ONLY=true \
TRAIN_COMPONENT_REQUIRED=true \
TRAIN_COMPONENT_TOP_IDS_ENABLE=true \
TRAIN_COMPONENT_TOP_IDS_ONLY=true \
EXP_ROOT="$EXP_ROOT" \
...
log_event "v290_train_done" "$EXP_ROOT"
