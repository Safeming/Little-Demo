...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-v298_boundary_protected_feature_refine_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
...
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v298_boundary_protected_feature_refine_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v298_boundary_protected_feature_refine_${RUN_ID}}"
...
Goal:
  v298 tests the structural failure mode exposed by v296/v297. Geometry,
  opacity, scale, rotation, pose and camera remain frozen. Texture MLP is
  frozen too; only Gaussian feature/SH rows may update, and boundary-tagged,
  live-boundary, or support-role rows have their feature gradients and Adam
  state cleared before every optimizer step. This verifies whether color
  changes on contour-contributor Gaussians are the cause of held-out raw
  silhouette breakage.
