...
START_CKPT="$ROOT/exp/stageA2/377_multiview_explicit_hq_rootfix_resume_v179c_v178c_k6_camquality_tightphoto_screen3000/best_ckpt.pth"

mkdir -p "$LOG_DIR"

BATCH_ID="${V180_RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
SUMMARY="$LOG_DIR/v180_batch_${BATCH_ID}_summary.tsv"
REGISTRY="$LOG_DIR/v180_batch_${BATCH_ID}_runs.txt"
RUNNER_LOG="$LOG_DIR/v180_batch_${BATCH_ID}.log"

printf '%s\n' "$RUNNER_LOG" > "$LOG_DIR/v180_latest_runner_log.txt"
printf '%s\n' "$SUMMARY" > "$LOG_DIR/v180_latest_summary.txt"
printf '%s\n' "$BATCH_ID" > "$LOG_DIR/v180_latest_run_id.txt"
...
run_render() {
  ...
  printf '%s\n' "$render_log" > "$LOG_DIR/v180_latest_render_log.txt"
...
run_one() {
  ...
  local exp_dir="$ROOT/exp/stageA2/377_multiview_explicit_hq_rootfix_resume_${tag}_v179c_${slug}_screen${iterations}"
  local train_log="$LOG_DIR/${tag}_${BATCH_ID}_${slug}.log"

  printf '%s\t%s\t%s\t%s\n' "$tag" "$exp_dir" "$train_log" "$option_name" >> "$REGISTRY"
  printf '%s\n' "$exp_dir" > "$LOG_DIR/v180_latest_exp.txt"
  printf '%s\n' "$train_log" > "$LOG_DIR/v180_latest_log.txt"

  if [ ! -f "$START_CKPT" ]; then
    echo "[$(date '+%F %T')] missing start checkpoint: $START_CKPT"
...
echo "[$(date '+%F %T')] v180 alignment-aware batch started"
echo "summary=$SUMMARY"
echo "registry=$REGISTRY"
echo "start_ckpt=$START_CKPT"

run_one "v180a" 2000 "stageA_377_multiview_explicit_hq_v180a_v179c_alignaware_edge_v1" "alignaware_edge"
run_one "v180b" 2000 "stageA_377_multiview_explicit_hq_v180b_v179c_alignaware_edge_luma_v1" "alignaware_edge_luma"

echo "[$(date '+%F %T')] v180 alignment-aware batch finished"
cat "$SUMMARY"
