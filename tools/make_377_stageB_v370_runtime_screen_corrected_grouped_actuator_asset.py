...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build v370 runtime-screen-corrected residual grouped actuator asset."
    )
    ...
    parser.add_argument("--screen-correct-enable", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--screen-correct-max-px", default=220.0, type=float)
                            ...
                            for minor_scale in minor_scales:
                                for covariance_scale in covariance_scales:
                                    ...
                                    for spec in specs:
                                        ...
                                        "screen_correct_enable": bool(args.screen_correct_enable),
                                        "screen_correct_max_px": float(args.screen_correct_max_px),
                                        ...
                                        "reason": "v370_runtime_screen_corrected_residual_grouped_inner_micro_child",
                                        "v369_mode": mode,
    ...
    payload = {
        "version": "v370_runtime_screen_corrected_grouped_actuator_asset",
        "policy": (
            "Subject-general residual discovery with runtime screen correction: current raw diagnostics "
            "select multiple residual targets per inner component row; each target becomes an action-level "
            "semantic-local-frame multi-micro child group corrected against the render camera target screen, "
            "and paired outer components become semantic local-3D protect/shrink actions. "
