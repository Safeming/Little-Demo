...
RUN_ID="${RUN_ID:-stageB_v259_clustered_multiview_support_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
GPU="${GPU:-0}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_stageB_v259_clustered_multiview_support_${RUN_ID}}"
mkdir -p "$LOG_DIR"
...
env \
  EXPERIMENT_VERSION="v259" \
  EXPERIMENT_SLUG="clustered_multiview_support" \
  TRAIN_NAME="v259a_clustered_multiview_support" \
  LOG_PREFIX="377_stageB_v259_clustered_multiview_support" \
  RENDER_TAG="v259" \
  PURPOSE="v259 clustered multiview support: fix 2D residual to 3D support placement by clustering per-parent target offsets and requiring distinct-view consensus before spawning support Gaussians" \
  RUN_ID="$RUN_ID" \
  GPU="$GPU" \
  LOG_DIR="$LOG_DIR" \
  WAIT_FOR_FREE_GPU=1 \
  ...
  BOUNDARY_COMPONENT_SUPPORT_PARENT_CONSENSUS_VERBOSE=true \
  ...
  BOUNDARY_COMPONENT_SUPPORT_PARENT_CONSENSUS_MAX_POINTS=72 \
...

{
  ...
  echo "EXPECTED_FINAL_CKPT=$ROOT/exp/stageB/377_hulk_light_v259a_clustered_multiview_support_${RUN_ID}/ckpt145310.pth"
