...
RUN_ID="${RUN_ID:-v394_config_consistent_mainline_gate_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/formal/logs/377_v394_config_consistent_mainline_gate_${RUN_ID}}"
GATE_RUN_ID="${GATE_RUN_ID:-formal_377_v394_config_consistent_dense_gate_${RUN_ID}}"
EXPORT_RUN_ID="${EXPORT_RUN_ID:-formal_377_v394_config_consistent_semantic_export_${RUN_ID}}"
EXPORT_EXP_DIR="${EXPORT_EXP_DIR:-$ROOT/exp/formal/377_v394_config_consistent_semantic_export_${RUN_ID}}"
...
CONFIG_CONTRACT=v392_selected_dense_gate_and_semantic_export_with_matching_directional_legacy_render_config
INFO

log_event dense_gate_start "$GATE_RUN_ID"
CANDIDATE_CKPT="$CANDIDATE_CKPT" \
CANDIDATE_VARIANT_NAME="candidate_v394_v392_selected_config_consistent_dense" \
...
"$ROOT/tools/formal/run_377_signed_geometry_export.sh" \
  ...
  "++model.gaussian.directional_boundary_residual_enable=true" \
  "++model.gaussian.directional_boundary_include_legacy_residual_enable=true" \
  "++model.gaussian.directional_boundary_residual_sign_clamp_enable=true" \
  "++model.gaussian.directional_boundary_residual_conflict_mode=freeze" \
  > "$EXPORT_LOG" 2>&1
...
candidate = next((row for row in rows if row.get("variant") == "candidate_v394_v392_selected_config_consistent_dense"), None)
