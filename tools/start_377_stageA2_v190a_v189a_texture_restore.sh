...
EXP_DIR="${1:-$ROOT/exp/stageA2/377_multiview_explicit_hq_rootfix_resume_v190a_v189a_texture_restore_$RUN_ID}"
...
exec env PYTHONUNBUFFERED=1 "$PYTHON_BIN" train.py \
  ...
  "++opt.lambda_camera_geometry_reg=0.024" \
  ...
  "++opt.lambda_perceptual_shoulder_arm=0.060" \
  "++opt.lambda_perceptual_waist=0.040" \
  "++opt.lambda_perceptual_face_patch=0.10" \
  "++opt.lambda_perceptual_shoulder_arm_patch=0.055" \
  "++opt.lambda_perceptual_waist_patch=0.035" \
  "++opt.lambda_perceptual_upper_torso_patch=0.055" \
  "++opt.lambda_perceptual_upper_torso_core_patch=0.055" \
  ...
  "++opt.reliable_view_highfreq_max_weight=1.18" \
  "opt.grad_clip=0.010" \
