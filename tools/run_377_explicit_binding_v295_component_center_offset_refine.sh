...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-v295_component_center_offset_refine_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
...
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v295_component_center_offset_refine_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v295_component_center_offset_refine_${RUN_ID}}"
...
Goal:
  v295 tests the current root-cause hypothesis directly: edge breakage is from
  wrong local boundary support position, not missing covariance size. The
  experiment keeps v281 dynamic_screen_mid as baseline and adds a render-time
  view/component-conditioned center offset on means3D. It does not write
  checkpoint geometry and does not use covariance grow as an inner fix.

Gate:
  Run raw no-train A/B first. Only a strict/probe pass gets a short color/SH/
  texture refine with xyz/opacity/scale/rotation frozen and render_export_refine=false.
EOF
