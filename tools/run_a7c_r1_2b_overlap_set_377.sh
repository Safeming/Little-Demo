...
OUT="${1:-${ROOT}/exp/acceptdata/a7c_r1_2b_dense_overlap_set_377_v1}"
PYTHON="/opt/miniconda3/envs/anim/bin/python"
CONTRACT="${ROOT}/configs/semantic/a7c_r1_2b_dense_overlap_set_377_v1.json"
DESIGN="${ROOT}/docs/superpowers/specs/2026-08-03-a7c-r1-2b-dense-overlap-set-compositor-design.md"
PARENT="${ROOT}/configs/semantic/a7c_r1_2a_quotient_compositor_377_v1.json"
...
mark_terminal() {
  local marker="$1"
  rm -f "${OUT}/.done" "${OUT}/.rejected" "${OUT}/.failed"
  touch "${OUT}/.${marker}"
}
...
check_sha "${DESIGN}" "fd099423bcb9bee1ae4b10608468744b511ab00d07866a5d8f5551e310302550"
check_sha "${PARENT}" "8f59a97ddbf730516ba0ba26871487210e2bddcb3cf8958619bca3a465c25ff4"
...
if [[ "${complete}" -eq 0 ]]; then
  "${PYTHON}" "${ROOT}/tools/train_a7c_r1_2b_overlap_set_compositor.py" \
    --contract "${CONTRACT}" \
    --probe "${PROBE}" \
    --evidence "${EVIDENCE}" \
    --a5-bank "${A5_BANK}" \
