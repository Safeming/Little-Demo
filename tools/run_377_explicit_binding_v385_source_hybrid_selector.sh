...
RUN_ID="${RUN_ID:-v385_source_hybrid_selector_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v385_source_hybrid_selector_${RUN_ID}}"
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v385_source_hybrid_selector_${RUN_ID}}"
...
V385_CLOSURE_ASSET_JSON="$ASSET_DIR/v385_source_hybrid_candidate_asset.json"
LAUNCH_LOG="$LOG_DIR/v385_launcher.log"
...
V385_CLOSURE_ASSET_JSON=$V385_CLOSURE_ASSET_JSON
...
"$PYTHON_BIN" - "$SOURCE_CLOSURE_ASSET_JSON" "$V385_CLOSURE_ASSET_JSON" <<'PY' > "$LOG_DIR/make_v385_asset.log" 2>&1
import json
import sys
...
def patch_child(item):
    ...
    item["v385_source_hybrid_selector"] = True
...
asset["version"] = "v385_source_hybrid_candidate_asset"
asset["policy"] = (
    "v382 residual candidate asset using source-frame source-screen activation and "
    "non-source projected-center activation, with hard outer drop only off source frames."
)
asset.setdefault("v385_runtime_contract", {})
asset["v385_runtime_contract"].update({
...
env -u LOG_DIR -u EXP_ROOT -u HYDRA_RUN_ROOT \
  ...
  RUN_ID="v385_selector_${RUN_ID}" \
  BASE_ASSET_JSON="$BASE_ASSET_JSON" \
  CLOSURE_ASSET_JSON="$V385_CLOSURE_ASSET_JSON" \
  PREFILTER_TSV="$PREFILTER_TSV" \
  CANARY_WORST_TSV="$CANARY_WORST_TSV" \
  CANARY_TOP_K="$CANARY_TOP_K" \
  MAX_CANDIDATES="$MAX_CANDIDATES" \
  ...
  --log-dir "$LOG_DIR/v385_selector" \
  --exp-root "$EXP_ROOT/v385_selector" \
  > "$LAUNCH_LOG" 2>&1

END_BJT="$(TZ=Asia/Shanghai date '+%F %T BJT')"
echo "LOG_DIR=$LOG_DIR"
echo "EXP_ROOT=$EXP_ROOT"
echo "V385_CLOSURE_ASSET_JSON=$V385_CLOSURE_ASSET_JSON"
