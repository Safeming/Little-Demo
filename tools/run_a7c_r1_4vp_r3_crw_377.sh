...
cd "${ROOT}"
OUT="${1:-${ROOT}/exp/acceptdata/a7c_r1_4vp_r3_crw_contribution_weighted_377_v1}"
...
expected_contract = "a81238d5daa8d0233cd36909f912c129aca0ec5bb72d457ce385633770322a19"
...
if not torch.cuda.is_available():
    raise RuntimeError("CUDA is unavailable for the formal R3 canary")
print(torch.cuda.get_device_name(0), flush=True)
PY

if [[ ! -f "${OUT}/models_frozen.json" ]]; then
  if "${PYTHON}" "${ROOT}/tools/train_a7c_r1_4vp_r3_crw.py" \
...
if "${PYTHON}" "${ROOT}/tools/audit_a7c_r1_4vp_r3_crw.py" \
