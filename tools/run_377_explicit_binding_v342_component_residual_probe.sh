...
RUN_ID="${RUN_ID:-v342_component_residual_probe_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v342_component_residual_probe_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v342_component_residual_probe_${RUN_ID}}"
...
WINDOW_SPECS="${WINDOW_SPECS:-c21_f480_481|[21]|[480,481,1];c22_f240_241|[22]|[240,241,1]}"
...
run_window() {
  ...
  render_and_analyze "$window" "$views_spec" "$frames_spec" baseline_base_no_preset "$BASE_CKPT" "$window_root/baseline_base_no_preset" \
  ...
  render_and_analyze "$window" "$views_spec" "$frames_spec" candidate_no_preset "$CANDIDATE_CKPT" "$window_root/candidate_no_preset" \
    "pipeline.compute_cov3D_python=true" \
    "++pipeline.covariance_mode=default" \
    "++pipeline.covariance_signed_dynamic_enable=false" \
    "++pipeline.covariance_signed_point_json=" \
  ...
  render_and_analyze "$window" "$views_spec" "$frames_spec" formal_v338_no_signed_dynamic "$CANDIDATE_CKPT" "$window_root/formal_v338_no_signed_dynamic" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_disable_signed_dynamic=true"

  render_and_analyze "$window" "$views_spec" "$frames_spec" formal_v338_no_geometry_fidelity "$CANDIDATE_CKPT" "$window_root/formal_v338_no_geometry_fidelity" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_disable_geometry_fidelity=true"

  render_and_analyze "$window" "$views_spec" "$frames_spec" formal_v338_no_component_paths "$CANDIDATE_CKPT" "$window_root/formal_v338_no_component_paths" \
    "++explicit_binding_render_preset=v338_temporal_selector_grow_only_guard" \
    "++explicit_binding_adopted_disable_signed_dynamic=true" \
    "++explicit_binding_adopted_disable_geometry_fidelity=true" \
...
variants = [
    "baseline_base_no_preset",
    "candidate_no_preset",
    "formal_v338_current",
    "formal_v338_no_signed_dynamic",
    "formal_v338_no_geometry_fidelity",
    "formal_v338_no_component_paths",
]
...
for window in windows:
    ...
    base = loaded["baseline_base_no_preset"]
    for variant, data in loaded.items():
        ...
        row = {
            ...
            **{f"{k}_delta_base": delta[k] for k in metrics},
            "status": "baseline" if variant == "baseline_base_no_preset" else status_for(delta),
        }
...
rows = []
base_total = totals["baseline_base_no_preset"]
for variant in variants:
    ...
    base = {k: base_total[k] / base_samples for k in metrics}
    delta = {k: data[k] - base[k] for k in metrics}
    rows.append({
        "variant": variant,
        "samples": data["samples"],
