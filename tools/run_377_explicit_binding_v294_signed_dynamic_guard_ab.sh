...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-v294_signed_dynamic_guard_ab_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
...
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v294_signed_dynamic_guard_ab_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v294_signed_dynamic_guard_ab_${RUN_ID}}"
...
Goal:
  v294 keeps the v281 all-frame view/component signed screen actuator as the
  baseline. It then adds a signed dynamic covariance guard only on the current
  residual component masks:
    outer residual component -> local shrink/aniso guard,
    inner residual component -> local grow/canonical or orth guard.
  This tests the root-cause mechanism: signed direction and footprint stability
  must be coupled at render time; global binding guards and point residual
  training are intentionally avoided.
EOF
...
render_raw() {
  ...
  local max_over="$9"
  local max_under="${10}"
  local guard_enable="${11}"
  local guard_shrink_mode="${12}"
  local guard_grow_mode="${13}"
  local guard_shrink_strength="${14}"
  local guard_grow_strength="${15}"
  local guard_power="${16}"
  local guard_min_weight="${17}"
  local guard_quantile="${18}"
  local guard_aniso_clamp="${19}"
  local hydra_dir="${20}"
  ...
  env "${COMMON_ENV[@]}" "$PYTHON_BIN" render.py \
    ...
    "++pipeline.covariance_signed_dynamic_score_weighting_enable=false" \
    "++pipeline.covariance_signed_dynamic_max_over_points=$max_over" \
    "++pipeline.covariance_signed_dynamic_max_under_points=$max_under" \
    "++pipeline.covariance_signed_dynamic_guard_enable=$guard_enable" \
    "++pipeline.covariance_signed_dynamic_guard_shrink_mode=$guard_shrink_mode" \
    "++pipeline.covariance_signed_dynamic_guard_grow_mode=$guard_grow_mode" \
    "++pipeline.covariance_signed_dynamic_guard_shrink_strength=$guard_shrink_strength" \
    "++pipeline.covariance_signed_dynamic_guard_grow_strength=$guard_grow_strength" \
    "++pipeline.covariance_signed_dynamic_guard_power=$guard_power" \
    "++pipeline.covariance_signed_dynamic_guard_min_weight=$guard_min_weight" \
    "++pipeline.covariance_signed_dynamic_guard_quantile=$guard_quantile" \
    "++pipeline.covariance_signed_dynamic_guard_anisotropy_clamp=$guard_aniso_clamp" \
...
append_summary() {
  ...
  local max_over="$8"
  local max_under="$9"
  local guard_enable="${10}"
  local guard_shrink_mode="${11}"
  local guard_grow_mode="${12}"
  local guard_shrink_strength="${13}"
  local guard_grow_strength="${14}"
  local guard_power="${15}"
  local guard_min_weight="${16}"
  local guard_quantile="${17}"
  local guard_aniso_clamp="${18}"
  local render_exp="${19}"

  "$PYTHON_BIN" - "$SUMMARY" "$variant" "$screen_shrink" "$screen_grow" "$component_required" "$component_signature" "$component_pad" "$component_scale" "$max_over" "$max_under" "$guard_enable" "$guard_shrink_mode" "$guard_grow_mode" "$guard_shrink_strength" "$guard_grow_strength" "$guard_power" "$guard_min_weight" "$guard_quantile" "$guard_aniso_clamp" "$render_exp" <<'PY'
import json
import sys
...
for row in lines[1:]:
    if row and row[0] == "baseline_v281_screen_mid":
        ...
        break
if baseline is None or variant == "baseline_v281_screen_mid":
...
run_variant() {
  ...
  render_raw "$variant" "$render_exp" "$screen_shrink" "$screen_grow" "$component_required" "$component_signature" "$component_pad" "$component_scale" "$max_over" "$max_under" "$guard_enable" "$guard_shrink_mode" "$guard_grow_mode" "$guard_shrink_strength" "$guard_grow_strength" "$guard_power" "$guard_min_weight" "$guard_quantile" "$guard_aniso_clamp" "$HYDRA_RUN_ROOT/render_${variant}" \
    > "$LOG_DIR/render_${variant}.log" 2>&1
  log_event "analyze_start" "$variant"
  analyze_raw "$variant" "$render_exp"
  append_summary "$variant" "$screen_shrink" "$screen_grow" "$component_required" "$component_signature" "$component_pad" "$component_scale" "$max_over" "$max_under" "$guard_enable" "$guard_shrink_mode" "$guard_grow_mode" "$guard_shrink_strength" "$guard_grow_strength" "$guard_power" "$guard_min_weight" "$guard_quantile" "$guard_aniso_clamp" "$render_exp"
  log_event "variant_done" "$variant"
}

run_variant baseline_v281_screen_mid 0.940 1.025 false false 10 1.25 96 96 false none none 0.00 0.00 1.0 0.00 0.90 1.20
run_variant shrink_aniso_soft 0.940 1.025 false false 10 1.25 96 96 true aniso_clamp none 0.18 0.00 1.0 0.15 0.90 1.18
run_variant grow_canonical_soft 0.940 1.025 false false 10 1.25 96 96 true none canonical_blend 0.00 0.12 1.0 0.15 0.90 1.20
run_variant signed_guard_balanced_soft 0.940 1.025 false false 10 1.25 96 96 true aniso_clamp canonical_blend 0.16 0.10 1.0 0.15 0.90 1.18
run_variant signed_guard_balanced_mid 0.940 1.025 false false 10 1.25 96 96 true aniso_clamp canonical_blend 0.24 0.16 1.0 0.20 0.90 1.15
run_variant signed_guard_orth_grow 0.940 1.025 false false 10 1.25 96 96 true aniso_clamp orthogonalized 0.20 0.14 1.0 0.20 0.90 1.18
run_variant signed_guard_component_required 0.940 1.025 true false 14 1.35 96 96 true aniso_clamp canonical_blend 0.18 0.12 1.0 0.20 0.90 1.18
run_variant signed_guard_signature 0.940 1.025 true true 18 1.55 96 96 true aniso_clamp canonical_blend 0.18 0.12 1.0 0.20 0.90 1.18
