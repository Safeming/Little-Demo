...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-v308_binding_internal_ab_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
...
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v308_binding_internal_ab_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v308_binding_internal_ab_${RUN_ID}}"
...
Goal:
  v308 tests whether the v307 adopted geometry correction can be reduced from
  residual-component CSV dependence to a binding-internal signed actuator based
  on layer/region/joint/boundary/surface/thin attributes. No training.
EOF
...
INTERNAL_RENDER_EXP="$EXP_ROOT/v308_binding_internal"

for spec in \
  "baseline_no_preset|$BASELINE_RENDER_EXP|none" \
  "v307_adopted_preset|$ADOPTED_RENDER_EXP|v307_adopted_geometry" \
  "v308_binding_internal|$INTERNAL_RENDER_EXP|v308_binding_internal"; do
  IFS='|' read -r variant render_exp preset <<< "$spec"
  log_event "render_start" "$variant"
  render_variant "$variant" "$render_exp" "$preset" "$HYDRA_RUN_ROOT/$variant"
  analyze_variant "$variant" "$render_exp"
  log_event "render_done" "$variant"
done

"$PYTHON_BIN" - "$SUMMARY" "$BASELINE_RENDER_EXP" "$ADOPTED_RENDER_EXP" "$INTERNAL_RENDER_EXP" <<'PY'
import csv
import json
import sys
from pathlib import Path
...
variant_paths = {
    ...
    "v308_binding_internal": Path(sys.argv[4]),
}
...
print(json.dumps({
    name: {"metrics": metrics[name], "render_exp": str(path)}
...
END_BJT="$(TZ=Asia/Shanghai date -d "@$END_EPOCH" '+%F %T BJT')"
{
  ...
  echo "INTERNAL_RENDER_EXP=$INTERNAL_RENDER_EXP"
