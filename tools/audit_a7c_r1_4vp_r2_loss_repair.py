...
def verify_frozen_artifacts(root: Path) -> dict[str, str]:
    ...
    if not isinstance(artifacts, dict) or len(artifacts) != 24:
        raise ValueError("R2 models_frozen must contain exactly 24 learned hashes")
    expected_names = {
    ...
    if set(artifacts) != expected_names:
        raise ValueError("R2 frozen artifact paths differ from contract")
    observed = {}
...
def _verify_prediction_bundle(root: Path, predictor: str, fold: int) -> None:
    prediction_path = root / predictor / f"fold_{fold}/predictions.npz"
    with np.load(prediction_path, allow_pickle=False) as source:
        ...
        if np.any(~np.isfinite(gates[prediction_mask])):
            raise ValueError(f"{predictor} prediction contains nonfinite values")
        if np.any(np.isfinite(gates[~prediction_mask])):
    ...
    if predictor == "training":
        certificate_path = root / predictor / f"fold_{fold}/projection_certificates.json"
        certificates = json.loads(certificate_path.read_text(encoding="utf-8"))
        if len(certificates) != 24 or any(
            float(row["maximum_primal_violation"]) > 1.0e-7
            for row in certificates
        ):
            raise ValueError("R2 projection certificates are incomplete")


def _summarize(records, contract) -> dict:
    summary = summarize_records(records, contract)
    summary["maximum_visibility_response_ratio"] = float(
        max(row["visibility_response_ratio"] for row in records)
    )
    ...
    summary["maximum_topology_mismatch_count"] = int(
        max(row["topology_mismatch_count"] for row in records)
    )
    summary["minimum_topology_slack"] = float(
        min(row["minimum_topology_slack"] for row in records)
    )
    summary["per_camera"] = {
        str(camera): {
            "outer_gain": float(np.mean([
                row["outer_gain"] for row in records if row["camera_index"] == camera
            ])),
            "boundary_gain": float(np.mean([
                row["boundary_gain"] for row in records if row["camera_index"] == camera
            ])),
        }
...
def classify_canary(learned_records, nearest_neighbor_records, contract) -> str:
    if len(learned_records) != 24 or len(nearest_neighbor_records) != 24:
        raise ValueError("R1.4-VP-R2 audit requires 24 learned and 24 NN records")
    ...
    per_camera_positive = all(
        value["outer_gain"] > 0.0 and value["boundary_gain"] > 0.0
        for value in learned["per_camera"].values()
    )
    superior = bool(
    ...
    passed = bool(
        ...
        and learned["maximum_visibility_response_ratio"]
            <= float(contract["maximum_visibility_response_ratio"]) + 1.0e-7
        and per_camera_positive and superior
    )
    return "CANARY_PROMOTED" if passed else "CANARY_NEGATIVE"


def _load_predictions(path: Path, expected_mask) -> np.ndarray:
    with np.load(path, allow_pickle=False) as source:
        mask = np.asarray(source["prediction_mask"], dtype=bool)
        gates = np.asarray(source["projected_gates"], dtype=np.float64)
    if not np.array_equal(mask, expected_mask):
        raise ValueError("prediction mask differs from frozen fit domain")
    return gates
...
def _evaluate_record(streams, gates, selected, pixels, fold, camera, guards) -> dict:
    ...
    return {
        ...
        "visibility_response_ratio": visibility_response_ratio(
            base_target, outputs["objective"]["target"], pixels[selected]
        ),
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Audit frozen R1.4-VP-R2 held blocks.")
    ...
    parser.add_argument("--nearest-neighbor-dir", type=Path, required=True)
...
def _run(args) -> tuple[dict, int]:
    ...
    artifact_hashes = verify_frozen_artifacts(args.frozen_root)
    source_hashes = {
    ...
    for fold in range(6):
        _verify_prediction_bundle(args.frozen_root, "training", fold)
        nn_path = args.nearest_neighbor_dir / f"fold_{fold}/predictions.npz"
        source_hashes[f"nearest neighbor {fold}"] = verify_source_file(
            nn_path, contract["source_nearest_neighbor_prediction_sha256"][fold],
            f"nearest neighbor {fold}",
        )
        _verify_prediction_bundle(args.nearest_neighbor_dir.parent, args.nearest_neighbor_dir.name, fold)
        witness_path = args.witness_dir / f"fold_{fold}/predictions.npz"
        source_hashes[f"R1.3-G witness {fold}"] = verify_source_file(
            witness_path, contract["source_r1_3g_witness_prediction_sha256"][fold],
            f"R1.3-G witness {fold}",
        )
    ...
    pixels = np.asarray(
        evidence["renderer_sequence_target_pixel_count"], np.float64
    )[:, part_index]
    support = np.asarray(
        evidence["temporal_consecutive_visible_count"], np.int64
    )[:, part_index] >= int(contract["min_pair_support"])
    ...
    for fold, held in enumerate(split["held_block_masks"]):
        learned = _load_predictions(
            args.frozen_root / f"training/fold_{fold}/predictions.npz", fit_domain
        )
        nearest = _load_predictions(
            args.nearest_neighbor_dir / f"fold_{fold}/predictions.npz", fit_domain
        )
        with np.load(args.witness_dir / f"fold_{fold}/predictions.npz", allow_pickle=False) as source:
            witness = np.asarray(source["replay_gates"], np.float64)
        held_fit = np.asarray(held, bool) & fit_domain
        for name, gates in (("learned", learned), ("nearest_neighbor", nearest)):
            candidate = lower[carriers][None, :] * gates[held_fit]
            topology = evaluate_topology_guard(
                lower[carriers], candidate, float(contract["selection_threshold"])
            )
            guards = {
                "topology_passed": topology["passed"],
                "topology_mismatch_count": topology["mismatch_count"],
                "minimum_topology_slack": topology["minimum_slack"],
                "coverage_passed": bool(
                    coverage >= float(contract["minimum_evidence_support_coverage"])
                ),
                "frozen_parts_passed": True,
                "weight_upper_bound_passed": bool(
                    np.all(candidate <= lower[carriers][None, :] + 1.0e-12)
                ),
            }
            for camera in range(4):
                selected = held_fit & (cameras == camera)
                records[name].append(_evaluate_record(
                    streams, gates, selected, pixels, fold, camera, guards
                ))
        oracle.append({
            "fold": fold,
            "learned_gate_mae": float(np.mean(np.abs(learned[held_fit] - witness[held_fit]))),
            "learned_temporal_difference_mae": float(np.mean(np.abs(
                np.diff(learned[held_fit], axis=0) - np.diff(witness[held_fit], axis=0)
            ))),
        })
    ...
    payload = {
        "stage": "r1_4vp_r2_held_canary", "verdict": verdict,
        "learned_summary": learned_summary,
        "nearest_neighbor_summary": nn_summary,
        "learned_records": records["learned"],
        "nearest_neighbor_records": records["nearest_neighbor"],
        "comparison_margins": {
...
def main(argv=None) -> int:
    ...
    try:
        ...
        payload = {
            "stage": "r1_4vp_r2_held_canary", "verdict": "TRAINING_ERROR",
