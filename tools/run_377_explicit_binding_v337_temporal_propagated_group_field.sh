...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-v337_temporal_propagated_group_field_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
...
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v337_temporal_propagated_group_field_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v337_temporal_propagated_group_field_${RUN_ID}}"
...
Goal:
  v337 tests whether v336's small gain is mainly caused by sparse keyframe
  coverage. It builds the same local_balanced group field, temporally
  propagates it to the full test-view frame range, and evaluates only
  formal + propagated no-train signed geometry variants.
EOF
...
propagate_field temporal_radius_15 15
propagate_field temporal_radius_30 30
propagate_field temporal_nearest_all all

BASELINE_EXP="$EXP_ROOT/baseline_no_preset"
FORMAL_EXP="$EXP_ROOT/formal_v320_v307_signed_geometry"
TEMPORAL_R15_EXP="$EXP_ROOT/formal_plus_temporal_radius_15"
TEMPORAL_R30_EXP="$EXP_ROOT/formal_plus_temporal_radius_30"
TEMPORAL_ALL_EXP="$EXP_ROOT/formal_plus_temporal_nearest_all"
...
render_formal_temporal formal_plus_temporal_radius_15 "$TEMPORAL_R15_EXP" "$FIELD_DIR/temporal_radius_15.json"
render_formal_temporal formal_plus_temporal_radius_30 "$TEMPORAL_R30_EXP" "$FIELD_DIR/temporal_radius_30.json"
render_formal_temporal formal_plus_temporal_nearest_all "$TEMPORAL_ALL_EXP" "$FIELD_DIR/temporal_nearest_all.json"

"$PYTHON_BIN" - "$SUMMARY" "$WORST_SUMMARY" \
  baseline_no_preset "$BASELINE_EXP" \
  formal_v320_v307_signed_geometry "$FORMAL_EXP" \
  formal_plus_temporal_radius_15 "$TEMPORAL_R15_EXP" \
  formal_plus_temporal_radius_30 "$TEMPORAL_R30_EXP" \
  formal_plus_temporal_nearest_all "$TEMPORAL_ALL_EXP" <<'PY'
import csv, json, sys
from pathlib import Path
