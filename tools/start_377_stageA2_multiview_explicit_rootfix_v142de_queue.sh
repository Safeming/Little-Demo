...
# Legacy filename kept for compatibility; only v142e remains worth rerunning.
QUEUE_ROOT="${1:-$ROOT/exp/comparisons/v142e_queue_$(date +%Y%m%d_%H%M%S)}"
ITERATIONS="${2:-2000}"
START_CKPT="${3:-$ROOT/exp/stageA2/377_multiview_explicit_hq_rootfix_fresh_v139a_v136a_boundarygeom_errordriven_resume/best_ckpt.pth}"
if [ "$#" -ge 3 ]; then
  shift 3
else
  shift "$#"
...
run_one \
  "v142e_signed_localrefresh_unsquare_residualpush" \
  "stageA_377_multiview_explicit_hq_v142e_v139a_signed_localrefresh_unsquare_residualpush_v1" \
  "$ROOT/exp/stageA2/377_multiview_explicit_hq_rootfix_fresh_v142e_v139a_signed_localrefresh_unsquare_residualpush_resume" \
  "$@"
