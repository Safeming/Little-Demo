...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-stageB_v265b_forced_support_only_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_stageB_v265b_forced_support_only_${RUN_ID}}"
mkdir -p "$LOG_DIR"
...
APPEND_EXP="${APPEND_EXP:-$ROOT/exp/stageB/377_hulk_light_v265a_verified_support_only_append_stageB_v265_verified_support_only_20260517_095827_bjt}"
APPEND_ITER="${APPEND_ITER:-135711}"
APPEND_CKPT="${APPEND_CKPT:-$APPEND_EXP/ckpt${APPEND_ITER}.pth}"

TRAIN_NAME="${TRAIN_NAME:-v265b_forced_support_only}"
...
FINAL_RENDER_EXP="${TRAIN_EXP}_render_v265b_ckpt${FINAL_ITER}"

SUMMARY="$LOG_DIR/summary.tsv"
EVENTS="$LOG_DIR/events.tsv"
STATUS_JSON="$LOG_DIR/status.json"
FINAL_GATE_JSON="$LOG_DIR/v265b_final_strong_gate.json"
...
check_required() {
  local missing=0
  for required in "$PYTHON_BIN" "$DATA_ROOT" "$PARSER_ROOT" "$COMPACT_MAPPING" "$BASELINE_JSON" "$SOURCE_CANDIDATE_SUMMARY" "$APPEND_EXP/.hydra/config.yaml" "$APPEND_CKPT"; do
    if [ ! -e "$required" ]; then
...
run_support_only_train() {
  ...
  log_event "$TRAIN_NAME" "train_start" "ckpt=$APPEND_CKPT iterations=$STAGEB_ITERATIONS final_iter=$FINAL_ITER"
...
render_ckpt "$TRAIN_EXP" "$TRAIN_EXP/ckpt${FINAL_ITER}.pth" "$FINAL_RENDER_EXP" "v265b_final_render" || true
run_gate "v265b_final_strong_gate" "$FINAL_RENDER_EXP" "$FINAL_GATE_JSON" || true
