...
run_residual_mining() {
  local out_dir="$LOG_DIR/v246a_residual_mining"
  log_event "$GPU" "v246a_residual_mining" "start" "render_exp=$BASE_RENDER_EXP"
  "$PYTHON_BIN" "$ROOT/tools/analyze_377_boundary_residuals.py" \
    ...
    --panel-width 220 \
    --out-dir "$out_dir" \
    > "$LOG_DIR/v246a_residual_mining.log" 2>&1
  local status=$?
  if [ "$status" -eq 0 ]; then
    log_event "$GPU" "v246a_residual_mining" "done" "$out_dir"
    summary_row "v246a_residual_mining" "diagnostic" "" "$BASE_RENDER_EXP" "" "ok" "$out_dir"
  else
    log_event "$GPU" "v246a_residual_mining" "failed" "status=$status"
    summary_row "v246a_residual_mining" "diagnostic" "" "$BASE_RENDER_EXP" "" "failed" "status=$status"
  fi
  return "$status"
}
...
render_checkpoint() {
  ...
  local render_exp="${exp_dir}_render_v246_ckpt${global_iter}"
  if [ ! -f "$ckpt" ]; then
...
train_v246b() {
  local name="v246b_boundary_footprint_projector"
  local iterations="${V246_ITERATIONS:-360}"
  local checkpoint_steps="${V246_CHECKPOINT_STEPS:-120,240,360}"
...

{
  ...
  echo "PURPOSE=v246 boundary footprint projector: footprint-neighborhood residual score + anchor-normal support clones + over-score shrink"
