...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-v309_point_prior_ab_$(TZ=Asia/Shanghai date '+%Y%m%d_%H%M%S_bjt')}"
...
EXP_ROOT="${EXP_ROOT:-$ROOT/exp/stageB/377_explicit_binding_v309_point_prior_ab_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$ROOT/exp/stageB/logs/377_explicit_binding_v309_point_prior_ab_${RUN_ID}}"
HYDRA_RUN_ROOT="$LOG_DIR/hydra_runtime"
EVENTS="$LOG_DIR/events.tsv"
SUMMARY="$LOG_DIR/summary.tsv"
POINT_PRIOR_JSON="$LOG_DIR/signed_point_prior.json"
...
Goal:
  v309 tests whether a checkpoint-level signed point prior can replace
  view-specific residual component localization. No training.
EOF
...
"$PYTHON_BIN" tools/make_377_signed_point_prior.py \
  ...
  --max-shrink "${MAX_SHRINK:-96}" \
  --max-grow "${MAX_GROW:-96}" \
  --min-abs-signed "${MIN_ABS_SIGNED:-0.0}" \
  --min-hit-gap "${MIN_HIT_GAP:-0}" \
...
render_variant() {
  ...
  if [ "$mode" = "adopted" ]; then
    ...
    extra_args=(
      ...
      "++pipeline.covariance_signed_max_shrink_points=${MAX_SHRINK:-96}"
      "++pipeline.covariance_signed_max_grow_points=${MAX_GROW:-96}"
...
POINT_RENDER_EXP="$EXP_ROOT/v309_point_prior"

for spec in \
  "baseline_no_preset|$BASELINE_RENDER_EXP|none" \
  "v307_adopted_preset|$ADOPTED_RENDER_EXP|adopted" \
  "v309_point_prior|$POINT_RENDER_EXP|point_prior"; do
...
"$PYTHON_BIN" - "$SUMMARY" "$BASELINE_RENDER_EXP" "$ADOPTED_RENDER_EXP" "$POINT_RENDER_EXP" <<'PY'
import csv
import json
import sys
...
variant_paths = {
    ...
    "v309_point_prior": Path(sys.argv[4]),
}
...
END_BJT="$(TZ=Asia/Shanghai date -d "@$END_EPOCH" '+%F %T BJT')"
{
  ...
  echo "POINT_RENDER_EXP=$POINT_RENDER_EXP"
