...
class Camera:
    def __init__(self, camera=None, **kwargs):
        ...
        for key in ('hard_mask', 'soft_mask', 'gt_alpha_mask'):
            if torch.is_tensor(self.data.get(key, None)):
                self.data[key] = self.data[key].float().to(self.data_device)
        ...
        if torch.is_tensor(self.data.get('posed_joints', None)):
            self.data['posed_joints'] = self.data['posed_joints'].float().to(self.data_device)
        for key, value in list(self.data.items()):
            if key.startswith('parsing_') and torch.is_tensor(value):
                self.data[key] = value.float().to(self.data_device)
