...
OUTPUT_ROOT="$ROOT/exp/acceptdata/a7_sparse_robust_v4_canary_377"
RUN_ROOT="$OUTPUT_ROOT/run_377_v4"
EVIDENCE="$ROOT/exp/acceptdata/a7_renderer_objective_v3_canary_377/evidence/377/evidence.npz"
...
A7_CONTRACT="$ROOT/configs/semantic/frozen_a7_sparse_robust_v4_canary_377.json"
...
log_event "A7 v4 sparse-robust start commit=$(git rev-parse HEAD) gpu=$GPU"

if [[ ! -s "$CANDIDATE_ROOT/candidate_index.json" ]]; then
  log_event "capacity start folds=4 candidate_count=1"
  start_epoch="$(date +%s)"
  "$PYTHON" tools/calibrate_sparse_robust_a7_weights.py \
    --a5-bank "$A5_BANK" \
    --evidence "$EVIDENCE" \
...
if [[ "${#CANDIDATES[@]}" -ne 1 ]]; then
  log_event "failed reason=expected_one_valid_candidate actual=${#CANDIDATES[@]}"
  exit 2
fi
candidate="${CANDIDATES[0]}"
log_event "validation candidate=$candidate cameras=${CAMERAS[*]}"
...
log_event "A7 v4 sparse-robust complete"
touch "$RUN_ROOT/.done"
