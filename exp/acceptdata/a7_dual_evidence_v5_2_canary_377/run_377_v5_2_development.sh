...
OUTPUT_ROOT="$ROOT/exp/acceptdata/a7_dual_evidence_v5_2_canary_377"
RUN_ROOT="$OUTPUT_ROOT/run_377_v5_2"
...
A7_CONTRACT="$ROOT/configs/semantic/frozen_a7_dual_evidence_v5_2_canary_377.json"
LOSO_CONFIG="$ROOT/exp/acceptdata/frozen_a5_five_subject_loso_stats_20260723/CoreView_377/loso_frozen_config.json"
EXPECTED_CANDIDATE=dual_evidence_constrained_v5_2
...
V5_2_PATHS=(
  ...
  tools/summarize_a7_v5_2_development.py
...
if ! git diff --quiet -- "${V5_2_PATHS[@]}" || ! git diff --cached --quiet -- "${V5_2_PATHS[@]}"; then
  log_event "failed reason=uncommitted_v5_2_implementation"
  exit 2
fi

log_event "A7 v5.2 dual-margin development start commit=$(git rev-parse HEAD) gpu=$GPU"
if [[ ! -s "$CANDIDATE_ROOT/candidate_index.json" ]]; then
  log_event "capacity start evidence=reuse training_target=0.9975 training_visibility=0.999"
  start_epoch="$(date +%s)"
...
candidate, _fingerprint = load_validated_candidate(index, contract)
...
if sha256(evidence) != contract["source_evidence_sha256"] or sha256(v4_bank) != contract["source_v4_bank_sha256"]:
    raise SystemExit("source artifact SHA-256 mismatch")
...
for camera in "${VALIDATION_CAMERAS[@]}"; do
  output="$OUTPUT_ROOT/development/$SUBJECT/$candidate/temporal/validation/c$camera"
  log="$RUN_ROOT/logs/${candidate}_validation_c${camera}.log"
  if [[ -s "$output/summary.json" && -s "$output/metrics.csv" ]]; then
    log_event "validation skip camera=c$camera reason=complete"
    continue
  fi
  if [[ -e "$output/summary.json" || -e "$output/metrics.csv" ]]; then
    log_event "failed reason=stale_partial_output stage=validation camera=c$camera"
    touch "$RUN_ROOT/.failed"
    exit 2
  fi
  log_event "validation start camera=c$camera"
  start_epoch="$(date +%s)"
  ...
  log_event "validation done camera=c$camera elapsed_seconds=$(( $(date +%s) - start_epoch ))"
done
...
if ! "$PYTHON" tools/summarize_a7_v5_2_development.py \
...
log_event "A7 v5.2 development complete paper_test_eligible=false"
touch "$RUN_ROOT/.done"
