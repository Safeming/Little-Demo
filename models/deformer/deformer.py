...
class Deformer(nn.Module):
    ...
    def forward(self, gaussians, camera, iteration, compute_loss=True):
        ...
        setattr(deformed_gaussians, 'canonical_xyz', gaussians.get_xyz)
        setattr(deformed_gaussians, 'canonical_gaussians', gaussians)
        deformed_gaussians = self.rigid(deformed_gaussians, iteration, camera)

        loss_reg.update(loss_non_rigid)
        if hasattr(self.rigid, 'get_loss_reg'):
            loss_reg.update(self.rigid.get_loss_reg())
