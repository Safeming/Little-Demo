...
class PoseCorrection(nn.Module):
    def __init__(self, config, metadata=None):
        ...
        v_template = np.load(get_body_model_misc_path('v_templates.npz'))[gender]
        lbs_weights = np.load(get_body_model_misc_path('skinning_weights_all.npz'))[gender]
        posedirs = np.load(get_body_model_misc_path('posedirs_all.npz'))[gender]
        posedirs = posedirs.reshape([posedirs.shape[0] * 3, -1]).T
        shapedirs = np.load(get_body_model_misc_path('shapedirs_all.npz'))[gender]
        J_regressor = np.load(get_body_model_misc_path('J_regressors.npz'))[gender]
        kintree_table = np.load(get_body_model_misc_path('kintree_table.npy'))
...
class DirectPoseOptimization(PoseCorrection):
    ...
    @staticmethod
    def _normalize_joint_ids(value):
        if value is None:
            return []
        ...
        for item in value:
            try:
                ...
                continue
            # pose_body stores SMPL joints 1..23; root joint 0 is root_orient.
            if 1 <= joint_id <= 23:
                ids.append(joint_id)
        return sorted(set(ids))

    def _configure_trainable_parameters(self):
        train_root = bool(self.cfg.get('train_root_orient', True))
        train_body = bool(self.cfg.get('train_pose_body', True))
        train_hand = bool(self.cfg.get('train_pose_hand', True))
        train_trans = bool(self.cfg.get('train_trans', True))
        train_betas = bool(self.cfg.get('train_betas', True))

        self.root_orients.weight.requires_grad_(train_root)
        self.pose_bodys.weight.requires_grad_(train_body)
        self.pose_hands.weight.requires_grad_(train_hand)
        self.trans.weight.requires_grad_(train_trans)
        self.betas.requires_grad_(train_betas)

        joint_ids = self._normalize_joint_ids(self.cfg.get('pose_body_train_joint_ids', None))
        if train_body and joint_ids:
            mask = torch.zeros_like(self.pose_bodys.weight)
            for joint_id in joint_ids:
                offset = (joint_id - 1) * 3
                mask[:, offset:offset + 3] = 1.0
            self.register_buffer('pose_body_train_mask', mask)

            def _mask_pose_body_grad(grad):
                return grad * self.pose_body_train_mask.to(device=grad.device, dtype=grad.dtype)

            self.pose_bodys.weight.register_hook(_mask_pose_body_grad)
        else:
            self.register_buffer('pose_body_train_mask', torch.ones_like(self.pose_bodys.weight))
